...
/*      */   
/*      */   public boolean trim() {
/* 1766 */     int l = HashCommon.arraySize(this.size, this.f);
...
/* 1798 */     int l = HashCommon.nextPowerOfTwo((int)Math.ceil((n / this.f)));
/* 1799 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1800 */       return true; 
/*      */     try {
/* 1802 */       rehash(l);
...
/* 1822 */     byte[] key = this.key;
/* 1823 */     float[] value = this.value;
/* 1824 */     int mask = newN - 1;
/* 1825 */     byte[] newKey = new byte[newN + 1];
/* 1826 */     float[] newValue = new float[newN + 1];
/* 1827 */     int i = this.first, prev = -1, newPrev = -1;
/* 1828 */     long[] link = this.link;
/* 1829 */     long[] newLink = new long[newN + 1];
/* 1830 */     this.first = -1;
/* 1831 */     for (int j = this.size; j-- != 0; ) {
/* 1832 */       int pos; if (key[i] == 0) {
/* 1833 */         pos = newN;
/*      */       } else {
/* 1835 */         pos = HashCommon.mix(key[i]) & mask;
/* 1836 */         while (newKey[pos] != 0)
/* 1837 */           pos = pos + 1 & mask; 
/*      */       } 
/* 1839 */       newKey[pos] = key[i];
/* 1840 */       newValue[pos] = value[i];
/* 1841 */       if (prev != -1) {
/* 1842 */         newLink[newPrev] = newLink[newPrev] ^ (newLink[newPrev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1843 */         newLink[pos] = newLink[pos] ^ (newLink[pos] ^ (newPrev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1844 */         newPrev = pos;
/*      */       } else {
/* 1846 */         newPrev = this.first = pos;
/*      */         
/* 1848 */         newLink[pos] = -1L;
/*      */       } 
/* 1850 */       int t = i;
...
/* 1852 */       prev = t;
/*      */     } 
/* 1854 */     this.link = newLink;
/* 1855 */     this.last = newPrev;
/* 1856 */     if (newPrev != -1)
/*      */     {
/* 1858 */       newLink[newPrev] = newLink[newPrev] | 0xFFFFFFFFL; } 
/* 1859 */     this.n = newN;
/* 1860 */     this.mask = mask;
/* 1861 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/* 1862 */     this.key = newKey;
/* 1863 */     this.value = newValue;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     try {
/* 1880 */       c = (Byte2FloatLinkedOpenHashMap)super.clone();
/* 1881 */     } catch (CloneNotSupportedException cantHappen) {
/* 1882 */       throw new InternalError();
/*      */     } 
/* 1884 */     c.keys = null;
/* 1885 */     c.values = null;
/* 1886 */     c.entries = null;
/* 1887 */     c.containsNullKey = this.containsNullKey;
/* 1888 */     c.key = (byte[])this.key.clone();
/* 1889 */     c.value = (float[])this.value.clone();
/* 1890 */     c.link = (long[])this.link.clone();
/* 1891 */     return c;
/*      */   }
/*      */ 
/*      */ 
...
/*      */   public int hashCode() {
/* 1904 */     int h = 0;
/* 1905 */     for (int j = realSize(), i = 0, t = 0; j-- != 0; ) {
/* 1906 */       while (this.key[i] == 0)
/* 1907 */         i++; 
/* 1908 */       t = this.key[i];
/* 1909 */       t ^= HashCommon.float2int(this.value[i]);
/* 1910 */       h += t;
/* 1911 */       i++;
/*      */     } 
/*      */     
/* 1914 */     if (this.containsNullKey)
/* 1915 */       h += HashCommon.float2int(this.value[this.n]); 
/* 1916 */     return h;
/*      */   }
/*      */   private void writeObject(ObjectOutputStream s) throws IOException {
/* 1919 */     byte[] key = this.key;
/* 1920 */     float[] value = this.value;
/* 1921 */     MapIterator i = new MapIterator();
/* 1922 */     s.defaultWriteObject();
/* 1923 */     for (int j = this.size; j-- != 0; ) {
/* 1924 */       int e = i.nextEntry();
/* 1925 */       s.writeByte(key[e]);
/* 1926 */       s.writeFloat(value[e]);
/*      */     } 
/*      */   }
/*      */   
/*      */   private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
/* 1931 */     s.defaultReadObject();
/* 1932 */     this.n = HashCommon.arraySize(this.size, this.f);
/* 1933 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/* 1934 */     this.mask = this.n - 1;
/* 1935 */     byte[] key = this.key = new byte[this.n + 1];
/* 1936 */     float[] value = this.value = new float[this.n + 1];
/* 1937 */     long[] link = this.link = new long[this.n + 1];
/* 1938 */     int prev = -1;
/* 1939 */     this.first = this.last = -1;
/*      */ 
/*      */     
/* 1942 */     for (int i = this.size; i-- != 0; ) {
/* 1943 */       int pos; byte k = s.readByte();
/* 1944 */       float v = s.readFloat();
/* 1945 */       if (k == 0) {
/* 1946 */         pos = this.n;
/* 1947 */         this.containsNullKey = true;
/*      */       } else {
/* 1949 */         pos = HashCommon.mix(k) & this.mask;
/* 1950 */         while (key[pos] != 0)
/* 1951 */           pos = pos + 1 & this.mask; 
/*      */       } 
/* 1953 */       key[pos] = k;
/* 1954 */       value[pos] = v;
/* 1955 */       if (this.first != -1) {
/* 1956 */         link[prev] = link[prev] ^ (link[prev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1957 */         link[pos] = link[pos] ^ (link[pos] ^ (prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1958 */         prev = pos; continue;
/*      */       } 
/* 1960 */       prev = this.first = pos;
/*      */       
/* 1962 */       link[pos] = link[pos] | 0xFFFFFFFF00000000L;
/*      */     } 
/*      */     
/* 1965 */     this.last = prev;
/* 1966 */     if (prev != -1)
/*      */     {
/* 1968 */       link[prev] = link[prev] | 0xFFFFFFFFL;
/*      */     }
/*      */   }
/*      */   
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\bytes\Byte2FloatLinkedOpenHashMap.class
