...
/*   70 */     this.actualComparator = ByteComparators.asByteComparator(this.storedComparator);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  143 */     this(ByteIterators.asByteIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  159 */     this(c);
/*  160 */     ByteArrays.ensureOffsetLength(a, offset, length);
...
/*  175 */     this(a, offset, length, (Comparator<? super Byte>)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  230 */     return (this.actualComparator == null) ? Byte.compare(k1, k2) : this.actualComparator.compare(k1, k2);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  939 */     int index = 0;
/*      */     SetIterator() {
/*  941 */       this.next = ByteRBTreeSet.this.firstEntry;
/*      */     }
/*      */     SetIterator(byte k) {
/*  944 */       if ((this.next = ByteRBTreeSet.this.locateKey(k)) != null)
/*  945 */         if (ByteRBTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1009 */       updatePrevious();
/* 1010 */       updateNext();
/* 1011 */       ByteRBTreeSet.this.remove(this.curr.key);
/* 1012 */       this.curr = null;
/*      */     }
/*      */   }
/*      */   
...
/* 1029 */     return new Subset((byte)0, true, to, false);
/*      */   }
/*      */   
/*      */   public ByteSortedSet tailSet(byte from) {
/* 1033 */     return new Subset(from, false, (byte)0, true);
/*      */   }
/*      */   
...
/* 1075 */       if (!bottom && !top && ByteRBTreeSet.this.compare(from, to) > 0) {
...
/* 1099 */       return ((this.bottom || ByteRBTreeSet.this.compare(k, this.from) >= 0) && (this.top || ByteRBTreeSet.this
/* 1100 */         .compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean contains(byte k) {
/* 1105 */       return (in(k) && ByteRBTreeSet.this.contains(k));
/*      */     }
/*      */     
/*      */     public boolean add(byte k) {
/* 1109 */       if (!in(k))
...
/* 1112 */       return ByteRBTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean remove(byte k) {
/* 1117 */       if (!in(k))
/* 1118 */         return false; 
/* 1119 */       return ByteRBTreeSet.this.remove(k);
/*      */     }
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public ByteComparator comparator() {
/* 1137 */       return ByteRBTreeSet.this.actualComparator;
/*      */     }
/*      */     
/*      */     public ByteBidirectionalIterator iterator() {
...
/* 1151 */       return (ByteRBTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */     
/*      */     public ByteSortedSet tailSet(byte from) {
/* 1155 */       if (this.bottom)
/* 1156 */         return new Subset(from, false, this.to, this.top); 
/* 1157 */       return (ByteRBTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */     
/*      */     public ByteSortedSet subSet(byte from, byte to) {
...
/* 1163 */       if (!this.top)
/* 1164 */         to = (ByteRBTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1165 */       if (!this.bottom)
/* 1166 */         from = (ByteRBTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1178 */       if (ByteRBTreeSet.this.tree == null) {
/* 1179 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1183 */       if (this.bottom) {
/* 1184 */         e = ByteRBTreeSet.this.firstEntry;
/*      */       } else {
/* 1186 */         e = ByteRBTreeSet.this.locateKey(this.from);
/*      */         
/* 1188 */         if (ByteRBTreeSet.this.compare(e.key, this.from) < 0) {
/* 1189 */           e = e.next();
/*      */         }
/*      */       } 
/*      */       
/* 1193 */       if (e == null || (!this.top && ByteRBTreeSet.this.compare(e.key, this.to) >= 0))
/* 1194 */         return null; 
/* 1195 */       return e;
/*      */     }
/*      */ 
...
/*      */       ByteRBTreeSet.Entry e;
/* 1204 */       if (ByteRBTreeSet.this.tree == null) {
/* 1205 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1209 */       if (this.top) {
/* 1210 */         e = ByteRBTreeSet.this.lastEntry;
/*      */       } else {
/* 1212 */         e = ByteRBTreeSet.this.locateKey(this.to);
/*      */         
/* 1214 */         if (ByteRBTreeSet.this.compare(e.key, this.to) >= 0) {
/* 1215 */           e = e.prev();
/*      */         }
/*      */       } 
/*      */       
/* 1219 */       if (e == null || (!this.bottom && ByteRBTreeSet.this.compare(e.key, this.from) < 0))
/* 1220 */         return null; 
/* 1221 */       return e;
/*      */     }
/*      */     
/*      */     public byte firstByte() {
/* 1225 */       ByteRBTreeSet.Entry e = firstEntry();
/* 1226 */       if (e == null)
...
/*      */     
/*      */     public byte lastByte() {
/* 1232 */       ByteRBTreeSet.Entry e = lastEntry();
/* 1233 */       if (e == null)
...
/*      */     {
/*      */       SubsetIterator() {
/* 1248 */         this.next = ByteRBTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       SubsetIterator(byte k) {
/* 1251 */         this();
/* 1252 */         if (this.next != null)
/* 1253 */           if (!ByteRBTreeSet.Subset.this.bottom && ByteRBTreeSet.this.compare(k, this.next.key) < 0) {
/* 1254 */             this.prev = null;
/* 1255 */           } else if (!ByteRBTreeSet.Subset.this.top && ByteRBTreeSet.this.compare(k, (this.prev = ByteRBTreeSet.Subset.this.lastEntry()).key) >= 0) {
/* 1256 */             this.next = null;
/*      */           } else {
/* 1258 */             this.next = ByteRBTreeSet.this.locateKey(k);
/* 1259 */             if (ByteRBTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1270 */         if (!ByteRBTreeSet.Subset.this.bottom && this.prev != null && ByteRBTreeSet.this.compare(this.prev.key, ByteRBTreeSet.Subset.this.from) < 0)
/* 1271 */           this.prev = null; 
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1275 */         this.next = this.next.next();
/* 1276 */         if (!ByteRBTreeSet.Subset.this.top && this.next != null && ByteRBTreeSet.this.compare(this.next.key, ByteRBTreeSet.Subset.this.to) >= 0) {
/* 1277 */           this.next = null;
/*      */         }
/*      */       }
/*      */     }
...
/*      */     ByteRBTreeSet c;
/*      */     try {
/* 1296 */       c = (ByteRBTreeSet)super.clone();
...
/* 1370 */     if (n == 1) {
/* 1371 */       Entry entry = new Entry(s.readByte());
...
/*      */ 
/*      */       
/* 1382 */       Entry entry = new Entry(s.readByte());
/* 1383 */       entry.black(true);
/* 1384 */       entry.right(new Entry(s.readByte()));
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\bytes\ByteRBTreeSet.class
