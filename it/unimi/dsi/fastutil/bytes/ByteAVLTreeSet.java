...
/*   69 */     this.actualComparator = ByteComparators.asByteComparator(this.storedComparator);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  142 */     this(ByteIterators.asByteIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  158 */     this(c);
/*  159 */     ByteArrays.ensureOffsetLength(a, offset, length);
...
/*  174 */     this(a, offset, length, (Comparator<? super Byte>)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  229 */     return (this.actualComparator == null) ? Byte.compare(k1, k2) : this.actualComparator.compare(k1, k2);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  980 */     int index = 0;
/*      */     SetIterator() {
/*  982 */       this.next = ByteAVLTreeSet.this.firstEntry;
/*      */     }
/*      */     SetIterator(byte k) {
/*  985 */       if ((this.next = ByteAVLTreeSet.this.locateKey(k)) != null)
/*  986 */         if (ByteAVLTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1050 */       updatePrevious();
/* 1051 */       updateNext();
/* 1052 */       ByteAVLTreeSet.this.remove(this.curr.key);
/* 1053 */       this.curr = null;
/*      */     }
/*      */   }
/*      */   
...
/* 1070 */     return new Subset((byte)0, true, to, false);
/*      */   }
/*      */   
/*      */   public ByteSortedSet tailSet(byte from) {
/* 1074 */     return new Subset(from, false, (byte)0, true);
/*      */   }
/*      */   
...
/* 1116 */       if (!bottom && !top && ByteAVLTreeSet.this.compare(from, to) > 0) {
...
/* 1140 */       return ((this.bottom || ByteAVLTreeSet.this.compare(k, this.from) >= 0) && (this.top || ByteAVLTreeSet.this
/* 1141 */         .compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean contains(byte k) {
/* 1146 */       return (in(k) && ByteAVLTreeSet.this.contains(k));
/*      */     }
/*      */     
/*      */     public boolean add(byte k) {
/* 1150 */       if (!in(k))
...
/* 1153 */       return ByteAVLTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean remove(byte k) {
/* 1158 */       if (!in(k))
/* 1159 */         return false; 
/* 1160 */       return ByteAVLTreeSet.this.remove(k);
/*      */     }
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public ByteComparator comparator() {
/* 1178 */       return ByteAVLTreeSet.this.actualComparator;
/*      */     }
/*      */     
/*      */     public ByteBidirectionalIterator iterator() {
...
/* 1192 */       return (ByteAVLTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */     
/*      */     public ByteSortedSet tailSet(byte from) {
/* 1196 */       if (this.bottom)
/* 1197 */         return new Subset(from, false, this.to, this.top); 
/* 1198 */       return (ByteAVLTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */     
/*      */     public ByteSortedSet subSet(byte from, byte to) {
...
/* 1204 */       if (!this.top)
/* 1205 */         to = (ByteAVLTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1206 */       if (!this.bottom)
/* 1207 */         from = (ByteAVLTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1219 */       if (ByteAVLTreeSet.this.tree == null) {
/* 1220 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1224 */       if (this.bottom) {
/* 1225 */         e = ByteAVLTreeSet.this.firstEntry;
/*      */       } else {
/* 1227 */         e = ByteAVLTreeSet.this.locateKey(this.from);
/*      */         
/* 1229 */         if (ByteAVLTreeSet.this.compare(e.key, this.from) < 0) {
/* 1230 */           e = e.next();
/*      */         }
/*      */       } 
/*      */       
/* 1234 */       if (e == null || (!this.top && ByteAVLTreeSet.this.compare(e.key, this.to) >= 0))
/* 1235 */         return null; 
/* 1236 */       return e;
/*      */     }
/*      */ 
...
/*      */       ByteAVLTreeSet.Entry e;
/* 1245 */       if (ByteAVLTreeSet.this.tree == null) {
/* 1246 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1250 */       if (this.top) {
/* 1251 */         e = ByteAVLTreeSet.this.lastEntry;
/*      */       } else {
/* 1253 */         e = ByteAVLTreeSet.this.locateKey(this.to);
/*      */         
/* 1255 */         if (ByteAVLTreeSet.this.compare(e.key, this.to) >= 0) {
/* 1256 */           e = e.prev();
/*      */         }
/*      */       } 
/*      */       
/* 1260 */       if (e == null || (!this.bottom && ByteAVLTreeSet.this.compare(e.key, this.from) < 0))
/* 1261 */         return null; 
/* 1262 */       return e;
/*      */     }
/*      */     
/*      */     public byte firstByte() {
/* 1266 */       ByteAVLTreeSet.Entry e = firstEntry();
/* 1267 */       if (e == null)
...
/*      */     
/*      */     public byte lastByte() {
/* 1273 */       ByteAVLTreeSet.Entry e = lastEntry();
/* 1274 */       if (e == null)
...
/*      */     {
/*      */       SubsetIterator() {
/* 1289 */         this.next = ByteAVLTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       SubsetIterator(byte k) {
/* 1292 */         this();
/* 1293 */         if (this.next != null)
/* 1294 */           if (!ByteAVLTreeSet.Subset.this.bottom && ByteAVLTreeSet.this.compare(k, this.next.key) < 0) {
/* 1295 */             this.prev = null;
/* 1296 */           } else if (!ByteAVLTreeSet.Subset.this.top && ByteAVLTreeSet.this.compare(k, (this.prev = ByteAVLTreeSet.Subset.this.lastEntry()).key) >= 0) {
/* 1297 */             this.next = null;
/*      */           } else {
/* 1299 */             this.next = ByteAVLTreeSet.this.locateKey(k);
/* 1300 */             if (ByteAVLTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1311 */         if (!ByteAVLTreeSet.Subset.this.bottom && this.prev != null && ByteAVLTreeSet.this.compare(this.prev.key, ByteAVLTreeSet.Subset.this.from) < 0)
/* 1312 */           this.prev = null; 
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1316 */         this.next = this.next.next();
/* 1317 */         if (!ByteAVLTreeSet.Subset.this.top && this.next != null && ByteAVLTreeSet.this.compare(this.next.key, ByteAVLTreeSet.Subset.this.to) >= 0) {
/* 1318 */           this.next = null;
/*      */         }
/*      */       }
/*      */     }
...
/*      */     ByteAVLTreeSet c;
/*      */     try {
/* 1337 */       c = (ByteAVLTreeSet)super.clone();
...
/* 1411 */     if (n == 1) {
/* 1412 */       Entry entry = new Entry(s.readByte());
...
/*      */ 
/*      */       
/* 1422 */       Entry entry = new Entry(s.readByte());
/* 1423 */       entry.right(new Entry(s.readByte()));
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\bytes\ByteAVLTreeSet.class
