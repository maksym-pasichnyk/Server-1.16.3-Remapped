...
/*  143 */     this.key = new byte[this.n + 1];
...
/*  239 */     this(ByteIterators.asByteIterator(i), f);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  249 */     this(ByteIterators.asByteIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  265 */     ByteArrays.ensureOffsetLength(a, offset, length);
...
/*  384 */     byte[] key = this.key; while (true) {
/*      */       byte curr; int last;
...
/*  658 */     Arrays.fill(this.key, (byte)0);
...
/*  825 */     int index = -1;
/*      */     SetIterator() {
/*  827 */       this.next = ByteLinkedOpenHashSet.this.first;
/*  828 */       this.index = 0;
/*      */     }
/*      */     SetIterator(byte from) {
/*  831 */       if (from == 0) {
/*  832 */         if (ByteLinkedOpenHashSet.this.containsNull) {
/*  833 */           this.next = (int)ByteLinkedOpenHashSet.this.link[ByteLinkedOpenHashSet.this.n];
/*  834 */           this.prev = ByteLinkedOpenHashSet.this.n;
/*      */           return;
/*      */         } 
/*  837 */         throw new NoSuchElementException("The key " + from + " does not belong to this set.");
/*      */       } 
/*  839 */       if (ByteLinkedOpenHashSet.this.key[ByteLinkedOpenHashSet.this.last] == from) {
/*  840 */         this.prev = ByteLinkedOpenHashSet.this.last;
/*  841 */         this.index = ByteLinkedOpenHashSet.this.size;
/*      */         
/*      */         return;
/*      */       } 
/*  845 */       byte[] key = ByteLinkedOpenHashSet.this.key;
/*  846 */       int pos = HashCommon.mix(from) & ByteLinkedOpenHashSet.this.mask;
/*      */       
/*  848 */       while (key[pos] != 0) {
/*  849 */         if (key[pos] == from) {
/*      */           
/*  851 */           this.next = (int)ByteLinkedOpenHashSet.this.link[pos];
/*  852 */           this.prev = pos;
/*      */           return;
/*      */         } 
/*  855 */         pos = pos + 1 & ByteLinkedOpenHashSet.this.mask;
/*      */       } 
...
/*  872 */       this.next = (int)ByteLinkedOpenHashSet.this.link[this.curr];
/*  873 */       this.prev = this.curr;
/*  874 */       if (this.index >= 0) {
/*  875 */         this.index++;
/*      */       }
/*      */       
/*  878 */       return ByteLinkedOpenHashSet.this.key[this.curr];
/*      */     }
/*      */     
...
/*  885 */       this.prev = (int)(ByteLinkedOpenHashSet.this.link[this.curr] >>> 32L);
/*  886 */       this.next = this.curr;
/*  887 */       if (this.index >= 0)
/*  888 */         this.index--; 
/*  889 */       return ByteLinkedOpenHashSet.this.key[this.curr];
/*      */     }
...
/*      */       } 
/*  898 */       if (this.next == -1) {
/*  899 */         this.index = ByteLinkedOpenHashSet.this.size;
/*      */         return;
/*      */       } 
/*  902 */       int pos = ByteLinkedOpenHashSet.this.first;
/*  903 */       this.index = 1;
/*  904 */       while (pos != this.prev) {
/*  905 */         pos = (int)ByteLinkedOpenHashSet.this.link[pos];
/*  906 */         this.index++;
/*      */       } 
/*      */     }
/*      */     
...
/*      */         
/*  929 */         this.index--;
/*  930 */         this.prev = (int)(ByteLinkedOpenHashSet.this.link[this.curr] >>> 32L);
/*      */       } else {
/*  932 */         this.next = (int)ByteLinkedOpenHashSet.this.link[this.curr];
/*  933 */       }  ByteLinkedOpenHashSet.this.size--;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/*  938 */       if (this.prev == -1) {
/*  939 */         ByteLinkedOpenHashSet.this.first = this.next;
/*      */       } else {
/*  941 */         ByteLinkedOpenHashSet.this.link[this.prev] = ByteLinkedOpenHashSet.this.link[this.prev] ^ (ByteLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  942 */       }  if (this.next == -1) {
/*  943 */         ByteLinkedOpenHashSet.this.last = this.prev;
/*      */       } else {
/*  945 */         ByteLinkedOpenHashSet.this.link[this.next] = ByteLinkedOpenHashSet.this.link[this.next] ^ (ByteLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/*  946 */       }  int pos = this.curr;
/*  947 */       this.curr = -1;
/*  948 */       if (pos == ByteLinkedOpenHashSet.this.n) {
/*  949 */         ByteLinkedOpenHashSet.this.containsNull = false;
/*  950 */         ByteLinkedOpenHashSet.this.key[ByteLinkedOpenHashSet.this.n] = 0;
/*      */       } else {
/*      */         
/*  953 */         byte[] key = ByteLinkedOpenHashSet.this.key;
/*      */         while (true) {
/*      */           byte curr;
/*      */           int last;
/*  957 */           pos = (last = pos) + 1 & ByteLinkedOpenHashSet.this.mask;
/*      */           while (true) {
/*  959 */             if ((curr = key[pos]) == 0) {
/*  960 */               key[last] = 0;
/*      */               return;
/*      */             } 
/*  963 */             int slot = HashCommon.mix(curr) & ByteLinkedOpenHashSet.this.mask;
/*  964 */             if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/*      */               break; 
/*  966 */             pos = pos + 1 & ByteLinkedOpenHashSet.this.mask;
/*      */           } 
/*  968 */           key[last] = curr;
...
/*  972 */             this.prev = last; 
/*  973 */           ByteLinkedOpenHashSet.this.fixPointers(pos, last);
/*      */         } 
/*      */       } 
/*      */     }
/*      */   }
...
/* 1075 */     byte[] key = this.key;
/* 1076 */     int mask = newN - 1;
/* 1077 */     byte[] newKey = new byte[newN + 1];
...
/* 1160 */     ByteIterator i = iterator();
/* 1161 */     s.defaultWriteObject();
/* 1162 */     for (int j = this.size; j-- != 0;)
/* 1163 */       s.writeByte(i.nextByte()); 
/*      */   }
/*      */   
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\bytes\ByteLinkedOpenHashSet.class
