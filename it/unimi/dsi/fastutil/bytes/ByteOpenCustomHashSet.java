...
/* 653 */     byte[] key = this.key;
/* 654 */     int mask = newN - 1;
/* 655 */     byte[] newKey = new byte[newN + 1];
/* 656 */     int i = this.n;
/* 657 */     for (int j = realSize(); j-- != 0; ) {
/* 658 */       while (key[--i] == 0); int pos;
/* 659 */       if (newKey[pos = HashCommon.mix(this.strategy.hashCode(key[i])) & mask] != 0)
/*     */       {
/* 661 */         while (newKey[pos = pos + 1 & mask] != 0); } 
...
/* 715 */     ByteIterator i = iterator();
/* 716 */     s.defaultWriteObject();
/* 717 */     for (int j = this.size; j-- != 0;)
/* 718 */       s.writeByte(i.nextByte()); 
/*     */   }
/*     */   
...
/* 726 */     byte[] key = this.key = new byte[this.n + 1];
/*     */     
/* 728 */     for (int i = this.size; i-- != 0; ) {
/* 729 */       int pos; byte k = s.readByte();
/* 730 */       if (this.strategy.equals(k, (byte)0)) {
/* 731 */         pos = this.n;
/* 732 */         this.containsNull = true;
/*     */       }
/* 734 */       else if (key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask] != 0) {
/* 735 */         while (key[pos = pos + 1 & this.mask] != 0);
/*     */       } 
/* 737 */       key[pos] = k;
/*     */     } 
/*     */   }
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\bytes\ByteOpenCustomHashSet.class
