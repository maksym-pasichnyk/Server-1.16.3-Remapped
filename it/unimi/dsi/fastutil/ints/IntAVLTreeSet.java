...
/*   69 */     this.actualComparator = IntComparators.asIntComparator(this.storedComparator);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  142 */     this(IntIterators.asIntIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  158 */     this(c);
/*  159 */     IntArrays.ensureOffsetLength(a, offset, length);
...
/*  174 */     this(a, offset, length, (Comparator<? super Integer>)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  229 */     return (this.actualComparator == null) ? Integer.compare(k1, k2) : this.actualComparator.compare(k1, k2);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  980 */     int index = 0;
/*      */     SetIterator() {
/*  982 */       this.next = IntAVLTreeSet.this.firstEntry;
/*      */     }
/*      */     SetIterator(int k) {
/*  985 */       if ((this.next = IntAVLTreeSet.this.locateKey(k)) != null)
/*  986 */         if (IntAVLTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1050 */       updatePrevious();
/* 1051 */       updateNext();
/* 1052 */       IntAVLTreeSet.this.remove(this.curr.key);
/* 1053 */       this.curr = null;
/*      */     }
/*      */   }
/*      */   
...
/* 1070 */     return new Subset(0, true, to, false);
/*      */   }
/*      */   
/*      */   public IntSortedSet tailSet(int from) {
/* 1074 */     return new Subset(from, false, 0, true);
/*      */   }
/*      */   
...
/* 1116 */       if (!bottom && !top && IntAVLTreeSet.this.compare(from, to) > 0) {
...
/* 1140 */       return ((this.bottom || IntAVLTreeSet.this.compare(k, this.from) >= 0) && (this.top || IntAVLTreeSet.this
/* 1141 */         .compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean contains(int k) {
/* 1146 */       return (in(k) && IntAVLTreeSet.this.contains(k));
/*      */     }
/*      */     
/*      */     public boolean add(int k) {
/* 1150 */       if (!in(k))
...
/* 1153 */       return IntAVLTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean remove(int k) {
/* 1158 */       if (!in(k))
/* 1159 */         return false; 
/* 1160 */       return IntAVLTreeSet.this.remove(k);
/*      */     }
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public IntComparator comparator() {
/* 1178 */       return IntAVLTreeSet.this.actualComparator;
/*      */     }
/*      */     
/*      */     public IntBidirectionalIterator iterator() {
...
/* 1192 */       return (IntAVLTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */     
/*      */     public IntSortedSet tailSet(int from) {
/* 1196 */       if (this.bottom)
/* 1197 */         return new Subset(from, false, this.to, this.top); 
/* 1198 */       return (IntAVLTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */     
/*      */     public IntSortedSet subSet(int from, int to) {
...
/* 1204 */       if (!this.top)
/* 1205 */         to = (IntAVLTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1206 */       if (!this.bottom)
/* 1207 */         from = (IntAVLTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1219 */       if (IntAVLTreeSet.this.tree == null) {
/* 1220 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1224 */       if (this.bottom) {
/* 1225 */         e = IntAVLTreeSet.this.firstEntry;
/*      */       } else {
/* 1227 */         e = IntAVLTreeSet.this.locateKey(this.from);
/*      */         
/* 1229 */         if (IntAVLTreeSet.this.compare(e.key, this.from) < 0) {
/* 1230 */           e = e.next();
/*      */         }
/*      */       } 
/*      */       
/* 1234 */       if (e == null || (!this.top && IntAVLTreeSet.this.compare(e.key, this.to) >= 0))
/* 1235 */         return null; 
/* 1236 */       return e;
/*      */     }
/*      */ 
...
/*      */       IntAVLTreeSet.Entry e;
/* 1245 */       if (IntAVLTreeSet.this.tree == null) {
/* 1246 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1250 */       if (this.top) {
/* 1251 */         e = IntAVLTreeSet.this.lastEntry;
/*      */       } else {
/* 1253 */         e = IntAVLTreeSet.this.locateKey(this.to);
/*      */         
/* 1255 */         if (IntAVLTreeSet.this.compare(e.key, this.to) >= 0) {
/* 1256 */           e = e.prev();
/*      */         }
/*      */       } 
/*      */       
/* 1260 */       if (e == null || (!this.bottom && IntAVLTreeSet.this.compare(e.key, this.from) < 0))
/* 1261 */         return null; 
/* 1262 */       return e;
/*      */     }
/*      */     
/*      */     public int firstInt() {
/* 1266 */       IntAVLTreeSet.Entry e = firstEntry();
/* 1267 */       if (e == null)
...
/*      */     
/*      */     public int lastInt() {
/* 1273 */       IntAVLTreeSet.Entry e = lastEntry();
/* 1274 */       if (e == null)
...
/*      */     {
/*      */       SubsetIterator() {
/* 1289 */         this.next = IntAVLTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       SubsetIterator(int k) {
/* 1292 */         this();
/* 1293 */         if (this.next != null)
/* 1294 */           if (!IntAVLTreeSet.Subset.this.bottom && IntAVLTreeSet.this.compare(k, this.next.key) < 0) {
/* 1295 */             this.prev = null;
/* 1296 */           } else if (!IntAVLTreeSet.Subset.this.top && IntAVLTreeSet.this.compare(k, (this.prev = IntAVLTreeSet.Subset.this.lastEntry()).key) >= 0) {
/* 1297 */             this.next = null;
/*      */           } else {
/* 1299 */             this.next = IntAVLTreeSet.this.locateKey(k);
/* 1300 */             if (IntAVLTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1311 */         if (!IntAVLTreeSet.Subset.this.bottom && this.prev != null && IntAVLTreeSet.this.compare(this.prev.key, IntAVLTreeSet.Subset.this.from) < 0)
/* 1312 */           this.prev = null; 
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1316 */         this.next = this.next.next();
/* 1317 */         if (!IntAVLTreeSet.Subset.this.top && this.next != null && IntAVLTreeSet.this.compare(this.next.key, IntAVLTreeSet.Subset.this.to) >= 0) {
/* 1318 */           this.next = null;
/*      */         }
/*      */       }
/*      */     }
...
/*      */     IntAVLTreeSet c;
/*      */     try {
/* 1337 */       c = (IntAVLTreeSet)super.clone();
...
/* 1411 */     if (n == 1) {
/* 1412 */       Entry entry = new Entry(s.readInt());
...
/*      */ 
/*      */       
/* 1422 */       Entry entry = new Entry(s.readInt());
/* 1423 */       entry.right(new Entry(s.readInt()));
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\ints\IntAVLTreeSet.class
