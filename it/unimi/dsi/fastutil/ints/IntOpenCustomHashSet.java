...
/* 653 */     int mask = newN - 1;
/* 654 */     int[] newKey = new int[newN + 1];
/* 655 */     int i = this.n;
/* 656 */     for (int j = realSize(); j-- != 0; ) {
/* 657 */       while (key[--i] == 0); int pos;
/* 658 */       if (newKey[pos = HashCommon.mix(this.strategy.hashCode(key[i])) & mask] != 0)
/* 659 */         while (newKey[pos = pos + 1 & mask] != 0); 
...
/* 724 */     int[] key = this.key = new int[this.n + 1];
/*     */     
/* 726 */     for (int i = this.size; i-- != 0; ) {
/* 727 */       int pos, k = s.readInt();
/* 728 */       if (this.strategy.equals(k, 0)) {
/* 729 */         pos = this.n;
/* 730 */         this.containsNull = true;
/*     */       }
/* 732 */       else if (key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask] != 0) {
/* 733 */         while (key[pos = pos + 1 & this.mask] != 0);
/*     */       } 
/* 735 */       key[pos] = k;
/*     */     } 
/*     */   }
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\ints\IntOpenCustomHashSet.class
