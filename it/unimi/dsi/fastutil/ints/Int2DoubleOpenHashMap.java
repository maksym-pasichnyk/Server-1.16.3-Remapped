/*      */ package it.unimi.dsi.fastutil.ints;
/*      */ 
/*      */ import it.unimi.dsi.fastutil.Hash;
/*      */ import it.unimi.dsi.fastutil.HashCommon;
/*      */ import it.unimi.dsi.fastutil.doubles.AbstractDoubleCollection;
/*      */ import it.unimi.dsi.fastutil.doubles.DoubleCollection;
/*      */ import it.unimi.dsi.fastutil.doubles.DoubleIterator;
/*      */ import it.unimi.dsi.fastutil.objects.AbstractObjectSet;
/*      */ import it.unimi.dsi.fastutil.objects.ObjectIterator;
/*      */ import it.unimi.dsi.fastutil.objects.ObjectSet;
/*      */ import java.io.IOException;
/*      */ import java.io.ObjectInputStream;
/*      */ import java.io.ObjectOutputStream;
/*      */ import java.io.Serializable;
/*      */ import java.util.Arrays;
/*      */ import java.util.Collection;
/*      */ import java.util.Iterator;
/*      */ import java.util.Map;
/*      */ import java.util.NoSuchElementException;
/*      */ import java.util.Objects;
/*      */ import java.util.Set;
/*      */ import java.util.function.BiFunction;
/*      */ import java.util.function.Consumer;
/*      */ import java.util.function.DoubleConsumer;
/*      */ import java.util.function.IntConsumer;
/*      */ import java.util.function.IntFunction;
/*      */ import java.util.function.IntToDoubleFunction;
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ public class Int2DoubleOpenHashMap
/*      */   extends AbstractInt2DoubleMap
/*      */   implements Serializable, Cloneable, Hash
/*      */ {
/*      */   private static final long serialVersionUID = 0L;
/*      */   private static final boolean ASSERTS = false;
/*      */   protected transient int[] key;
/*      */   protected transient double[] value;
/*      */   protected transient int mask;
/*      */   protected transient boolean containsNullKey;
/*      */   protected transient int n;
/*      */   protected transient int maxFill;
/*      */   protected final transient int minN;
/*      */   protected int size;
/*      */   protected final float f;
/*      */   protected transient Int2DoubleMap.FastEntrySet entries;
/*      */   protected transient IntSet keys;
/*      */   protected transient DoubleCollection values;
/*      */   
/*      */   public Int2DoubleOpenHashMap(int expected, float f) {
/*   96 */     if (f <= 0.0F || f > 1.0F)
/*   97 */       throw new IllegalArgumentException("Load factor must be greater than 0 and smaller than or equal to 1"); 
/*   98 */     if (expected < 0)
/*   99 */       throw new IllegalArgumentException("The expected number of elements must be nonnegative"); 
/*  100 */     this.f = f;
/*  101 */     this.minN = this.n = HashCommon.arraySize(expected, f);
/*  102 */     this.mask = this.n - 1;
/*  103 */     this.maxFill = HashCommon.maxFill(this.n, f);
/*  104 */     this.key = new int[this.n + 1];
/*  105 */     this.value = new double[this.n + 1];
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public Int2DoubleOpenHashMap(int expected) {
/*  114 */     this(expected, 0.75F);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public Int2DoubleOpenHashMap() {
/*  122 */     this(16, 0.75F);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public Int2DoubleOpenHashMap(Map<? extends Integer, ? extends Double> m, float f) {
/*  133 */     this(m.size(), f);
/*  134 */     putAll(m);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public Int2DoubleOpenHashMap(Map<? extends Integer, ? extends Double> m) {
/*  144 */     this(m, 0.75F);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public Int2DoubleOpenHashMap(Int2DoubleMap m, float f) {
/*  155 */     this(m.size(), f);
/*  156 */     putAll(m);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public Int2DoubleOpenHashMap(Int2DoubleMap m) {
/*  166 */     this(m, 0.75F);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public Int2DoubleOpenHashMap(int[] k, double[] v, float f) {
/*  181 */     this(k.length, f);
/*  182 */     if (k.length != v.length) {
/*  183 */       throw new IllegalArgumentException("The key array and the value array have different lengths (" + k.length + " and " + v.length + ")");
/*      */     }
/*  185 */     for (int i = 0; i < k.length; i++) {
/*  186 */       put(k[i], v[i]);
/*      */     }
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public Int2DoubleOpenHashMap(int[] k, double[] v) {
/*  200 */     this(k, v, 0.75F);
/*      */   }
/*      */   private int realSize() {
/*  203 */     return this.containsNullKey ? (this.size - 1) : this.size;
/*      */   }
/*      */   private void ensureCapacity(int capacity) {
/*  206 */     int needed = HashCommon.arraySize(capacity, this.f);
/*  207 */     if (needed > this.n)
/*  208 */       rehash(needed); 
/*      */   }
/*      */   private void tryCapacity(long capacity) {
/*  211 */     int needed = (int)Math.min(1073741824L, 
/*  212 */         Math.max(2L, HashCommon.nextPowerOfTwo((long)Math.ceil(((float)capacity / this.f)))));
/*  213 */     if (needed > this.n)
/*  214 */       rehash(needed); 
/*      */   }
/*      */   private double removeEntry(int pos) {
/*  217 */     double oldValue = this.value[pos];
/*  218 */     this.size--;
/*  219 */     shiftKeys(pos);
/*  220 */     if (this.n > this.minN && this.size < this.maxFill / 4 && this.n > 16)
/*  221 */       rehash(this.n / 2); 
/*  222 */     return oldValue;
/*      */   }
/*      */   private double removeNullEntry() {
/*  225 */     this.containsNullKey = false;
/*  226 */     double oldValue = this.value[this.n];
/*  227 */     this.size--;
/*  228 */     if (this.n > this.minN && this.size < this.maxFill / 4 && this.n > 16)
/*  229 */       rehash(this.n / 2); 
/*  230 */     return oldValue;
/*      */   }
/*      */   
/*      */   public void putAll(Map<? extends Integer, ? extends Double> m) {
/*  234 */     if (this.f <= 0.5D) {
/*  235 */       ensureCapacity(m.size());
/*      */     } else {
/*  237 */       tryCapacity((size() + m.size()));
/*      */     } 
/*  239 */     super.putAll(m);
/*      */   }
/*      */   
/*      */   private int find(int k) {
/*  243 */     if (k == 0) {
/*  244 */       return this.containsNullKey ? this.n : -(this.n + 1);
/*      */     }
/*  246 */     int[] key = this.key;
/*      */     
/*      */     int curr, pos;
/*  249 */     if ((curr = key[pos = HashCommon.mix(k) & this.mask]) == 0)
/*  250 */       return -(pos + 1); 
/*  251 */     if (k == curr) {
/*  252 */       return pos;
/*      */     }
/*      */     while (true) {
/*  255 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0)
/*  256 */         return -(pos + 1); 
/*  257 */       if (k == curr)
/*  258 */         return pos; 
/*      */     } 
/*      */   }
/*      */   private void insert(int pos, int k, double v) {
/*  262 */     if (pos == this.n)
/*  263 */       this.containsNullKey = true; 
/*  264 */     this.key[pos] = k;
/*  265 */     this.value[pos] = v;
/*  266 */     if (this.size++ >= this.maxFill) {
/*  267 */       rehash(HashCommon.arraySize(this.size + 1, this.f));
/*      */     }
/*      */   }
/*      */ 
/*      */   
/*      */   public double put(int k, double v) {
/*  273 */     int pos = find(k);
/*  274 */     if (pos < 0) {
/*  275 */       insert(-pos - 1, k, v);
/*  276 */       return this.defRetValue;
/*      */     } 
/*  278 */     double oldValue = this.value[pos];
/*  279 */     this.value[pos] = v;
/*  280 */     return oldValue;
/*      */   }
/*      */   private double addToValue(int pos, double incr) {
/*  283 */     double oldValue = this.value[pos];
/*  284 */     this.value[pos] = oldValue + incr;
/*  285 */     return oldValue;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public double addTo(int k, double incr) {
/*      */     int pos;
/*  305 */     if (k == 0) {
/*  306 */       if (this.containsNullKey)
/*  307 */         return addToValue(this.n, incr); 
/*  308 */       pos = this.n;
/*  309 */       this.containsNullKey = true;
/*      */     } else {
/*      */       
/*  312 */       int[] key = this.key;
/*      */       int curr;
/*  314 */       if ((curr = key[pos = HashCommon.mix(k) & this.mask]) != 0) {
/*  315 */         if (curr == k)
/*  316 */           return addToValue(pos, incr); 
/*  317 */         while ((curr = key[pos = pos + 1 & this.mask]) != 0) {
/*  318 */           if (curr == k)
/*  319 */             return addToValue(pos, incr); 
/*      */         } 
/*      */       } 
/*  322 */     }  this.key[pos] = k;
/*  323 */     this.value[pos] = this.defRetValue + incr;
/*  324 */     if (this.size++ >= this.maxFill) {
/*  325 */       rehash(HashCommon.arraySize(this.size + 1, this.f));
/*      */     }
/*      */     
/*  328 */     return this.defRetValue;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected final void shiftKeys(int pos) {
/*  341 */     int[] key = this.key; while (true) {
/*      */       int curr, last;
/*  343 */       pos = (last = pos) + 1 & this.mask;
/*      */       while (true) {
/*  345 */         if ((curr = key[pos]) == 0) {
/*  346 */           key[last] = 0;
/*      */           return;
/*      */         } 
/*  349 */         int slot = HashCommon.mix(curr) & this.mask;
/*  350 */         if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/*      */           break; 
/*  352 */         pos = pos + 1 & this.mask;
/*      */       } 
/*  354 */       key[last] = curr;
/*  355 */       this.value[last] = this.value[pos];
/*      */     } 
/*      */   }
/*      */ 
/*      */   
/*      */   public double remove(int k) {
/*  361 */     if (k == 0) {
/*  362 */       if (this.containsNullKey)
/*  363 */         return removeNullEntry(); 
/*  364 */       return this.defRetValue;
/*      */     } 
/*      */     
/*  367 */     int[] key = this.key;
/*      */     
/*      */     int curr, pos;
/*  370 */     if ((curr = key[pos = HashCommon.mix(k) & this.mask]) == 0)
/*  371 */       return this.defRetValue; 
/*  372 */     if (k == curr)
/*  373 */       return removeEntry(pos); 
/*      */     while (true) {
/*  375 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0)
/*  376 */         return this.defRetValue; 
/*  377 */       if (k == curr) {
/*  378 */         return removeEntry(pos);
/*      */       }
/*      */     } 
/*      */   }
/*      */   
/*      */   public double get(int k) {
/*  384 */     if (k == 0) {
/*  385 */       return this.containsNullKey ? this.value[this.n] : this.defRetValue;
/*      */     }
/*  387 */     int[] key = this.key;
/*      */     
/*      */     int curr, pos;
/*  390 */     if ((curr = key[pos = HashCommon.mix(k) & this.mask]) == 0)
/*  391 */       return this.defRetValue; 
/*  392 */     if (k == curr) {
/*  393 */       return this.value[pos];
/*      */     }
/*      */     while (true) {
/*  396 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0)
/*  397 */         return this.defRetValue; 
/*  398 */       if (k == curr) {
/*  399 */         return this.value[pos];
/*      */       }
/*      */     } 
/*      */   }
/*      */   
/*      */   public boolean containsKey(int k) {
/*  405 */     if (k == 0) {
/*  406 */       return this.containsNullKey;
/*      */     }
/*  408 */     int[] key = this.key;
/*      */     
/*      */     int curr, pos;
/*  411 */     if ((curr = key[pos = HashCommon.mix(k) & this.mask]) == 0)
/*  412 */       return false; 
/*  413 */     if (k == curr) {
/*  414 */       return true;
/*      */     }
/*      */     while (true) {
/*  417 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0)
/*  418 */         return false; 
/*  419 */       if (k == curr)
/*  420 */         return true; 
/*      */     } 
/*      */   }
/*      */   
/*      */   public boolean containsValue(double v) {
/*  425 */     double[] value = this.value;
/*  426 */     int[] key = this.key;
/*  427 */     if (this.containsNullKey && Double.doubleToLongBits(value[this.n]) == Double.doubleToLongBits(v))
/*  428 */       return true; 
/*  429 */     for (int i = this.n; i-- != 0;) {
/*  430 */       if (key[i] != 0 && Double.doubleToLongBits(value[i]) == Double.doubleToLongBits(v))
/*  431 */         return true; 
/*  432 */     }  return false;
/*      */   }
/*      */ 
/*      */ 
/*      */   
/*      */   public double getOrDefault(int k, double defaultValue) {
/*  438 */     if (k == 0) {
/*  439 */       return this.containsNullKey ? this.value[this.n] : defaultValue;
/*      */     }
/*  441 */     int[] key = this.key;
/*      */     
/*      */     int curr, pos;
/*  444 */     if ((curr = key[pos = HashCommon.mix(k) & this.mask]) == 0)
/*  445 */       return defaultValue; 
/*  446 */     if (k == curr) {
/*  447 */       return this.value[pos];
/*      */     }
/*      */     while (true) {
/*  450 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0)
/*  451 */         return defaultValue; 
/*  452 */       if (k == curr) {
/*  453 */         return this.value[pos];
/*      */       }
/*      */     } 
/*      */   }
/*      */   
/*      */   public double putIfAbsent(int k, double v) {
/*  459 */     int pos = find(k);
/*  460 */     if (pos >= 0)
/*  461 */       return this.value[pos]; 
/*  462 */     insert(-pos - 1, k, v);
/*  463 */     return this.defRetValue;
/*      */   }
/*      */ 
/*      */ 
/*      */   
/*      */   public boolean remove(int k, double v) {
/*  469 */     if (k == 0) {
/*  470 */       if (this.containsNullKey && Double.doubleToLongBits(v) == Double.doubleToLongBits(this.value[this.n])) {
/*  471 */         removeNullEntry();
/*  472 */         return true;
/*      */       } 
/*  474 */       return false;
/*      */     } 
/*      */     
/*  477 */     int[] key = this.key;
/*      */     
/*      */     int curr, pos;
/*  480 */     if ((curr = key[pos = HashCommon.mix(k) & this.mask]) == 0)
/*  481 */       return false; 
/*  482 */     if (k == curr && Double.doubleToLongBits(v) == Double.doubleToLongBits(this.value[pos])) {
/*  483 */       removeEntry(pos);
/*  484 */       return true;
/*      */     } 
/*      */     while (true) {
/*  487 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0)
/*  488 */         return false; 
/*  489 */       if (k == curr && Double.doubleToLongBits(v) == Double.doubleToLongBits(this.value[pos])) {
/*  490 */         removeEntry(pos);
/*  491 */         return true;
/*      */       } 
/*      */     } 
/*      */   }
/*      */ 
/*      */   
/*      */   public boolean replace(int k, double oldValue, double v) {
/*  498 */     int pos = find(k);
/*  499 */     if (pos < 0 || Double.doubleToLongBits(oldValue) != Double.doubleToLongBits(this.value[pos]))
/*  500 */       return false; 
/*  501 */     this.value[pos] = v;
/*  502 */     return true;
/*      */   }
/*      */ 
/*      */   
/*      */   public double replace(int k, double v) {
/*  507 */     int pos = find(k);
/*  508 */     if (pos < 0)
/*  509 */       return this.defRetValue; 
/*  510 */     double oldValue = this.value[pos];
/*  511 */     this.value[pos] = v;
/*  512 */     return oldValue;
/*      */   }
/*      */ 
/*      */   
/*      */   public double computeIfAbsent(int k, IntToDoubleFunction mappingFunction) {
/*  517 */     Objects.requireNonNull(mappingFunction);
/*  518 */     int pos = find(k);
/*  519 */     if (pos >= 0)
/*  520 */       return this.value[pos]; 
/*  521 */     double newValue = mappingFunction.applyAsDouble(k);
/*  522 */     insert(-pos - 1, k, newValue);
/*  523 */     return newValue;
/*      */   }
/*      */ 
/*      */ 
/*      */   
/*      */   public double computeIfAbsentNullable(int k, IntFunction<? extends Double> mappingFunction) {
/*  529 */     Objects.requireNonNull(mappingFunction);
/*  530 */     int pos = find(k);
/*  531 */     if (pos >= 0)
/*  532 */       return this.value[pos]; 
/*  533 */     Double newValue = mappingFunction.apply(k);
/*  534 */     if (newValue == null)
/*  535 */       return this.defRetValue; 
/*  536 */     double v = newValue.doubleValue();
/*  537 */     insert(-pos - 1, k, v);
/*  538 */     return v;
/*      */   }
/*      */ 
/*      */ 
/*      */   
/*      */   public double computeIfPresent(int k, BiFunction<? super Integer, ? super Double, ? extends Double> remappingFunction) {
/*  544 */     Objects.requireNonNull(remappingFunction);
/*  545 */     int pos = find(k);
/*  546 */     if (pos < 0)
/*  547 */       return this.defRetValue; 
/*  548 */     Double newValue = remappingFunction.apply(Integer.valueOf(k), Double.valueOf(this.value[pos]));
/*  549 */     if (newValue == null) {
/*  550 */       if (k == 0) {
/*  551 */         removeNullEntry();
/*      */       } else {
/*  553 */         removeEntry(pos);
/*  554 */       }  return this.defRetValue;
/*      */     } 
/*  556 */     this.value[pos] = newValue.doubleValue(); return newValue.doubleValue();
/*      */   }
/*      */ 
/*      */ 
/*      */   
/*      */   public double compute(int k, BiFunction<? super Integer, ? super Double, ? extends Double> remappingFunction) {
/*  562 */     Objects.requireNonNull(remappingFunction);
/*  563 */     int pos = find(k);
/*  564 */     Double newValue = remappingFunction.apply(Integer.valueOf(k), 
/*  565 */         (pos >= 0) ? Double.valueOf(this.value[pos]) : null);
/*  566 */     if (newValue == null) {
/*  567 */       if (pos >= 0)
/*  568 */         if (k == 0) {
/*  569 */           removeNullEntry();
/*      */         } else {
/*  571 */           removeEntry(pos);
/*      */         }  
/*  573 */       return this.defRetValue;
/*      */     } 
/*  575 */     double newVal = newValue.doubleValue();
/*  576 */     if (pos < 0) {
/*  577 */       insert(-pos - 1, k, newVal);
/*  578 */       return newVal;
/*      */     } 
/*  580 */     this.value[pos] = newVal; return newVal;
/*      */   }
/*      */ 
/*      */ 
/*      */   
/*      */   public double merge(int k, double v, BiFunction<? super Double, ? super Double, ? extends Double> remappingFunction) {
/*  586 */     Objects.requireNonNull(remappingFunction);
/*  587 */     int pos = find(k);
/*  588 */     if (pos < 0) {
/*  589 */       insert(-pos - 1, k, v);
/*  590 */       return v;
/*      */     } 
/*  592 */     Double newValue = remappingFunction.apply(Double.valueOf(this.value[pos]), Double.valueOf(v));
/*  593 */     if (newValue == null) {
/*  594 */       if (k == 0) {
/*  595 */         removeNullEntry();
/*      */       } else {
/*  597 */         removeEntry(pos);
/*  598 */       }  return this.defRetValue;
/*      */     } 
/*  600 */     this.value[pos] = newValue.doubleValue(); return newValue.doubleValue();
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public void clear() {
/*  611 */     if (this.size == 0)
/*      */       return; 
/*  613 */     this.size = 0;
/*  614 */     this.containsNullKey = false;
/*  615 */     Arrays.fill(this.key, 0);
/*      */   }
/*      */   
/*      */   public int size() {
/*  619 */     return this.size;
/*      */   }
/*      */   
/*      */   public boolean isEmpty() {
/*  623 */     return (this.size == 0);
/*      */   }
/*      */ 
/*      */ 
/*      */   
/*      */   final class MapEntry
/*      */     implements Int2DoubleMap.Entry, Map.Entry<Integer, Double>
/*      */   {
/*      */     int index;
/*      */ 
/*      */     
/*      */     MapEntry(int index) {
/*  635 */       this.index = index;
/*      */     }
/*      */     
/*      */     MapEntry() {}
/*      */     
/*      */     public int getIntKey() {
/*  641 */       return Int2DoubleOpenHashMap.this.key[this.index];
/*      */     }
/*      */     
/*      */     public double getDoubleValue() {
/*  645 */       return Int2DoubleOpenHashMap.this.value[this.index];
/*      */     }
/*      */     
/*      */     public double setValue(double v) {
/*  649 */       double oldValue = Int2DoubleOpenHashMap.this.value[this.index];
/*  650 */       Int2DoubleOpenHashMap.this.value[this.index] = v;
/*  651 */       return oldValue;
/*      */     }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */     
/*      */     @Deprecated
/*      */     public Integer getKey() {
/*  661 */       return Integer.valueOf(Int2DoubleOpenHashMap.this.key[this.index]);
/*      */     }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */     
/*      */     @Deprecated
/*      */     public Double getValue() {
/*  671 */       return Double.valueOf(Int2DoubleOpenHashMap.this.value[this.index]);
/*      */     }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */     
/*      */     @Deprecated
/*      */     public Double setValue(Double v) {
/*  681 */       return Double.valueOf(setValue(v.doubleValue()));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean equals(Object o) {
/*  686 */       if (!(o instanceof Map.Entry))
/*  687 */         return false; 
/*  688 */       Map.Entry<Integer, Double> e = (Map.Entry<Integer, Double>)o;
/*  689 */       return (Int2DoubleOpenHashMap.this.key[this.index] == ((Integer)e.getKey()).intValue() && 
/*  690 */         Double.doubleToLongBits(Int2DoubleOpenHashMap.this.value[this.index]) == Double.doubleToLongBits(((Double)e.getValue()).doubleValue()));
/*      */     }
/*      */     
/*      */     public int hashCode() {
/*  694 */       return Int2DoubleOpenHashMap.this.key[this.index] ^ HashCommon.double2int(Int2DoubleOpenHashMap.this.value[this.index]);
/*      */     }
/*      */     
/*      */     public String toString() {
/*  698 */       return Int2DoubleOpenHashMap.this.key[this.index] + "=>" + Int2DoubleOpenHashMap.this.value[this.index];
/*      */     }
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   private class MapIterator
/*      */   {
/*  708 */     int pos = Int2DoubleOpenHashMap.this.n;
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */     
/*  715 */     int last = -1;
/*      */     
/*  717 */     int c = Int2DoubleOpenHashMap.this.size;
/*      */ 
/*      */ 
/*      */     
/*  721 */     boolean mustReturnNullKey = Int2DoubleOpenHashMap.this.containsNullKey;
/*      */ 
/*      */     
/*      */     IntArrayList wrapped;
/*      */ 
/*      */     
/*      */     public boolean hasNext() {
/*  728 */       return (this.c != 0);
/*      */     }
/*      */     public int nextEntry() {
/*  731 */       if (!hasNext())
/*  732 */         throw new NoSuchElementException(); 
/*  733 */       this.c--;
/*  734 */       if (this.mustReturnNullKey) {
/*  735 */         this.mustReturnNullKey = false;
/*  736 */         return this.last = Int2DoubleOpenHashMap.this.n;
/*      */       } 
/*  738 */       int[] key = Int2DoubleOpenHashMap.this.key;
/*      */       while (true) {
/*  740 */         if (--this.pos < 0) {
/*      */           
/*  742 */           this.last = Integer.MIN_VALUE;
/*  743 */           int k = this.wrapped.getInt(-this.pos - 1);
/*  744 */           int p = HashCommon.mix(k) & Int2DoubleOpenHashMap.this.mask;
/*  745 */           while (k != key[p])
/*  746 */             p = p + 1 & Int2DoubleOpenHashMap.this.mask; 
/*  747 */           return p;
/*      */         } 
/*  749 */         if (key[this.pos] != 0) {
/*  750 */           return this.last = this.pos;
/*      */         }
/*      */       } 
/*      */     }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */     
/*      */     private void shiftKeys(int pos) {
/*  764 */       int[] key = Int2DoubleOpenHashMap.this.key; while (true) {
/*      */         int curr, last;
/*  766 */         pos = (last = pos) + 1 & Int2DoubleOpenHashMap.this.mask;
/*      */         while (true) {
/*  768 */           if ((curr = key[pos]) == 0) {
/*  769 */             key[last] = 0;
/*      */             return;
/*      */           } 
/*  772 */           int slot = HashCommon.mix(curr) & Int2DoubleOpenHashMap.this.mask;
/*  773 */           if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/*      */             break; 
/*  775 */           pos = pos + 1 & Int2DoubleOpenHashMap.this.mask;
/*      */         } 
/*  777 */         if (pos < last) {
/*  778 */           if (this.wrapped == null)
/*  779 */             this.wrapped = new IntArrayList(2); 
/*  780 */           this.wrapped.add(key[pos]);
/*      */         } 
/*  782 */         key[last] = curr;
/*  783 */         Int2DoubleOpenHashMap.this.value[last] = Int2DoubleOpenHashMap.this.value[pos];
/*      */       } 
/*      */     }
/*      */     public void remove() {
/*  787 */       if (this.last == -1)
/*  788 */         throw new IllegalStateException(); 
/*  789 */       if (this.last == Int2DoubleOpenHashMap.this.n) {
/*  790 */         Int2DoubleOpenHashMap.this.containsNullKey = false;
/*  791 */       } else if (this.pos >= 0) {
/*  792 */         shiftKeys(this.last);
/*      */       } else {
/*      */         
/*  795 */         Int2DoubleOpenHashMap.this.remove(this.wrapped.getInt(-this.pos - 1));
/*  796 */         this.last = -1;
/*      */         return;
/*      */       } 
/*  799 */       Int2DoubleOpenHashMap.this.size--;
/*  800 */       this.last = -1;
/*      */     }
/*      */ 
/*      */     
/*      */     public int skip(int n) {
/*  805 */       int i = n;
/*  806 */       while (i-- != 0 && hasNext())
/*  807 */         nextEntry(); 
/*  808 */       return n - i - 1;
/*      */     }
/*      */     private MapIterator() {} }
/*      */   
/*      */   private class EntryIterator extends MapIterator implements ObjectIterator<Int2DoubleMap.Entry> { private Int2DoubleOpenHashMap.MapEntry entry;
/*      */     
/*      */     public Int2DoubleOpenHashMap.MapEntry next() {
/*  815 */       return this.entry = new Int2DoubleOpenHashMap.MapEntry(nextEntry());
/*      */     }
/*      */     private EntryIterator() {}
/*      */     public void remove() {
/*  819 */       super.remove();
/*  820 */       this.entry.index = -1;
/*      */     } }
/*      */   
/*      */   private class FastEntryIterator extends MapIterator implements ObjectIterator<Int2DoubleMap.Entry> { private FastEntryIterator() {
/*  824 */       this.entry = new Int2DoubleOpenHashMap.MapEntry();
/*      */     } private final Int2DoubleOpenHashMap.MapEntry entry;
/*      */     public Int2DoubleOpenHashMap.MapEntry next() {
/*  827 */       this.entry.index = nextEntry();
/*  828 */       return this.entry;
/*      */     } }
/*      */   
/*      */   private final class MapEntrySet extends AbstractObjectSet<Int2DoubleMap.Entry> implements Int2DoubleMap.FastEntrySet { private MapEntrySet() {}
/*      */     
/*      */     public ObjectIterator<Int2DoubleMap.Entry> iterator() {
/*  834 */       return new Int2DoubleOpenHashMap.EntryIterator();
/*      */     }
/*      */     
/*      */     public ObjectIterator<Int2DoubleMap.Entry> fastIterator() {
/*  838 */       return new Int2DoubleOpenHashMap.FastEntryIterator();
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean contains(Object o) {
/*  843 */       if (!(o instanceof Map.Entry))
/*  844 */         return false; 
/*  845 */       Map.Entry<?, ?> e = (Map.Entry<?, ?>)o;
/*  846 */       if (e.getKey() == null || !(e.getKey() instanceof Integer))
/*  847 */         return false; 
/*  848 */       if (e.getValue() == null || !(e.getValue() instanceof Double))
/*  849 */         return false; 
/*  850 */       int k = ((Integer)e.getKey()).intValue();
/*  851 */       double v = ((Double)e.getValue()).doubleValue();
/*  852 */       if (k == 0) {
/*  853 */         return (Int2DoubleOpenHashMap.this.containsNullKey && 
/*  854 */           Double.doubleToLongBits(Int2DoubleOpenHashMap.this.value[Int2DoubleOpenHashMap.this.n]) == Double.doubleToLongBits(v));
/*      */       }
/*  856 */       int[] key = Int2DoubleOpenHashMap.this.key;
/*      */       
/*      */       int curr, pos;
/*  859 */       if ((curr = key[pos = HashCommon.mix(k) & Int2DoubleOpenHashMap.this.mask]) == 0)
/*  860 */         return false; 
/*  861 */       if (k == curr) {
/*  862 */         return (Double.doubleToLongBits(Int2DoubleOpenHashMap.this.value[pos]) == Double.doubleToLongBits(v));
/*      */       }
/*      */       while (true) {
/*  865 */         if ((curr = key[pos = pos + 1 & Int2DoubleOpenHashMap.this.mask]) == 0)
/*  866 */           return false; 
/*  867 */         if (k == curr) {
/*  868 */           return (Double.doubleToLongBits(Int2DoubleOpenHashMap.this.value[pos]) == Double.doubleToLongBits(v));
/*      */         }
/*      */       } 
/*      */     }
/*      */     
/*      */     public boolean remove(Object o) {
/*  874 */       if (!(o instanceof Map.Entry))
/*  875 */         return false; 
/*  876 */       Map.Entry<?, ?> e = (Map.Entry<?, ?>)o;
/*  877 */       if (e.getKey() == null || !(e.getKey() instanceof Integer))
/*  878 */         return false; 
/*  879 */       if (e.getValue() == null || !(e.getValue() instanceof Double))
/*  880 */         return false; 
/*  881 */       int k = ((Integer)e.getKey()).intValue();
/*  882 */       double v = ((Double)e.getValue()).doubleValue();
/*  883 */       if (k == 0) {
/*  884 */         if (Int2DoubleOpenHashMap.this.containsNullKey && Double.doubleToLongBits(Int2DoubleOpenHashMap.this.value[Int2DoubleOpenHashMap.this.n]) == Double.doubleToLongBits(v)) {
/*  885 */           Int2DoubleOpenHashMap.this.removeNullEntry();
/*  886 */           return true;
/*      */         } 
/*  888 */         return false;
/*      */       } 
/*      */       
/*  891 */       int[] key = Int2DoubleOpenHashMap.this.key;
/*      */       
/*      */       int curr, pos;
/*  894 */       if ((curr = key[pos = HashCommon.mix(k) & Int2DoubleOpenHashMap.this.mask]) == 0)
/*  895 */         return false; 
/*  896 */       if (curr == k) {
/*  897 */         if (Double.doubleToLongBits(Int2DoubleOpenHashMap.this.value[pos]) == Double.doubleToLongBits(v)) {
/*  898 */           Int2DoubleOpenHashMap.this.removeEntry(pos);
/*  899 */           return true;
/*      */         } 
/*  901 */         return false;
/*      */       } 
/*      */       while (true) {
/*  904 */         if ((curr = key[pos = pos + 1 & Int2DoubleOpenHashMap.this.mask]) == 0)
/*  905 */           return false; 
/*  906 */         if (curr == k && 
/*  907 */           Double.doubleToLongBits(Int2DoubleOpenHashMap.this.value[pos]) == Double.doubleToLongBits(v)) {
/*  908 */           Int2DoubleOpenHashMap.this.removeEntry(pos);
/*  909 */           return true;
/*      */         } 
/*      */       } 
/*      */     }
/*      */ 
/*      */     
/*      */     public int size() {
/*  916 */       return Int2DoubleOpenHashMap.this.size;
/*      */     }
/*      */     
/*      */     public void clear() {
/*  920 */       Int2DoubleOpenHashMap.this.clear();
/*      */     }
/*      */ 
/*      */     
/*      */     public void forEach(Consumer<? super Int2DoubleMap.Entry> consumer) {
/*  925 */       if (Int2DoubleOpenHashMap.this.containsNullKey)
/*  926 */         consumer.accept(new AbstractInt2DoubleMap.BasicEntry(Int2DoubleOpenHashMap.this.key[Int2DoubleOpenHashMap.this.n], Int2DoubleOpenHashMap.this.value[Int2DoubleOpenHashMap.this.n])); 
/*  927 */       for (int pos = Int2DoubleOpenHashMap.this.n; pos-- != 0;) {
/*  928 */         if (Int2DoubleOpenHashMap.this.key[pos] != 0)
/*  929 */           consumer.accept(new AbstractInt2DoubleMap.BasicEntry(Int2DoubleOpenHashMap.this.key[pos], Int2DoubleOpenHashMap.this.value[pos])); 
/*      */       } 
/*      */     }
/*      */     
/*      */     public void fastForEach(Consumer<? super Int2DoubleMap.Entry> consumer) {
/*  934 */       AbstractInt2DoubleMap.BasicEntry entry = new AbstractInt2DoubleMap.BasicEntry();
/*  935 */       if (Int2DoubleOpenHashMap.this.containsNullKey) {
/*  936 */         entry.key = Int2DoubleOpenHashMap.this.key[Int2DoubleOpenHashMap.this.n];
/*  937 */         entry.value = Int2DoubleOpenHashMap.this.value[Int2DoubleOpenHashMap.this.n];
/*  938 */         consumer.accept(entry);
/*      */       } 
/*  940 */       for (int pos = Int2DoubleOpenHashMap.this.n; pos-- != 0;) {
/*  941 */         if (Int2DoubleOpenHashMap.this.key[pos] != 0) {
/*  942 */           entry.key = Int2DoubleOpenHashMap.this.key[pos];
/*  943 */           entry.value = Int2DoubleOpenHashMap.this.value[pos];
/*  944 */           consumer.accept(entry);
/*      */         } 
/*      */       } 
/*      */     } }
/*      */   
/*      */   public Int2DoubleMap.FastEntrySet int2DoubleEntrySet() {
/*  950 */     if (this.entries == null)
/*  951 */       this.entries = new MapEntrySet(); 
/*  952 */     return this.entries;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   private final class KeyIterator
/*      */     extends MapIterator
/*      */     implements IntIterator
/*      */   {
/*      */     public int nextInt() {
/*  969 */       return Int2DoubleOpenHashMap.this.key[nextEntry()];
/*      */     } }
/*      */   
/*      */   private final class KeySet extends AbstractIntSet { private KeySet() {}
/*      */     
/*      */     public IntIterator iterator() {
/*  975 */       return new Int2DoubleOpenHashMap.KeyIterator();
/*      */     }
/*      */ 
/*      */     
/*      */     public void forEach(IntConsumer consumer) {
/*  980 */       if (Int2DoubleOpenHashMap.this.containsNullKey)
/*  981 */         consumer.accept(Int2DoubleOpenHashMap.this.key[Int2DoubleOpenHashMap.this.n]); 
/*  982 */       for (int pos = Int2DoubleOpenHashMap.this.n; pos-- != 0; ) {
/*  983 */         int k = Int2DoubleOpenHashMap.this.key[pos];
/*  984 */         if (k != 0)
/*  985 */           consumer.accept(k); 
/*      */       } 
/*      */     }
/*      */     
/*      */     public int size() {
/*  990 */       return Int2DoubleOpenHashMap.this.size;
/*      */     }
/*      */     
/*      */     public boolean contains(int k) {
/*  994 */       return Int2DoubleOpenHashMap.this.containsKey(k);
/*      */     }
/*      */     
/*      */     public boolean remove(int k) {
/*  998 */       int oldSize = Int2DoubleOpenHashMap.this.size;
/*  999 */       Int2DoubleOpenHashMap.this.remove(k);
/* 1000 */       return (Int2DoubleOpenHashMap.this.size != oldSize);
/*      */     }
/*      */     
/*      */     public void clear() {
/* 1004 */       Int2DoubleOpenHashMap.this.clear();
/*      */     } }
/*      */ 
/*      */   
/*      */   public IntSet keySet() {
/* 1009 */     if (this.keys == null)
/* 1010 */       this.keys = new KeySet(); 
/* 1011 */     return this.keys;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   private final class ValueIterator
/*      */     extends MapIterator
/*      */     implements DoubleIterator
/*      */   {
/*      */     public double nextDouble() {
/* 1028 */       return Int2DoubleOpenHashMap.this.value[nextEntry()];
/*      */     }
/*      */   }
/*      */   
/*      */   public DoubleCollection values() {
/* 1033 */     if (this.values == null)
/* 1034 */       this.values = (DoubleCollection)new AbstractDoubleCollection()
/*      */         {
/*      */           public DoubleIterator iterator() {
/* 1037 */             return new Int2DoubleOpenHashMap.ValueIterator();
/*      */           }
/*      */           
/*      */           public int size() {
/* 1041 */             return Int2DoubleOpenHashMap.this.size;
/*      */           }
/*      */           
/*      */           public boolean contains(double v) {
/* 1045 */             return Int2DoubleOpenHashMap.this.containsValue(v);
/*      */           }
/*      */           
/*      */           public void clear() {
/* 1049 */             Int2DoubleOpenHashMap.this.clear();
/*      */           }
/*      */           
/*      */           public void forEach(DoubleConsumer consumer)
/*      */           {
/* 1054 */             if (Int2DoubleOpenHashMap.this.containsNullKey)
/* 1055 */               consumer.accept(Int2DoubleOpenHashMap.this.value[Int2DoubleOpenHashMap.this.n]); 
/* 1056 */             for (int pos = Int2DoubleOpenHashMap.this.n; pos-- != 0;) {
/* 1057 */               if (Int2DoubleOpenHashMap.this.key[pos] != 0)
/* 1058 */                 consumer.accept(Int2DoubleOpenHashMap.this.value[pos]); 
/*      */             }  }
/*      */         }; 
/* 1061 */     return this.values;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public boolean trim() {
/* 1078 */     int l = HashCommon.arraySize(this.size, this.f);
/* 1079 */     if (l >= this.n || this.size > HashCommon.maxFill(l, this.f))
/* 1080 */       return true; 
/*      */     try {
/* 1082 */       rehash(l);
/* 1083 */     } catch (OutOfMemoryError cantDoIt) {
/* 1084 */       return false;
/*      */     } 
/* 1086 */     return true;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public boolean trim(int n) {
/* 1110 */     int l = HashCommon.nextPowerOfTwo((int)Math.ceil((n / this.f)));
/* 1111 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1112 */       return true; 
/*      */     try {
/* 1114 */       rehash(l);
/* 1115 */     } catch (OutOfMemoryError cantDoIt) {
/* 1116 */       return false;
/*      */     } 
/* 1118 */     return true;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected void rehash(int newN) {
/* 1134 */     int[] key = this.key;
/* 1135 */     double[] value = this.value;
/* 1136 */     int mask = newN - 1;
/* 1137 */     int[] newKey = new int[newN + 1];
/* 1138 */     double[] newValue = new double[newN + 1];
/* 1139 */     int i = this.n;
/* 1140 */     for (int j = realSize(); j-- != 0; ) {
/* 1141 */       while (key[--i] == 0); int pos;
/* 1142 */       if (newKey[pos = HashCommon.mix(key[i]) & mask] != 0)
/* 1143 */         while (newKey[pos = pos + 1 & mask] != 0); 
/* 1144 */       newKey[pos] = key[i];
/* 1145 */       newValue[pos] = value[i];
/*      */     } 
/* 1147 */     newValue[newN] = value[this.n];
/* 1148 */     this.n = newN;
/* 1149 */     this.mask = mask;
/* 1150 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/* 1151 */     this.key = newKey;
/* 1152 */     this.value = newValue;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public Int2DoubleOpenHashMap clone() {
/*      */     Int2DoubleOpenHashMap c;
/*      */     try {
/* 1169 */       c = (Int2DoubleOpenHashMap)super.clone();
/* 1170 */     } catch (CloneNotSupportedException cantHappen) {
/* 1171 */       throw new InternalError();
/*      */     } 
/* 1173 */     c.keys = null;
/* 1174 */     c.values = null;
/* 1175 */     c.entries = null;
/* 1176 */     c.containsNullKey = this.containsNullKey;
/* 1177 */     c.key = (int[])this.key.clone();
/* 1178 */     c.value = (double[])this.value.clone();
/* 1179 */     return c;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public int hashCode() {
/* 1192 */     int h = 0;
/* 1193 */     for (int j = realSize(), i = 0, t = 0; j-- != 0; ) {
/* 1194 */       while (this.key[i] == 0)
/* 1195 */         i++; 
/* 1196 */       t = this.key[i];
/* 1197 */       t ^= HashCommon.double2int(this.value[i]);
/* 1198 */       h += t;
/* 1199 */       i++;
/*      */     } 
/*      */     
/* 1202 */     if (this.containsNullKey)
/* 1203 */       h += HashCommon.double2int(this.value[this.n]); 
/* 1204 */     return h;
/*      */   }
/*      */   private void writeObject(ObjectOutputStream s) throws IOException {
/* 1207 */     int[] key = this.key;
/* 1208 */     double[] value = this.value;
/* 1209 */     MapIterator i = new MapIterator();
/* 1210 */     s.defaultWriteObject();
/* 1211 */     for (int j = this.size; j-- != 0; ) {
/* 1212 */       int e = i.nextEntry();
/* 1213 */       s.writeInt(key[e]);
/* 1214 */       s.writeDouble(value[e]);
/*      */     } 
/*      */   }
/*      */   
/*      */   private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
/* 1219 */     s.defaultReadObject();
/* 1220 */     this.n = HashCommon.arraySize(this.size, this.f);
/* 1221 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/* 1222 */     this.mask = this.n - 1;
/* 1223 */     int[] key = this.key = new int[this.n + 1];
/* 1224 */     double[] value = this.value = new double[this.n + 1];
/*      */ 
/*      */     
/* 1227 */     for (int i = this.size; i-- != 0; ) {
/* 1228 */       int pos, k = s.readInt();
/* 1229 */       double v = s.readDouble();
/* 1230 */       if (k == 0) {
/* 1231 */         pos = this.n;
/* 1232 */         this.containsNullKey = true;
/*      */       } else {
/* 1234 */         pos = HashCommon.mix(k) & this.mask;
/* 1235 */         while (key[pos] != 0)
/* 1236 */           pos = pos + 1 & this.mask; 
/*      */       } 
/* 1238 */       key[pos] = k;
/* 1239 */       value[pos] = v;
/*      */     } 
/*      */   }
/*      */   
/*      */   private void checkTable() {}
/*      */ }


/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\ints\Int2DoubleOpenHashMap.class
 * Java compiler version: 8 (52.0)
 * JD-Core Version:       1.1.3
 */