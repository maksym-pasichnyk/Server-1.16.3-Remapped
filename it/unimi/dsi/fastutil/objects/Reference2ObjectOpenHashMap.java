...
/*      */   
/*      */   public boolean trim() {
/*  837 */     int l = HashCommon.arraySize(this.size, this.f);
/*  838 */     if (l >= this.n || this.size > HashCommon.maxFill(l, this.f))
/*  839 */       return true; 
/*      */     try {
/*  841 */       rehash(l);
/*  842 */     } catch (OutOfMemoryError cantDoIt) {
/*  843 */       return false;
/*      */     } 
/*  845 */     return true;
/*      */   }
...
/*  869 */     int l = HashCommon.nextPowerOfTwo((int)Math.ceil((n / this.f)));
/*  870 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/*  871 */       return true; 
/*      */     try {
/*  873 */       rehash(l);
...
/*  893 */     K[] key = this.key;
/*  894 */     V[] value = this.value;
/*  895 */     int mask = newN - 1;
/*  896 */     K[] newKey = (K[])new Object[newN + 1];
/*  897 */     V[] newValue = (V[])new Object[newN + 1];
/*  898 */     int i = this.n;
/*  899 */     for (int j = realSize(); j-- != 0; ) {
/*  900 */       while (key[--i] == null); int pos;
/*  901 */       if (newKey[pos = HashCommon.mix(System.identityHashCode(key[i])) & mask] != null)
/*      */       {
/*  903 */         while (newKey[pos = pos + 1 & mask] != null); } 
/*  904 */       newKey[pos] = key[i];
/*  905 */       newValue[pos] = value[i];
/*      */     } 
/*  907 */     newValue[newN] = value[this.n];
/*  908 */     this.n = newN;
/*  909 */     this.mask = mask;
/*  910 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/*  911 */     this.key = newKey;
/*  912 */     this.value = newValue;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     try {
/*  929 */       c = (Reference2ObjectOpenHashMap<K, V>)super.clone();
/*  930 */     } catch (CloneNotSupportedException cantHappen) {
/*  931 */       throw new InternalError();
/*      */     } 
/*  933 */     c.keys = null;
/*  934 */     c.values = null;
/*  935 */     c.entries = null;
/*  936 */     c.containsNullKey = this.containsNullKey;
/*  937 */     c.key = (K[])this.key.clone();
/*  938 */     c.value = (V[])this.value.clone();
/*  939 */     return c;
/*      */   }
/*      */ 
/*      */ 
...
/*      */   public int hashCode() {
/*  952 */     int h = 0;
/*  953 */     for (int j = realSize(), i = 0, t = 0; j-- != 0; ) {
/*  954 */       while (this.key[i] == null)
/*  955 */         i++; 
/*  956 */       if (this != this.key[i])
/*  957 */         t = System.identityHashCode(this.key[i]); 
/*  958 */       if (this != this.value[i])
/*  959 */         t ^= (this.value[i] == null) ? 0 : this.value[i].hashCode(); 
/*  960 */       h += t;
/*  961 */       i++;
/*      */     } 
/*      */     
/*  964 */     if (this.containsNullKey)
/*  965 */       h += (this.value[this.n] == null) ? 0 : this.value[this.n].hashCode(); 
/*  966 */     return h;
/*      */   }
/*      */   private void writeObject(ObjectOutputStream s) throws IOException {
/*  969 */     K[] key = this.key;
/*  970 */     V[] value = this.value;
/*  971 */     MapIterator i = new MapIterator();
/*  972 */     s.defaultWriteObject();
/*  973 */     for (int j = this.size; j-- != 0; ) {
/*  974 */       int e = i.nextEntry();
/*  975 */       s.writeObject(key[e]);
/*  976 */       s.writeObject(value[e]);
/*      */     } 
/*      */   }
/*      */   
/*      */   private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
/*  981 */     s.defaultReadObject();
/*  982 */     this.n = HashCommon.arraySize(this.size, this.f);
/*  983 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/*  984 */     this.mask = this.n - 1;
/*  985 */     K[] key = this.key = (K[])new Object[this.n + 1];
/*  986 */     V[] value = this.value = (V[])new Object[this.n + 1];
/*      */ 
/*      */     
/*  989 */     for (int i = this.size; i-- != 0; ) {
/*  990 */       int pos; K k = (K)s.readObject();
/*  991 */       V v = (V)s.readObject();
/*  992 */       if (k == null) {
/*  993 */         pos = this.n;
/*  994 */         this.containsNullKey = true;
/*      */       } else {
/*  996 */         pos = HashCommon.mix(System.identityHashCode(k)) & this.mask;
/*  997 */         while (key[pos] != null)
/*  998 */           pos = pos + 1 & this.mask; 
/*      */       } 
/* 1000 */       key[pos] = k;
/* 1001 */       value[pos] = v;
/*      */     } 
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\objects\Reference2ObjectOpenHashMap.class
