...
/*      */   
/*      */   public boolean trim() {
/* 1743 */     int l = HashCommon.arraySize(this.size, this.f);
/* 1744 */     if (l >= this.n || this.size > HashCommon.maxFill(l, this.f))
/* 1745 */       return true; 
/*      */     try {
/* 1747 */       rehash(l);
...
/* 1775 */     int l = HashCommon.nextPowerOfTwo((int)Math.ceil((n / this.f)));
/* 1776 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1777 */       return true; 
/*      */     try {
/* 1779 */       rehash(l);
...
/* 1799 */     K[] key = this.key;
/* 1800 */     long[] value = this.value;
/* 1801 */     int mask = newN - 1;
/* 1802 */     K[] newKey = (K[])new Object[newN + 1];
/* 1803 */     long[] newValue = new long[newN + 1];
...
/* 1809 */       int pos; if (key[i] == null) {
/* 1810 */         pos = newN;
/*      */       } else {
/* 1812 */         pos = HashCommon.mix(key[i].hashCode()) & mask;
/* 1813 */         while (newKey[pos] != null)
/* 1814 */           pos = pos + 1 & mask; 
/*      */       } 
/* 1816 */       newKey[pos] = key[i];
/* 1817 */       newValue[pos] = value[i];
/* 1818 */       if (prev != -1) {
/* 1819 */         newLink[newPrev] = newLink[newPrev] ^ (newLink[newPrev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1820 */         newLink[pos] = newLink[pos] ^ (newLink[pos] ^ (newPrev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1821 */         newPrev = pos;
/*      */       } else {
/* 1823 */         newPrev = this.first = pos;
/*      */         
/* 1825 */         newLink[pos] = -1L;
/*      */       } 
/* 1827 */       int t = i;
...
/* 1829 */       prev = t;
/*      */     } 
/* 1831 */     this.link = newLink;
/* 1832 */     this.last = newPrev;
/* 1833 */     if (newPrev != -1)
/*      */     {
/* 1835 */       newLink[newPrev] = newLink[newPrev] | 0xFFFFFFFFL; } 
/* 1836 */     this.n = newN;
/* 1837 */     this.mask = mask;
/* 1838 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/* 1839 */     this.key = newKey;
/* 1840 */     this.value = newValue;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     try {
/* 1857 */       c = (Object2LongLinkedOpenHashMap<K>)super.clone();
/* 1858 */     } catch (CloneNotSupportedException cantHappen) {
/* 1859 */       throw new InternalError();
/*      */     } 
/* 1861 */     c.keys = null;
/* 1862 */     c.values = null;
/* 1863 */     c.entries = null;
/* 1864 */     c.containsNullKey = this.containsNullKey;
/* 1865 */     c.key = (K[])this.key.clone();
/* 1866 */     c.value = (long[])this.value.clone();
/* 1867 */     c.link = (long[])this.link.clone();
/* 1868 */     return c;
/*      */   }
/*      */ 
/*      */ 
...
/*      */   public int hashCode() {
/* 1881 */     int h = 0;
/* 1882 */     for (int j = realSize(), i = 0, t = 0; j-- != 0; ) {
/* 1883 */       while (this.key[i] == null)
/* 1884 */         i++; 
/* 1885 */       if (this != this.key[i])
/* 1886 */         t = this.key[i].hashCode(); 
/* 1887 */       t ^= HashCommon.long2int(this.value[i]);
/* 1888 */       h += t;
/* 1889 */       i++;
/*      */     } 
/*      */     
/* 1892 */     if (this.containsNullKey)
/* 1893 */       h += HashCommon.long2int(this.value[this.n]); 
/* 1894 */     return h;
/*      */   }
/*      */   private void writeObject(ObjectOutputStream s) throws IOException {
/* 1897 */     K[] key = this.key;
/* 1898 */     long[] value = this.value;
/* 1899 */     MapIterator i = new MapIterator();
/* 1900 */     s.defaultWriteObject();
/* 1901 */     for (int j = this.size; j-- != 0; ) {
/* 1902 */       int e = i.nextEntry();
/* 1903 */       s.writeObject(key[e]);
/* 1904 */       s.writeLong(value[e]);
/*      */     } 
/*      */   }
/*      */   
/*      */   private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
/* 1909 */     s.defaultReadObject();
/* 1910 */     this.n = HashCommon.arraySize(this.size, this.f);
/* 1911 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/* 1912 */     this.mask = this.n - 1;
/* 1913 */     K[] key = this.key = (K[])new Object[this.n + 1];
/* 1914 */     long[] value = this.value = new long[this.n + 1];
/* 1915 */     long[] link = this.link = new long[this.n + 1];
/* 1916 */     int prev = -1;
/* 1917 */     this.first = this.last = -1;
/*      */ 
/*      */     
/* 1920 */     for (int i = this.size; i-- != 0; ) {
/* 1921 */       int pos; K k = (K)s.readObject();
/* 1922 */       long v = s.readLong();
/* 1923 */       if (k == null) {
/* 1924 */         pos = this.n;
/* 1925 */         this.containsNullKey = true;
/*      */       } else {
/* 1927 */         pos = HashCommon.mix(k.hashCode()) & this.mask;
/* 1928 */         while (key[pos] != null)
/* 1929 */           pos = pos + 1 & this.mask; 
/*      */       } 
/* 1931 */       key[pos] = k;
/* 1932 */       value[pos] = v;
/* 1933 */       if (this.first != -1) {
/* 1934 */         link[prev] = link[prev] ^ (link[prev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1935 */         link[pos] = link[pos] ^ (link[pos] ^ (prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1936 */         prev = pos; continue;
/*      */       } 
/* 1938 */       prev = this.first = pos;
/*      */       
/* 1940 */       link[pos] = link[pos] | 0xFFFFFFFF00000000L;
/*      */     } 
/*      */     
/* 1943 */     this.last = prev;
/* 1944 */     if (prev != -1)
/*      */     {
/* 1946 */       link[prev] = link[prev] | 0xFFFFFFFFL;
/*      */     }
/*      */   }
/*      */   
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\objects\Object2LongLinkedOpenHashMap.class
