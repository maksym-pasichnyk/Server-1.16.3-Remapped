...
/*      */   
/*      */   public boolean trim() {
/*  838 */     int l = HashCommon.arraySize(this.size, this.f);
/*  839 */     if (l >= this.n || this.size > HashCommon.maxFill(l, this.f))
/*  840 */       return true; 
/*      */     try {
/*  842 */       rehash(l);
/*  843 */     } catch (OutOfMemoryError cantDoIt) {
/*  844 */       return false;
/*      */     } 
/*  846 */     return true;
/*      */   }
...
/*  870 */     int l = HashCommon.nextPowerOfTwo((int)Math.ceil((n / this.f)));
/*  871 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/*  872 */       return true; 
/*      */     try {
/*  874 */       rehash(l);
...
/*  894 */     K[] key = this.key;
/*  895 */     V[] value = this.value;
/*  896 */     int mask = newN - 1;
/*  897 */     K[] newKey = (K[])new Object[newN + 1];
/*  898 */     V[] newValue = (V[])new Object[newN + 1];
/*  899 */     int i = this.n;
/*  900 */     for (int j = realSize(); j-- != 0; ) {
/*  901 */       while (key[--i] == null); int pos;
/*  902 */       if (newKey[pos = HashCommon.mix(System.identityHashCode(key[i])) & mask] != null)
/*      */       {
/*  904 */         while (newKey[pos = pos + 1 & mask] != null); } 
/*  905 */       newKey[pos] = key[i];
/*  906 */       newValue[pos] = value[i];
/*      */     } 
/*  908 */     newValue[newN] = value[this.n];
/*  909 */     this.n = newN;
/*  910 */     this.mask = mask;
/*  911 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/*  912 */     this.key = newKey;
/*  913 */     this.value = newValue;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     try {
/*  930 */       c = (Reference2ReferenceOpenHashMap<K, V>)super.clone();
/*  931 */     } catch (CloneNotSupportedException cantHappen) {
/*  932 */       throw new InternalError();
/*      */     } 
/*  934 */     c.keys = null;
/*  935 */     c.values = null;
/*  936 */     c.entries = null;
/*  937 */     c.containsNullKey = this.containsNullKey;
/*  938 */     c.key = (K[])this.key.clone();
/*  939 */     c.value = (V[])this.value.clone();
/*  940 */     return c;
/*      */   }
/*      */ 
/*      */ 
...
/*      */   public int hashCode() {
/*  953 */     int h = 0;
/*  954 */     for (int j = realSize(), i = 0, t = 0; j-- != 0; ) {
/*  955 */       while (this.key[i] == null)
/*  956 */         i++; 
/*  957 */       if (this != this.key[i])
/*  958 */         t = System.identityHashCode(this.key[i]); 
/*  959 */       if (this != this.value[i])
/*  960 */         t ^= (this.value[i] == null) ? 0 : System.identityHashCode(this.value[i]); 
/*  961 */       h += t;
/*  962 */       i++;
/*      */     } 
/*      */     
/*  965 */     if (this.containsNullKey)
/*  966 */       h += (this.value[this.n] == null) ? 0 : System.identityHashCode(this.value[this.n]); 
/*  967 */     return h;
/*      */   }
/*      */   private void writeObject(ObjectOutputStream s) throws IOException {
/*  970 */     K[] key = this.key;
/*  971 */     V[] value = this.value;
/*  972 */     MapIterator i = new MapIterator();
/*  973 */     s.defaultWriteObject();
/*  974 */     for (int j = this.size; j-- != 0; ) {
/*  975 */       int e = i.nextEntry();
/*  976 */       s.writeObject(key[e]);
/*  977 */       s.writeObject(value[e]);
/*      */     } 
/*      */   }
/*      */   
/*      */   private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
/*  982 */     s.defaultReadObject();
/*  983 */     this.n = HashCommon.arraySize(this.size, this.f);
/*  984 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/*  985 */     this.mask = this.n - 1;
/*  986 */     K[] key = this.key = (K[])new Object[this.n + 1];
/*  987 */     V[] value = this.value = (V[])new Object[this.n + 1];
/*      */ 
/*      */     
/*  990 */     for (int i = this.size; i-- != 0; ) {
/*  991 */       int pos; K k = (K)s.readObject();
/*  992 */       V v = (V)s.readObject();
/*  993 */       if (k == null) {
/*  994 */         pos = this.n;
/*  995 */         this.containsNullKey = true;
/*      */       } else {
/*  997 */         pos = HashCommon.mix(System.identityHashCode(k)) & this.mask;
/*  998 */         while (key[pos] != null)
/*  999 */           pos = pos + 1 & this.mask; 
/*      */       } 
/* 1001 */       key[pos] = k;
/* 1002 */       value[pos] = v;
/*      */     } 
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\objects\Reference2ReferenceOpenHashMap.class
