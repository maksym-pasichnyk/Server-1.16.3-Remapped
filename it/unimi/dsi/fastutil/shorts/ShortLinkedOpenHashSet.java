...
/*  143 */     this.key = new short[this.n + 1];
...
/*  239 */     this(ShortIterators.asShortIterator(i), f);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  249 */     this(ShortIterators.asShortIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  265 */     ShortArrays.ensureOffsetLength(a, offset, length);
...
/*  384 */     short[] key = this.key; while (true) {
/*      */       short curr; int last;
...
/*  658 */     Arrays.fill(this.key, (short)0);
...
/*  825 */     int index = -1;
/*      */     SetIterator() {
/*  827 */       this.next = ShortLinkedOpenHashSet.this.first;
/*  828 */       this.index = 0;
/*      */     }
/*      */     SetIterator(short from) {
/*  831 */       if (from == 0) {
/*  832 */         if (ShortLinkedOpenHashSet.this.containsNull) {
/*  833 */           this.next = (int)ShortLinkedOpenHashSet.this.link[ShortLinkedOpenHashSet.this.n];
/*  834 */           this.prev = ShortLinkedOpenHashSet.this.n;
/*      */           return;
/*      */         } 
/*  837 */         throw new NoSuchElementException("The key " + from + " does not belong to this set.");
/*      */       } 
/*  839 */       if (ShortLinkedOpenHashSet.this.key[ShortLinkedOpenHashSet.this.last] == from) {
/*  840 */         this.prev = ShortLinkedOpenHashSet.this.last;
/*  841 */         this.index = ShortLinkedOpenHashSet.this.size;
/*      */         
/*      */         return;
/*      */       } 
/*  845 */       short[] key = ShortLinkedOpenHashSet.this.key;
/*  846 */       int pos = HashCommon.mix(from) & ShortLinkedOpenHashSet.this.mask;
/*      */       
/*  848 */       while (key[pos] != 0) {
/*  849 */         if (key[pos] == from) {
/*      */           
/*  851 */           this.next = (int)ShortLinkedOpenHashSet.this.link[pos];
/*  852 */           this.prev = pos;
/*      */           return;
/*      */         } 
/*  855 */         pos = pos + 1 & ShortLinkedOpenHashSet.this.mask;
/*      */       } 
...
/*  872 */       this.next = (int)ShortLinkedOpenHashSet.this.link[this.curr];
/*  873 */       this.prev = this.curr;
/*  874 */       if (this.index >= 0) {
/*  875 */         this.index++;
/*      */       }
/*      */       
/*  878 */       return ShortLinkedOpenHashSet.this.key[this.curr];
/*      */     }
/*      */     
...
/*  885 */       this.prev = (int)(ShortLinkedOpenHashSet.this.link[this.curr] >>> 32L);
/*  886 */       this.next = this.curr;
/*  887 */       if (this.index >= 0)
/*  888 */         this.index--; 
/*  889 */       return ShortLinkedOpenHashSet.this.key[this.curr];
/*      */     }
...
/*      */       } 
/*  898 */       if (this.next == -1) {
/*  899 */         this.index = ShortLinkedOpenHashSet.this.size;
/*      */         return;
/*      */       } 
/*  902 */       int pos = ShortLinkedOpenHashSet.this.first;
/*  903 */       this.index = 1;
/*  904 */       while (pos != this.prev) {
/*  905 */         pos = (int)ShortLinkedOpenHashSet.this.link[pos];
/*  906 */         this.index++;
/*      */       } 
/*      */     }
/*      */     
...
/*      */         
/*  929 */         this.index--;
/*  930 */         this.prev = (int)(ShortLinkedOpenHashSet.this.link[this.curr] >>> 32L);
/*      */       } else {
/*  932 */         this.next = (int)ShortLinkedOpenHashSet.this.link[this.curr];
/*  933 */       }  ShortLinkedOpenHashSet.this.size--;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/*  938 */       if (this.prev == -1) {
/*  939 */         ShortLinkedOpenHashSet.this.first = this.next;
/*      */       } else {
/*  941 */         ShortLinkedOpenHashSet.this.link[this.prev] = ShortLinkedOpenHashSet.this.link[this.prev] ^ (ShortLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  942 */       }  if (this.next == -1) {
/*  943 */         ShortLinkedOpenHashSet.this.last = this.prev;
/*      */       } else {
/*  945 */         ShortLinkedOpenHashSet.this.link[this.next] = ShortLinkedOpenHashSet.this.link[this.next] ^ (ShortLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/*  946 */       }  int pos = this.curr;
/*  947 */       this.curr = -1;
/*  948 */       if (pos == ShortLinkedOpenHashSet.this.n) {
/*  949 */         ShortLinkedOpenHashSet.this.containsNull = false;
/*  950 */         ShortLinkedOpenHashSet.this.key[ShortLinkedOpenHashSet.this.n] = 0;
/*      */       } else {
/*      */         
/*  953 */         short[] key = ShortLinkedOpenHashSet.this.key;
/*      */         while (true) {
/*      */           short curr;
/*      */           int last;
/*  957 */           pos = (last = pos) + 1 & ShortLinkedOpenHashSet.this.mask;
/*      */           while (true) {
/*  959 */             if ((curr = key[pos]) == 0) {
/*  960 */               key[last] = 0;
/*      */               return;
/*      */             } 
/*  963 */             int slot = HashCommon.mix(curr) & ShortLinkedOpenHashSet.this.mask;
/*  964 */             if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/*      */               break; 
/*  966 */             pos = pos + 1 & ShortLinkedOpenHashSet.this.mask;
/*      */           } 
/*  968 */           key[last] = curr;
...
/*  972 */             this.prev = last; 
/*  973 */           ShortLinkedOpenHashSet.this.fixPointers(pos, last);
/*      */         } 
/*      */       } 
/*      */     }
/*      */   }
...
/* 1075 */     short[] key = this.key;
/* 1076 */     int mask = newN - 1;
/* 1077 */     short[] newKey = new short[newN + 1];
...
/* 1083 */       int pos; if (key[i] == 0) {
/* 1084 */         pos = newN;
/*      */       } else {
/* 1086 */         pos = HashCommon.mix(key[i]) & mask;
/* 1087 */         while (newKey[pos] != 0)
/* 1088 */           pos = pos + 1 & mask; 
/*      */       } 
...
/* 1160 */     ShortIterator i = iterator();
/* 1161 */     s.defaultWriteObject();
/* 1162 */     for (int j = this.size; j-- != 0;)
/* 1163 */       s.writeShort(i.nextShort()); 
/*      */   }
/*      */   
...
/* 1171 */     short[] key = this.key = new short[this.n + 1];
/* 1172 */     long[] link = this.link = new long[this.n + 1];
/* 1173 */     int prev = -1;
/* 1174 */     this.first = this.last = -1;
/*      */     
/* 1176 */     for (int i = this.size; i-- != 0; ) {
/* 1177 */       int pos; short k = s.readShort();
/* 1178 */       if (k == 0) {
/* 1179 */         pos = this.n;
/* 1180 */         this.containsNull = true;
/*      */       }
/* 1182 */       else if (key[pos = HashCommon.mix(k) & this.mask] != 0) {
/* 1183 */         while (key[pos = pos + 1 & this.mask] != 0);
/*      */       } 
/* 1185 */       key[pos] = k;
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\shorts\ShortLinkedOpenHashSet.class
