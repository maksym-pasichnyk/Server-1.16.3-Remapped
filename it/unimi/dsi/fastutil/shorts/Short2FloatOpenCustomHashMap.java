...
/*      */   
/*      */   public boolean trim() {
/* 1124 */     int l = HashCommon.arraySize(this.size, this.f);
...
/* 1157 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1158 */       return true; 
/*      */     try {
/* 1160 */       rehash(l);
...
/* 1180 */     short[] key = this.key;
/* 1181 */     float[] value = this.value;
/* 1182 */     int mask = newN - 1;
/* 1183 */     short[] newKey = new short[newN + 1];
/* 1184 */     float[] newValue = new float[newN + 1];
/* 1185 */     int i = this.n;
/* 1186 */     for (int j = realSize(); j-- != 0; ) {
/* 1187 */       while (key[--i] == 0); int pos;
/* 1188 */       if (newKey[pos = HashCommon.mix(this.strategy.hashCode(key[i])) & mask] != 0)
/*      */       {
/* 1190 */         while (newKey[pos = pos + 1 & mask] != 0); } 
...
/*      */     try {
/* 1216 */       c = (Short2FloatOpenCustomHashMap)super.clone();
...
/* 1224 */     c.key = (short[])this.key.clone();
/* 1225 */     c.value = (float[])this.value.clone();
...
/* 1242 */       while (this.key[i] == 0)
/* 1243 */         i++; 
/* 1244 */       t = this.strategy.hashCode(this.key[i]);
/* 1245 */       t ^= HashCommon.float2int(this.value[i]);
/* 1246 */       h += t;
/* 1247 */       i++;
/*      */     } 
/*      */     
/* 1250 */     if (this.containsNullKey)
/* 1251 */       h += HashCommon.float2int(this.value[this.n]); 
/* 1252 */     return h;
/*      */   }
/*      */   private void writeObject(ObjectOutputStream s) throws IOException {
/* 1255 */     short[] key = this.key;
/* 1256 */     float[] value = this.value;
/* 1257 */     MapIterator i = new MapIterator();
/* 1258 */     s.defaultWriteObject();
/* 1259 */     for (int j = this.size; j-- != 0; ) {
/* 1260 */       int e = i.nextEntry();
/* 1261 */       s.writeShort(key[e]);
/* 1262 */       s.writeFloat(value[e]);
/*      */     } 
/*      */   }
/*      */   
...
/* 1271 */     short[] key = this.key = new short[this.n + 1];
/* 1272 */     float[] value = this.value = new float[this.n + 1];
/*      */ 
/*      */     
/* 1275 */     for (int i = this.size; i-- != 0; ) {
/* 1276 */       int pos; short k = s.readShort();
/* 1277 */       float v = s.readFloat();
/* 1278 */       if (this.strategy.equals(k, (short)0)) {
/* 1279 */         pos = this.n;
/* 1280 */         this.containsNullKey = true;
/*      */       } else {
/* 1282 */         pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask;
/* 1283 */         while (key[pos] != 0)
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\shorts\Short2FloatOpenCustomHashMap.class
