...
/*      */ {
/*   63 */   public static final short[][] EMPTY_BIG_ARRAY = new short[0][];
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*   72 */   public static final short[][] DEFAULT_EMPTY_BIG_ARRAY = new short[0][];
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*  125 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (short)(array[BigArrays.segment(index)][BigArrays.displacement(index)] + incr);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  139 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (short)(array[BigArrays.segment(index)][BigArrays.displacement(index)] * factor);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  150 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (short)(array[BigArrays.segment(index)][BigArrays.displacement(index)] + 1);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  161 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (short)(array[BigArrays.segment(index)][BigArrays.displacement(index)] - 1);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  309 */     short[][] base = new short[baseLength][];
/*  310 */     int residual = (int)(length & 0x7FFFFFFL);
/*  311 */     if (residual != 0) {
/*  312 */       for (int i = 0; i < baseLength - 1; i++)
/*  313 */         base[i] = new short[134217728]; 
/*  314 */       base[baseLength - 1] = new short[residual];
/*      */     } else {
/*  316 */       for (int i = 0; i < baseLength; i++)
/*  317 */         base[i] = new short[134217728]; 
/*  318 */     }  return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  335 */       return new short[][] { array }; 
/*  336 */     short[][] bigArray = newBigArray(array.length);
...
/*  388 */     short[][] base = Arrays.<short[]>copyOf(array, baseLength);
/*  389 */     int residual = (int)(length & 0x7FFFFFFL);
/*  390 */     if (residual != 0) {
/*  391 */       for (int i = valid; i < baseLength - 1; i++)
/*  392 */         base[i] = new short[134217728]; 
/*  393 */       base[baseLength - 1] = new short[residual];
/*      */     } else {
/*  395 */       for (int i = valid; i < baseLength; i++)
/*  396 */         base[i] = new short[134217728]; 
...
/*  507 */     short[][] base = Arrays.<short[]>copyOf(array, baseLength);
/*  508 */     int residual = (int)(length & 0x7FFFFFFL);
/*  509 */     if (residual != 0)
/*  510 */       base[baseLength - 1] = ShortArrays.trim(base[baseLength - 1], residual); 
/*  511 */     return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  555 */     short[][] a = newBigArray(length);
...
/*  567 */     short[][] base = (short[][])array.clone();
/*  568 */     for (int i = base.length; i-- != 0;)
/*  569 */       base[i] = (short[])array[i].clone(); 
/*  570 */     return base;
/*      */   }
/*      */ 
/*      */ 
...
/*      */   }
/*      */   private static final class BigArrayHashStrategy implements Hash.Strategy<short[][]>, Serializable { private static final long serialVersionUID = -7046029254386353129L;
/*      */     
...
/*  733 */       return ShortBigArrays.equals(a, b);
/*      */     } }
/*      */ 
/*      */ 
/*      */   
...
/*      */       return;
/*      */     } 
/*      */     swap(x, b++, c--); } private static long med3(short[][] x, long a, long b, long c) {
/*  839 */     int ab = Short.compare(get(x, a), get(x, b));
/*  840 */     int ac = Short.compare(get(x, a), get(x, c));
/*  841 */     int bc = Short.compare(get(x, b), get(x, c));
...
/*      */     
/*      */     int comparison;
/*  915 */     while (b <= c && (comparison = Short.compare(get(x, b), v)) <= 0) {
/*  916 */       if (comparison == 0)
/*  917 */         swap(x, a++, b); 
/*  918 */       b++;
/*      */     } 
/*  920 */     while (c >= b && (comparison = Short.compare(get(x, c), v)) >= 0) {
/*  921 */       if (comparison == 0)
/*  922 */         swap(x, c, d--); 
/*  923 */       c--;
/*      */     } 
...
/* 1151 */     int maxLevel = 1;
/* 1152 */     int stackSize = 256;
/* 1153 */     long[] offsetStack = new long[256];
/* 1154 */     int offsetPos = 0;
/* 1155 */     long[] lengthStack = new long[256];
/* 1156 */     int lengthPos = 0;
/* 1157 */     int[] levelStack = new int[256];
/* 1158 */     int levelPos = 0;
...
/* 1169 */       int signMask = (level % 2 == 0) ? 128 : 0;
/* 1170 */       if (length < 40L) {
/* 1171 */         selectionSort(a, first, first + length);
/*      */         continue;
/*      */       } 
/* 1174 */       int shift = (1 - level % 2) * 8;
/*      */ 
/*      */       
/*      */       long i;
/*      */       
...
/* 1310 */     int maxLevel = 3;
/* 1311 */     int stackSize = 766;
/* 1312 */     long[] offsetStack = new long[766];
/* 1313 */     int offsetPos = 0;
/* 1314 */     long[] lengthStack = new long[766];
/* 1315 */     int lengthPos = 0;
/* 1316 */     int[] levelStack = new int[766];
/* 1317 */     int levelPos = 0;
...
/* 1328 */       int signMask = (level % 2 == 0) ? 128 : 0;
/* 1329 */       if (length < 40L) {
/* 1330 */         selectionSort(a, b, first, first + length);
/*      */         continue;
/*      */       } 
/* 1333 */       short[][] k = (level < 2) ? a : b;
/* 1334 */       int shift = (1 - level % 2) * 8;
/*      */ 
/*      */       
/*      */       long i;
/*      */       
...
/* 1364 */         short t = get(a, l1 + first);
/* 1365 */         short u = get(b, l1 + first);
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\shorts\ShortBigArrays.class
