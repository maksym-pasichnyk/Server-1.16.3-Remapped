...
/*   69 */     this.actualComparator = ShortComparators.asShortComparator(this.storedComparator);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  142 */     this(ShortIterators.asShortIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  158 */     this(c);
/*  159 */     ShortArrays.ensureOffsetLength(a, offset, length);
...
/*  174 */     this(a, offset, length, (Comparator<? super Short>)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  229 */     return (this.actualComparator == null) ? Short.compare(k1, k2) : this.actualComparator.compare(k1, k2);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  980 */     int index = 0;
/*      */     SetIterator() {
/*  982 */       this.next = ShortAVLTreeSet.this.firstEntry;
/*      */     }
/*      */     SetIterator(short k) {
/*  985 */       if ((this.next = ShortAVLTreeSet.this.locateKey(k)) != null)
/*  986 */         if (ShortAVLTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1050 */       updatePrevious();
/* 1051 */       updateNext();
/* 1052 */       ShortAVLTreeSet.this.remove(this.curr.key);
/* 1053 */       this.curr = null;
/*      */     }
/*      */   }
/*      */   
...
/* 1070 */     return new Subset((short)0, true, to, false);
/*      */   }
/*      */   
/*      */   public ShortSortedSet tailSet(short from) {
/* 1074 */     return new Subset(from, false, (short)0, true);
/*      */   }
/*      */   
...
/* 1116 */       if (!bottom && !top && ShortAVLTreeSet.this.compare(from, to) > 0) {
...
/* 1140 */       return ((this.bottom || ShortAVLTreeSet.this.compare(k, this.from) >= 0) && (this.top || ShortAVLTreeSet.this
/* 1141 */         .compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean contains(short k) {
/* 1146 */       return (in(k) && ShortAVLTreeSet.this.contains(k));
/*      */     }
/*      */     
/*      */     public boolean add(short k) {
/* 1150 */       if (!in(k))
...
/* 1153 */       return ShortAVLTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean remove(short k) {
/* 1158 */       if (!in(k))
/* 1159 */         return false; 
/* 1160 */       return ShortAVLTreeSet.this.remove(k);
/*      */     }
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public ShortComparator comparator() {
/* 1178 */       return ShortAVLTreeSet.this.actualComparator;
/*      */     }
/*      */     
/*      */     public ShortBidirectionalIterator iterator() {
...
/* 1192 */       return (ShortAVLTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */     
/*      */     public ShortSortedSet tailSet(short from) {
/* 1196 */       if (this.bottom)
/* 1197 */         return new Subset(from, false, this.to, this.top); 
/* 1198 */       return (ShortAVLTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */     
/*      */     public ShortSortedSet subSet(short from, short to) {
...
/* 1204 */       if (!this.top)
/* 1205 */         to = (ShortAVLTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1206 */       if (!this.bottom)
/* 1207 */         from = (ShortAVLTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1219 */       if (ShortAVLTreeSet.this.tree == null) {
/* 1220 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1224 */       if (this.bottom) {
/* 1225 */         e = ShortAVLTreeSet.this.firstEntry;
/*      */       } else {
/* 1227 */         e = ShortAVLTreeSet.this.locateKey(this.from);
/*      */         
/* 1229 */         if (ShortAVLTreeSet.this.compare(e.key, this.from) < 0) {
/* 1230 */           e = e.next();
/*      */         }
/*      */       } 
/*      */       
/* 1234 */       if (e == null || (!this.top && ShortAVLTreeSet.this.compare(e.key, this.to) >= 0))
/* 1235 */         return null; 
/* 1236 */       return e;
/*      */     }
/*      */ 
...
/*      */       ShortAVLTreeSet.Entry e;
/* 1245 */       if (ShortAVLTreeSet.this.tree == null) {
/* 1246 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1250 */       if (this.top) {
/* 1251 */         e = ShortAVLTreeSet.this.lastEntry;
/*      */       } else {
/* 1253 */         e = ShortAVLTreeSet.this.locateKey(this.to);
/*      */         
/* 1255 */         if (ShortAVLTreeSet.this.compare(e.key, this.to) >= 0) {
/* 1256 */           e = e.prev();
/*      */         }
/*      */       } 
/*      */       
/* 1260 */       if (e == null || (!this.bottom && ShortAVLTreeSet.this.compare(e.key, this.from) < 0))
/* 1261 */         return null; 
/* 1262 */       return e;
/*      */     }
/*      */     
/*      */     public short firstShort() {
/* 1266 */       ShortAVLTreeSet.Entry e = firstEntry();
/* 1267 */       if (e == null)
...
/*      */     
/*      */     public short lastShort() {
/* 1273 */       ShortAVLTreeSet.Entry e = lastEntry();
/* 1274 */       if (e == null)
...
/*      */     {
/*      */       SubsetIterator() {
/* 1289 */         this.next = ShortAVLTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       SubsetIterator(short k) {
/* 1292 */         this();
/* 1293 */         if (this.next != null)
/* 1294 */           if (!ShortAVLTreeSet.Subset.this.bottom && ShortAVLTreeSet.this.compare(k, this.next.key) < 0) {
/* 1295 */             this.prev = null;
/* 1296 */           } else if (!ShortAVLTreeSet.Subset.this.top && ShortAVLTreeSet.this.compare(k, (this.prev = ShortAVLTreeSet.Subset.this.lastEntry()).key) >= 0) {
/* 1297 */             this.next = null;
/*      */           } else {
/* 1299 */             this.next = ShortAVLTreeSet.this.locateKey(k);
/* 1300 */             if (ShortAVLTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1311 */         if (!ShortAVLTreeSet.Subset.this.bottom && this.prev != null && ShortAVLTreeSet.this.compare(this.prev.key, ShortAVLTreeSet.Subset.this.from) < 0)
/* 1312 */           this.prev = null; 
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1316 */         this.next = this.next.next();
/* 1317 */         if (!ShortAVLTreeSet.Subset.this.top && this.next != null && ShortAVLTreeSet.this.compare(this.next.key, ShortAVLTreeSet.Subset.this.to) >= 0) {
/* 1318 */           this.next = null;
/*      */         }
/*      */       }
/*      */     }
...
/*      */     ShortAVLTreeSet c;
/*      */     try {
/* 1337 */       c = (ShortAVLTreeSet)super.clone();
...
/* 1411 */     if (n == 1) {
/* 1412 */       Entry entry = new Entry(s.readShort());
...
/*      */ 
/*      */       
/* 1422 */       Entry entry = new Entry(s.readShort());
/* 1423 */       entry.right(new Entry(s.readShort()));
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\shorts\ShortAVLTreeSet.class
