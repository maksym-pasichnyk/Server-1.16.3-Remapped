...
/*   70 */     this.actualComparator = ShortComparators.asShortComparator(this.storedComparator);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  143 */     this(ShortIterators.asShortIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  159 */     this(c);
/*  160 */     ShortArrays.ensureOffsetLength(a, offset, length);
...
/*  175 */     this(a, offset, length, (Comparator<? super Short>)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  230 */     return (this.actualComparator == null) ? Short.compare(k1, k2) : this.actualComparator.compare(k1, k2);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  939 */     int index = 0;
/*      */     SetIterator() {
/*  941 */       this.next = ShortRBTreeSet.this.firstEntry;
/*      */     }
/*      */     SetIterator(short k) {
/*  944 */       if ((this.next = ShortRBTreeSet.this.locateKey(k)) != null)
/*  945 */         if (ShortRBTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1009 */       updatePrevious();
/* 1010 */       updateNext();
/* 1011 */       ShortRBTreeSet.this.remove(this.curr.key);
/* 1012 */       this.curr = null;
/*      */     }
/*      */   }
/*      */   
...
/* 1029 */     return new Subset((short)0, true, to, false);
/*      */   }
/*      */   
/*      */   public ShortSortedSet tailSet(short from) {
/* 1033 */     return new Subset(from, false, (short)0, true);
/*      */   }
/*      */   
...
/* 1075 */       if (!bottom && !top && ShortRBTreeSet.this.compare(from, to) > 0) {
...
/* 1099 */       return ((this.bottom || ShortRBTreeSet.this.compare(k, this.from) >= 0) && (this.top || ShortRBTreeSet.this
/* 1100 */         .compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean contains(short k) {
/* 1105 */       return (in(k) && ShortRBTreeSet.this.contains(k));
/*      */     }
/*      */     
/*      */     public boolean add(short k) {
/* 1109 */       if (!in(k))
...
/* 1112 */       return ShortRBTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean remove(short k) {
/* 1117 */       if (!in(k))
/* 1118 */         return false; 
/* 1119 */       return ShortRBTreeSet.this.remove(k);
/*      */     }
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public ShortComparator comparator() {
/* 1137 */       return ShortRBTreeSet.this.actualComparator;
/*      */     }
/*      */     
/*      */     public ShortBidirectionalIterator iterator() {
...
/* 1151 */       return (ShortRBTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */     
/*      */     public ShortSortedSet tailSet(short from) {
/* 1155 */       if (this.bottom)
/* 1156 */         return new Subset(from, false, this.to, this.top); 
/* 1157 */       return (ShortRBTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */     
/*      */     public ShortSortedSet subSet(short from, short to) {
...
/* 1163 */       if (!this.top)
/* 1164 */         to = (ShortRBTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1165 */       if (!this.bottom)
/* 1166 */         from = (ShortRBTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1178 */       if (ShortRBTreeSet.this.tree == null) {
/* 1179 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1183 */       if (this.bottom) {
/* 1184 */         e = ShortRBTreeSet.this.firstEntry;
/*      */       } else {
/* 1186 */         e = ShortRBTreeSet.this.locateKey(this.from);
/*      */         
/* 1188 */         if (ShortRBTreeSet.this.compare(e.key, this.from) < 0) {
/* 1189 */           e = e.next();
/*      */         }
/*      */       } 
/*      */       
/* 1193 */       if (e == null || (!this.top && ShortRBTreeSet.this.compare(e.key, this.to) >= 0))
/* 1194 */         return null; 
/* 1195 */       return e;
/*      */     }
/*      */ 
...
/*      */       ShortRBTreeSet.Entry e;
/* 1204 */       if (ShortRBTreeSet.this.tree == null) {
/* 1205 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1209 */       if (this.top) {
/* 1210 */         e = ShortRBTreeSet.this.lastEntry;
/*      */       } else {
/* 1212 */         e = ShortRBTreeSet.this.locateKey(this.to);
/*      */         
/* 1214 */         if (ShortRBTreeSet.this.compare(e.key, this.to) >= 0) {
/* 1215 */           e = e.prev();
/*      */         }
/*      */       } 
/*      */       
/* 1219 */       if (e == null || (!this.bottom && ShortRBTreeSet.this.compare(e.key, this.from) < 0))
/* 1220 */         return null; 
/* 1221 */       return e;
/*      */     }
/*      */     
/*      */     public short firstShort() {
/* 1225 */       ShortRBTreeSet.Entry e = firstEntry();
/* 1226 */       if (e == null)
...
/*      */     
/*      */     public short lastShort() {
/* 1232 */       ShortRBTreeSet.Entry e = lastEntry();
/* 1233 */       if (e == null)
...
/*      */     {
/*      */       SubsetIterator() {
/* 1248 */         this.next = ShortRBTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       SubsetIterator(short k) {
/* 1251 */         this();
/* 1252 */         if (this.next != null)
/* 1253 */           if (!ShortRBTreeSet.Subset.this.bottom && ShortRBTreeSet.this.compare(k, this.next.key) < 0) {
/* 1254 */             this.prev = null;
/* 1255 */           } else if (!ShortRBTreeSet.Subset.this.top && ShortRBTreeSet.this.compare(k, (this.prev = ShortRBTreeSet.Subset.this.lastEntry()).key) >= 0) {
/* 1256 */             this.next = null;
/*      */           } else {
/* 1258 */             this.next = ShortRBTreeSet.this.locateKey(k);
/* 1259 */             if (ShortRBTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1270 */         if (!ShortRBTreeSet.Subset.this.bottom && this.prev != null && ShortRBTreeSet.this.compare(this.prev.key, ShortRBTreeSet.Subset.this.from) < 0)
/* 1271 */           this.prev = null; 
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1275 */         this.next = this.next.next();
/* 1276 */         if (!ShortRBTreeSet.Subset.this.top && this.next != null && ShortRBTreeSet.this.compare(this.next.key, ShortRBTreeSet.Subset.this.to) >= 0) {
/* 1277 */           this.next = null;
/*      */         }
/*      */       }
/*      */     }
...
/*      */     ShortRBTreeSet c;
/*      */     try {
/* 1296 */       c = (ShortRBTreeSet)super.clone();
...
/* 1370 */     if (n == 1) {
/* 1371 */       Entry entry = new Entry(s.readShort());
...
/*      */ 
/*      */       
/* 1382 */       Entry entry = new Entry(s.readShort());
/* 1383 */       entry.black(true);
/* 1384 */       entry.right(new Entry(s.readShort()));
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\shorts\ShortRBTreeSet.class
