...
/*      */   
/*      */   public boolean trim() {
/* 1080 */     int l = HashCommon.arraySize(this.size, this.f);
...
/* 1113 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1114 */       return true; 
/*      */     try {
/* 1116 */       rehash(l);
...
/* 1136 */     char[] key = this.key;
/* 1137 */     int mask = newN - 1;
/* 1138 */     char[] newKey = new char[newN + 1];
/* 1139 */     int i = this.first, prev = -1, newPrev = -1;
/* 1140 */     long[] link = this.link;
/* 1141 */     long[] newLink = new long[newN + 1];
/* 1142 */     this.first = -1;
/* 1143 */     for (int j = this.size; j-- != 0; ) {
/* 1144 */       int pos; if (this.strategy.equals(key[i], false)) {
/* 1145 */         pos = newN;
/*      */       } else {
/* 1147 */         pos = HashCommon.mix(this.strategy.hashCode(key[i])) & mask;
/* 1148 */         while (newKey[pos] != '\000')
/* 1149 */           pos = pos + 1 & mask; 
/*      */       } 
/* 1151 */       newKey[pos] = key[i];
/* 1152 */       if (prev != -1) {
/* 1153 */         newLink[newPrev] = newLink[newPrev] ^ (newLink[newPrev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1154 */         newLink[pos] = newLink[pos] ^ (newLink[pos] ^ (newPrev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1155 */         newPrev = pos;
/*      */       } else {
/* 1157 */         newPrev = this.first = pos;
/*      */         
/* 1159 */         newLink[pos] = -1L;
/*      */       } 
/* 1161 */       int t = i;
...
/* 1163 */       prev = t;
/*      */     } 
/* 1165 */     this.link = newLink;
/* 1166 */     this.last = newPrev;
/* 1167 */     if (newPrev != -1)
/*      */     {
/* 1169 */       newLink[newPrev] = newLink[newPrev] | 0xFFFFFFFFL; } 
/* 1170 */     this.n = newN;
/* 1171 */     this.mask = mask;
...
/*      */     try {
/* 1190 */       c = (CharLinkedOpenCustomHashSet)super.clone();
/* 1191 */     } catch (CloneNotSupportedException cantHappen) {
/* 1192 */       throw new InternalError();
/*      */     } 
/* 1194 */     c.key = (char[])this.key.clone();
/* 1195 */     c.containsNull = this.containsNull;
/* 1196 */     c.link = (long[])this.link.clone();
/* 1197 */     c.strategy = this.strategy;
/* 1198 */     return c;
/*      */   }
/*      */ 
/*      */ 
...
/*      */   public int hashCode() {
/* 1211 */     int h = 0;
/* 1212 */     for (int j = realSize(), i = 0; j-- != 0; ) {
/* 1213 */       while (this.key[i] == '\000')
/* 1214 */         i++; 
/* 1215 */       h += this.strategy.hashCode(this.key[i]);
/* 1216 */       i++;
/*      */     } 
/*      */     
/* 1219 */     return h;
...
/* 1222 */     CharIterator i = iterator();
/* 1223 */     s.defaultWriteObject();
/* 1224 */     for (int j = this.size; j-- != 0;)
/* 1225 */       s.writeChar(i.nextChar()); 
/*      */   }
/*      */   
...
/* 1240 */       if (this.strategy.equals(k, false)) {
/* 1241 */         pos = this.n;
/* 1242 */         this.containsNull = true;
/*      */       }
/* 1244 */       else if (key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask] != '\000') {
/* 1245 */         while (key[pos = pos + 1 & this.mask] != '\000');
/*      */       } 
/* 1247 */       key[pos] = k;
/* 1248 */       if (this.first != -1) {
/* 1249 */         link[prev] = link[prev] ^ (link[prev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1250 */         link[pos] = link[pos] ^ (link[pos] ^ (prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1251 */         prev = pos; continue;
/*      */       } 
/* 1253 */       prev = this.first = pos;
/*      */       
/* 1255 */       link[pos] = link[pos] | 0xFFFFFFFF00000000L;
/*      */     } 
/*      */     
/* 1258 */     this.last = prev;
/* 1259 */     if (prev != -1)
/*      */     {
/* 1261 */       link[prev] = link[prev] | 0xFFFFFFFFL;
/*      */     }
/*      */   }
/*      */   
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\chars\CharLinkedOpenCustomHashSet.class
