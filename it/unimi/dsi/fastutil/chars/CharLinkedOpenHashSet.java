/*      */ package it.unimi.dsi.fastutil.chars;
/*      */ 
/*      */ import it.unimi.dsi.fastutil.Hash;
/*      */ import it.unimi.dsi.fastutil.HashCommon;
/*      */ import java.io.IOException;
/*      */ import java.io.ObjectInputStream;
/*      */ import java.io.ObjectOutputStream;
/*      */ import java.io.Serializable;
/*      */ import java.util.Arrays;
/*      */ import java.util.Collection;
/*      */ import java.util.Comparator;
/*      */ import java.util.Iterator;
/*      */ import java.util.NoSuchElementException;
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ public class CharLinkedOpenHashSet
/*      */   extends AbstractCharSortedSet
/*      */   implements Serializable, Cloneable, Hash
/*      */ {
/*      */   private static final long serialVersionUID = 0L;
/*      */   private static final boolean ASSERTS = false;
/*      */   protected transient char[] key;
/*      */   protected transient int mask;
/*      */   protected transient boolean containsNull;
/*   92 */   protected transient int first = -1;
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*   97 */   protected transient int last = -1;
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected transient long[] link;
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected transient int n;
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected transient int maxFill;
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected final transient int minN;
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected int size;
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected final float f;
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(int expected, float f) {
/*  135 */     if (f <= 0.0F || f > 1.0F)
/*  136 */       throw new IllegalArgumentException("Load factor must be greater than 0 and smaller than or equal to 1"); 
/*  137 */     if (expected < 0)
/*  138 */       throw new IllegalArgumentException("The expected number of elements must be nonnegative"); 
/*  139 */     this.f = f;
/*  140 */     this.minN = this.n = HashCommon.arraySize(expected, f);
/*  141 */     this.mask = this.n - 1;
/*  142 */     this.maxFill = HashCommon.maxFill(this.n, f);
/*  143 */     this.key = new char[this.n + 1];
/*  144 */     this.link = new long[this.n + 1];
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(int expected) {
/*  153 */     this(expected, 0.75F);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet() {
/*  161 */     this(16, 0.75F);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(Collection<? extends Character> c, float f) {
/*  172 */     this(c.size(), f);
/*  173 */     addAll(c);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(Collection<? extends Character> c) {
/*  183 */     this(c, 0.75F);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(CharCollection c, float f) {
/*  194 */     this(c.size(), f);
/*  195 */     addAll(c);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(CharCollection c) {
/*  205 */     this(c, 0.75F);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(CharIterator i, float f) {
/*  216 */     this(16, f);
/*  217 */     while (i.hasNext()) {
/*  218 */       add(i.nextChar());
/*      */     }
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(CharIterator i) {
/*  228 */     this(i, 0.75F);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(Iterator<?> i, float f) {
/*  239 */     this(CharIterators.asCharIterator(i), f);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(Iterator<?> i) {
/*  249 */     this(CharIterators.asCharIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(char[] a, int offset, int length, float f) {
/*  264 */     this((length < 0) ? 0 : length, f);
/*  265 */     CharArrays.ensureOffsetLength(a, offset, length);
/*  266 */     for (int i = 0; i < length; i++) {
/*  267 */       add(a[offset + i]);
/*      */     }
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(char[] a, int offset, int length) {
/*  281 */     this(a, offset, length, 0.75F);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(char[] a, float f) {
/*  292 */     this(a, 0, a.length, f);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet(char[] a) {
/*  302 */     this(a, 0.75F);
/*      */   }
/*      */   private int realSize() {
/*  305 */     return this.containsNull ? (this.size - 1) : this.size;
/*      */   }
/*      */   private void ensureCapacity(int capacity) {
/*  308 */     int needed = HashCommon.arraySize(capacity, this.f);
/*  309 */     if (needed > this.n)
/*  310 */       rehash(needed); 
/*      */   }
/*      */   private void tryCapacity(long capacity) {
/*  313 */     int needed = (int)Math.min(1073741824L, 
/*  314 */         Math.max(2L, HashCommon.nextPowerOfTwo((long)Math.ceil(((float)capacity / this.f)))));
/*  315 */     if (needed > this.n)
/*  316 */       rehash(needed); 
/*      */   }
/*      */   
/*      */   public boolean addAll(CharCollection c) {
/*  320 */     if (this.f <= 0.5D) {
/*  321 */       ensureCapacity(c.size());
/*      */     } else {
/*  323 */       tryCapacity((size() + c.size()));
/*      */     } 
/*  325 */     return super.addAll(c);
/*      */   }
/*      */ 
/*      */   
/*      */   public boolean addAll(Collection<? extends Character> c) {
/*  330 */     if (this.f <= 0.5D) {
/*  331 */       ensureCapacity(c.size());
/*      */     } else {
/*  333 */       tryCapacity((size() + c.size()));
/*      */     } 
/*  335 */     return super.addAll(c);
/*      */   }
/*      */   
/*      */   public boolean add(char k) {
/*      */     int pos;
/*  340 */     if (k == '\000') {
/*  341 */       if (this.containsNull)
/*  342 */         return false; 
/*  343 */       pos = this.n;
/*  344 */       this.containsNull = true;
/*      */     } else {
/*      */       
/*  347 */       char[] key = this.key;
/*      */       char curr;
/*  349 */       if ((curr = key[pos = HashCommon.mix(k) & this.mask]) != '\000') {
/*  350 */         if (curr == k)
/*  351 */           return false; 
/*  352 */         while ((curr = key[pos = pos + 1 & this.mask]) != '\000') {
/*  353 */           if (curr == k)
/*  354 */             return false; 
/*      */         } 
/*  356 */       }  key[pos] = k;
/*      */     } 
/*  358 */     if (this.size == 0) {
/*  359 */       this.first = this.last = pos;
/*      */       
/*  361 */       this.link[pos] = -1L;
/*      */     } else {
/*  363 */       this.link[this.last] = this.link[this.last] ^ (this.link[this.last] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  364 */       this.link[pos] = (this.last & 0xFFFFFFFFL) << 32L | 0xFFFFFFFFL;
/*  365 */       this.last = pos;
/*      */     } 
/*  367 */     if (this.size++ >= this.maxFill) {
/*  368 */       rehash(HashCommon.arraySize(this.size + 1, this.f));
/*      */     }
/*      */     
/*  371 */     return true;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected final void shiftKeys(int pos) {
/*  384 */     char[] key = this.key; while (true) {
/*      */       char curr; int last;
/*  386 */       pos = (last = pos) + 1 & this.mask;
/*      */       while (true) {
/*  388 */         if ((curr = key[pos]) == '\000') {
/*  389 */           key[last] = Character.MIN_VALUE;
/*      */           return;
/*      */         } 
/*  392 */         int slot = HashCommon.mix(curr) & this.mask;
/*  393 */         if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/*      */           break; 
/*  395 */         pos = pos + 1 & this.mask;
/*      */       } 
/*  397 */       key[last] = curr;
/*  398 */       fixPointers(pos, last);
/*      */     } 
/*      */   }
/*      */   private boolean removeEntry(int pos) {
/*  402 */     this.size--;
/*  403 */     fixPointers(pos);
/*  404 */     shiftKeys(pos);
/*  405 */     if (this.n > this.minN && this.size < this.maxFill / 4 && this.n > 16)
/*  406 */       rehash(this.n / 2); 
/*  407 */     return true;
/*      */   }
/*      */   private boolean removeNullEntry() {
/*  410 */     this.containsNull = false;
/*  411 */     this.key[this.n] = Character.MIN_VALUE;
/*  412 */     this.size--;
/*  413 */     fixPointers(this.n);
/*  414 */     if (this.n > this.minN && this.size < this.maxFill / 4 && this.n > 16)
/*  415 */       rehash(this.n / 2); 
/*  416 */     return true;
/*      */   }
/*      */ 
/*      */   
/*      */   public boolean remove(char k) {
/*  421 */     if (k == '\000') {
/*  422 */       if (this.containsNull)
/*  423 */         return removeNullEntry(); 
/*  424 */       return false;
/*      */     } 
/*      */     
/*  427 */     char[] key = this.key;
/*      */     char curr;
/*      */     int pos;
/*  430 */     if ((curr = key[pos = HashCommon.mix(k) & this.mask]) == '\000')
/*  431 */       return false; 
/*  432 */     if (k == curr)
/*  433 */       return removeEntry(pos); 
/*      */     while (true) {
/*  435 */       if ((curr = key[pos = pos + 1 & this.mask]) == '\000')
/*  436 */         return false; 
/*  437 */       if (k == curr) {
/*  438 */         return removeEntry(pos);
/*      */       }
/*      */     } 
/*      */   }
/*      */   
/*      */   public boolean contains(char k) {
/*  444 */     if (k == '\000') {
/*  445 */       return this.containsNull;
/*      */     }
/*  447 */     char[] key = this.key;
/*      */     char curr;
/*      */     int pos;
/*  450 */     if ((curr = key[pos = HashCommon.mix(k) & this.mask]) == '\000')
/*  451 */       return false; 
/*  452 */     if (k == curr)
/*  453 */       return true; 
/*      */     while (true) {
/*  455 */       if ((curr = key[pos = pos + 1 & this.mask]) == '\000')
/*  456 */         return false; 
/*  457 */       if (k == curr) {
/*  458 */         return true;
/*      */       }
/*      */     } 
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public char removeFirstChar() {
/*  469 */     if (this.size == 0)
/*  470 */       throw new NoSuchElementException(); 
/*  471 */     int pos = this.first;
/*      */     
/*  473 */     this.first = (int)this.link[pos];
/*  474 */     if (0 <= this.first)
/*      */     {
/*  476 */       this.link[this.first] = this.link[this.first] | 0xFFFFFFFF00000000L;
/*      */     }
/*  478 */     char k = this.key[pos];
/*  479 */     this.size--;
/*  480 */     if (k == '\000') {
/*  481 */       this.containsNull = false;
/*  482 */       this.key[this.n] = Character.MIN_VALUE;
/*      */     } else {
/*  484 */       shiftKeys(pos);
/*  485 */     }  if (this.n > this.minN && this.size < this.maxFill / 4 && this.n > 16)
/*  486 */       rehash(this.n / 2); 
/*  487 */     return k;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public char removeLastChar() {
/*  497 */     if (this.size == 0)
/*  498 */       throw new NoSuchElementException(); 
/*  499 */     int pos = this.last;
/*      */     
/*  501 */     this.last = (int)(this.link[pos] >>> 32L);
/*  502 */     if (0 <= this.last)
/*      */     {
/*  504 */       this.link[this.last] = this.link[this.last] | 0xFFFFFFFFL;
/*      */     }
/*  506 */     char k = this.key[pos];
/*  507 */     this.size--;
/*  508 */     if (k == '\000') {
/*  509 */       this.containsNull = false;
/*  510 */       this.key[this.n] = Character.MIN_VALUE;
/*      */     } else {
/*  512 */       shiftKeys(pos);
/*  513 */     }  if (this.n > this.minN && this.size < this.maxFill / 4 && this.n > 16)
/*  514 */       rehash(this.n / 2); 
/*  515 */     return k;
/*      */   }
/*      */   private void moveIndexToFirst(int i) {
/*  518 */     if (this.size == 1 || this.first == i)
/*      */       return; 
/*  520 */     if (this.last == i) {
/*  521 */       this.last = (int)(this.link[i] >>> 32L);
/*      */       
/*  523 */       this.link[this.last] = this.link[this.last] | 0xFFFFFFFFL;
/*      */     } else {
/*  525 */       long linki = this.link[i];
/*  526 */       int prev = (int)(linki >>> 32L);
/*  527 */       int next = (int)linki;
/*  528 */       this.link[prev] = this.link[prev] ^ (this.link[prev] ^ linki & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  529 */       this.link[next] = this.link[next] ^ (this.link[next] ^ linki & 0xFFFFFFFF00000000L) & 0xFFFFFFFF00000000L;
/*      */     } 
/*  531 */     this.link[this.first] = this.link[this.first] ^ (this.link[this.first] ^ (i & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/*  532 */     this.link[i] = 0xFFFFFFFF00000000L | this.first & 0xFFFFFFFFL;
/*  533 */     this.first = i;
/*      */   }
/*      */   private void moveIndexToLast(int i) {
/*  536 */     if (this.size == 1 || this.last == i)
/*      */       return; 
/*  538 */     if (this.first == i) {
/*  539 */       this.first = (int)this.link[i];
/*      */       
/*  541 */       this.link[this.first] = this.link[this.first] | 0xFFFFFFFF00000000L;
/*      */     } else {
/*  543 */       long linki = this.link[i];
/*  544 */       int prev = (int)(linki >>> 32L);
/*  545 */       int next = (int)linki;
/*  546 */       this.link[prev] = this.link[prev] ^ (this.link[prev] ^ linki & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  547 */       this.link[next] = this.link[next] ^ (this.link[next] ^ linki & 0xFFFFFFFF00000000L) & 0xFFFFFFFF00000000L;
/*      */     } 
/*  549 */     this.link[this.last] = this.link[this.last] ^ (this.link[this.last] ^ i & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  550 */     this.link[i] = (this.last & 0xFFFFFFFFL) << 32L | 0xFFFFFFFFL;
/*  551 */     this.last = i;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public boolean addAndMoveToFirst(char k) {
/*      */     int pos;
/*  563 */     if (k == '\000') {
/*  564 */       if (this.containsNull) {
/*  565 */         moveIndexToFirst(this.n);
/*  566 */         return false;
/*      */       } 
/*  568 */       this.containsNull = true;
/*  569 */       pos = this.n;
/*      */     } else {
/*      */       
/*  572 */       char[] key = this.key;
/*  573 */       pos = HashCommon.mix(k) & this.mask;
/*      */       
/*  575 */       while (key[pos] != '\000') {
/*  576 */         if (k == key[pos]) {
/*  577 */           moveIndexToFirst(pos);
/*  578 */           return false;
/*      */         } 
/*  580 */         pos = pos + 1 & this.mask;
/*      */       } 
/*      */     } 
/*  583 */     this.key[pos] = k;
/*  584 */     if (this.size == 0) {
/*  585 */       this.first = this.last = pos;
/*      */       
/*  587 */       this.link[pos] = -1L;
/*      */     } else {
/*  589 */       this.link[this.first] = this.link[this.first] ^ (this.link[this.first] ^ (pos & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/*  590 */       this.link[pos] = 0xFFFFFFFF00000000L | this.first & 0xFFFFFFFFL;
/*  591 */       this.first = pos;
/*      */     } 
/*  593 */     if (this.size++ >= this.maxFill) {
/*  594 */       rehash(HashCommon.arraySize(this.size, this.f));
/*      */     }
/*      */     
/*  597 */     return true;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public boolean addAndMoveToLast(char k) {
/*      */     int pos;
/*  609 */     if (k == '\000') {
/*  610 */       if (this.containsNull) {
/*  611 */         moveIndexToLast(this.n);
/*  612 */         return false;
/*      */       } 
/*  614 */       this.containsNull = true;
/*  615 */       pos = this.n;
/*      */     } else {
/*      */       
/*  618 */       char[] key = this.key;
/*  619 */       pos = HashCommon.mix(k) & this.mask;
/*      */       
/*  621 */       while (key[pos] != '\000') {
/*  622 */         if (k == key[pos]) {
/*  623 */           moveIndexToLast(pos);
/*  624 */           return false;
/*      */         } 
/*  626 */         pos = pos + 1 & this.mask;
/*      */       } 
/*      */     } 
/*  629 */     this.key[pos] = k;
/*  630 */     if (this.size == 0) {
/*  631 */       this.first = this.last = pos;
/*      */       
/*  633 */       this.link[pos] = -1L;
/*      */     } else {
/*  635 */       this.link[this.last] = this.link[this.last] ^ (this.link[this.last] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  636 */       this.link[pos] = (this.last & 0xFFFFFFFFL) << 32L | 0xFFFFFFFFL;
/*  637 */       this.last = pos;
/*      */     } 
/*  639 */     if (this.size++ >= this.maxFill) {
/*  640 */       rehash(HashCommon.arraySize(this.size, this.f));
/*      */     }
/*      */     
/*  643 */     return true;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public void clear() {
/*  654 */     if (this.size == 0)
/*      */       return; 
/*  656 */     this.size = 0;
/*  657 */     this.containsNull = false;
/*  658 */     Arrays.fill(this.key, false);
/*  659 */     this.first = this.last = -1;
/*      */   }
/*      */   
/*      */   public int size() {
/*  663 */     return this.size;
/*      */   }
/*      */   
/*      */   public boolean isEmpty() {
/*  667 */     return (this.size == 0);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected void fixPointers(int i) {
/*  677 */     if (this.size == 0) {
/*  678 */       this.first = this.last = -1;
/*      */       return;
/*      */     } 
/*  681 */     if (this.first == i) {
/*  682 */       this.first = (int)this.link[i];
/*  683 */       if (0 <= this.first)
/*      */       {
/*  685 */         this.link[this.first] = this.link[this.first] | 0xFFFFFFFF00000000L;
/*      */       }
/*      */       return;
/*      */     } 
/*  689 */     if (this.last == i) {
/*  690 */       this.last = (int)(this.link[i] >>> 32L);
/*  691 */       if (0 <= this.last)
/*      */       {
/*  693 */         this.link[this.last] = this.link[this.last] | 0xFFFFFFFFL;
/*      */       }
/*      */       return;
/*      */     } 
/*  697 */     long linki = this.link[i];
/*  698 */     int prev = (int)(linki >>> 32L);
/*  699 */     int next = (int)linki;
/*  700 */     this.link[prev] = this.link[prev] ^ (this.link[prev] ^ linki & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  701 */     this.link[next] = this.link[next] ^ (this.link[next] ^ linki & 0xFFFFFFFF00000000L) & 0xFFFFFFFF00000000L;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected void fixPointers(int s, int d) {
/*  713 */     if (this.size == 1) {
/*  714 */       this.first = this.last = d;
/*      */       
/*  716 */       this.link[d] = -1L;
/*      */       return;
/*      */     } 
/*  719 */     if (this.first == s) {
/*  720 */       this.first = d;
/*  721 */       this.link[(int)this.link[s]] = this.link[(int)this.link[s]] ^ (this.link[(int)this.link[s]] ^ (d & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/*  722 */       this.link[d] = this.link[s];
/*      */       return;
/*      */     } 
/*  725 */     if (this.last == s) {
/*  726 */       this.last = d;
/*  727 */       this.link[(int)(this.link[s] >>> 32L)] = this.link[(int)(this.link[s] >>> 32L)] ^ (this.link[(int)(this.link[s] >>> 32L)] ^ d & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  728 */       this.link[d] = this.link[s];
/*      */       return;
/*      */     } 
/*  731 */     long links = this.link[s];
/*  732 */     int prev = (int)(links >>> 32L);
/*  733 */     int next = (int)links;
/*  734 */     this.link[prev] = this.link[prev] ^ (this.link[prev] ^ d & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  735 */     this.link[next] = this.link[next] ^ (this.link[next] ^ (d & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/*  736 */     this.link[d] = links;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public char firstChar() {
/*  745 */     if (this.size == 0)
/*  746 */       throw new NoSuchElementException(); 
/*  747 */     return this.key[this.first];
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public char lastChar() {
/*  756 */     if (this.size == 0)
/*  757 */       throw new NoSuchElementException(); 
/*  758 */     return this.key[this.last];
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharSortedSet tailSet(char from) {
/*  767 */     throw new UnsupportedOperationException();
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharSortedSet headSet(char to) {
/*  776 */     throw new UnsupportedOperationException();
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharSortedSet subSet(char from, char to) {
/*  785 */     throw new UnsupportedOperationException();
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharComparator comparator() {
/*  794 */     return null;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   private class SetIterator
/*      */     implements CharListIterator
/*      */   {
/*  809 */     int prev = -1;
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */     
/*  815 */     int next = -1;
/*      */ 
/*      */ 
/*      */ 
/*      */     
/*  820 */     int curr = -1;
/*      */ 
/*      */ 
/*      */ 
/*      */     
/*  825 */     int index = -1;
/*      */     SetIterator() {
/*  827 */       this.next = CharLinkedOpenHashSet.this.first;
/*  828 */       this.index = 0;
/*      */     }
/*      */     SetIterator(char from) {
/*  831 */       if (from == '\000') {
/*  832 */         if (CharLinkedOpenHashSet.this.containsNull) {
/*  833 */           this.next = (int)CharLinkedOpenHashSet.this.link[CharLinkedOpenHashSet.this.n];
/*  834 */           this.prev = CharLinkedOpenHashSet.this.n;
/*      */           return;
/*      */         } 
/*  837 */         throw new NoSuchElementException("The key " + from + " does not belong to this set.");
/*      */       } 
/*  839 */       if (CharLinkedOpenHashSet.this.key[CharLinkedOpenHashSet.this.last] == from) {
/*  840 */         this.prev = CharLinkedOpenHashSet.this.last;
/*  841 */         this.index = CharLinkedOpenHashSet.this.size;
/*      */         
/*      */         return;
/*      */       } 
/*  845 */       char[] key = CharLinkedOpenHashSet.this.key;
/*  846 */       int pos = HashCommon.mix(from) & CharLinkedOpenHashSet.this.mask;
/*      */       
/*  848 */       while (key[pos] != '\000') {
/*  849 */         if (key[pos] == from) {
/*      */           
/*  851 */           this.next = (int)CharLinkedOpenHashSet.this.link[pos];
/*  852 */           this.prev = pos;
/*      */           return;
/*      */         } 
/*  855 */         pos = pos + 1 & CharLinkedOpenHashSet.this.mask;
/*      */       } 
/*  857 */       throw new NoSuchElementException("The key " + from + " does not belong to this set.");
/*      */     }
/*      */     
/*      */     public boolean hasNext() {
/*  861 */       return (this.next != -1);
/*      */     }
/*      */     
/*      */     public boolean hasPrevious() {
/*  865 */       return (this.prev != -1);
/*      */     }
/*      */     
/*      */     public char nextChar() {
/*  869 */       if (!hasNext())
/*  870 */         throw new NoSuchElementException(); 
/*  871 */       this.curr = this.next;
/*  872 */       this.next = (int)CharLinkedOpenHashSet.this.link[this.curr];
/*  873 */       this.prev = this.curr;
/*  874 */       if (this.index >= 0) {
/*  875 */         this.index++;
/*      */       }
/*      */       
/*  878 */       return CharLinkedOpenHashSet.this.key[this.curr];
/*      */     }
/*      */     
/*      */     public char previousChar() {
/*  882 */       if (!hasPrevious())
/*  883 */         throw new NoSuchElementException(); 
/*  884 */       this.curr = this.prev;
/*  885 */       this.prev = (int)(CharLinkedOpenHashSet.this.link[this.curr] >>> 32L);
/*  886 */       this.next = this.curr;
/*  887 */       if (this.index >= 0)
/*  888 */         this.index--; 
/*  889 */       return CharLinkedOpenHashSet.this.key[this.curr];
/*      */     }
/*      */     private final void ensureIndexKnown() {
/*  892 */       if (this.index >= 0)
/*      */         return; 
/*  894 */       if (this.prev == -1) {
/*  895 */         this.index = 0;
/*      */         return;
/*      */       } 
/*  898 */       if (this.next == -1) {
/*  899 */         this.index = CharLinkedOpenHashSet.this.size;
/*      */         return;
/*      */       } 
/*  902 */       int pos = CharLinkedOpenHashSet.this.first;
/*  903 */       this.index = 1;
/*  904 */       while (pos != this.prev) {
/*  905 */         pos = (int)CharLinkedOpenHashSet.this.link[pos];
/*  906 */         this.index++;
/*      */       } 
/*      */     }
/*      */     
/*      */     public int nextIndex() {
/*  911 */       ensureIndexKnown();
/*  912 */       return this.index;
/*      */     }
/*      */     
/*      */     public int previousIndex() {
/*  916 */       ensureIndexKnown();
/*  917 */       return this.index - 1;
/*      */     }
/*      */     
/*      */     public void remove() {
/*  921 */       ensureIndexKnown();
/*  922 */       if (this.curr == -1)
/*  923 */         throw new IllegalStateException(); 
/*  924 */       if (this.curr == this.prev) {
/*      */ 
/*      */ 
/*      */ 
/*      */         
/*  929 */         this.index--;
/*  930 */         this.prev = (int)(CharLinkedOpenHashSet.this.link[this.curr] >>> 32L);
/*      */       } else {
/*  932 */         this.next = (int)CharLinkedOpenHashSet.this.link[this.curr];
/*  933 */       }  CharLinkedOpenHashSet.this.size--;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/*  938 */       if (this.prev == -1) {
/*  939 */         CharLinkedOpenHashSet.this.first = this.next;
/*      */       } else {
/*  941 */         CharLinkedOpenHashSet.this.link[this.prev] = CharLinkedOpenHashSet.this.link[this.prev] ^ (CharLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/*  942 */       }  if (this.next == -1) {
/*  943 */         CharLinkedOpenHashSet.this.last = this.prev;
/*      */       } else {
/*  945 */         CharLinkedOpenHashSet.this.link[this.next] = CharLinkedOpenHashSet.this.link[this.next] ^ (CharLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/*  946 */       }  int pos = this.curr;
/*  947 */       this.curr = -1;
/*  948 */       if (pos == CharLinkedOpenHashSet.this.n) {
/*  949 */         CharLinkedOpenHashSet.this.containsNull = false;
/*  950 */         CharLinkedOpenHashSet.this.key[CharLinkedOpenHashSet.this.n] = Character.MIN_VALUE;
/*      */       } else {
/*      */         
/*  953 */         char[] key = CharLinkedOpenHashSet.this.key;
/*      */         while (true) {
/*      */           char curr;
/*      */           int last;
/*  957 */           pos = (last = pos) + 1 & CharLinkedOpenHashSet.this.mask;
/*      */           while (true) {
/*  959 */             if ((curr = key[pos]) == '\000') {
/*  960 */               key[last] = Character.MIN_VALUE;
/*      */               return;
/*      */             } 
/*  963 */             int slot = HashCommon.mix(curr) & CharLinkedOpenHashSet.this.mask;
/*  964 */             if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/*      */               break; 
/*  966 */             pos = pos + 1 & CharLinkedOpenHashSet.this.mask;
/*      */           } 
/*  968 */           key[last] = curr;
/*  969 */           if (this.next == pos)
/*  970 */             this.next = last; 
/*  971 */           if (this.prev == pos)
/*  972 */             this.prev = last; 
/*  973 */           CharLinkedOpenHashSet.this.fixPointers(pos, last);
/*      */         } 
/*      */       } 
/*      */     }
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharListIterator iterator(char from) {
/*  991 */     return new SetIterator(from);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharListIterator iterator() {
/* 1002 */     return new SetIterator();
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public boolean trim() {
/* 1019 */     int l = HashCommon.arraySize(this.size, this.f);
/* 1020 */     if (l >= this.n || this.size > HashCommon.maxFill(l, this.f))
/* 1021 */       return true; 
/*      */     try {
/* 1023 */       rehash(l);
/* 1024 */     } catch (OutOfMemoryError cantDoIt) {
/* 1025 */       return false;
/*      */     } 
/* 1027 */     return true;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public boolean trim(int n) {
/* 1051 */     int l = HashCommon.nextPowerOfTwo((int)Math.ceil((n / this.f)));
/* 1052 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1053 */       return true; 
/*      */     try {
/* 1055 */       rehash(l);
/* 1056 */     } catch (OutOfMemoryError cantDoIt) {
/* 1057 */       return false;
/*      */     } 
/* 1059 */     return true;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   protected void rehash(int newN) {
/* 1075 */     char[] key = this.key;
/* 1076 */     int mask = newN - 1;
/* 1077 */     char[] newKey = new char[newN + 1];
/* 1078 */     int i = this.first, prev = -1, newPrev = -1;
/* 1079 */     long[] link = this.link;
/* 1080 */     long[] newLink = new long[newN + 1];
/* 1081 */     this.first = -1;
/* 1082 */     for (int j = this.size; j-- != 0; ) {
/* 1083 */       int pos; if (key[i] == '\000') {
/* 1084 */         pos = newN;
/*      */       } else {
/* 1086 */         pos = HashCommon.mix(key[i]) & mask;
/* 1087 */         while (newKey[pos] != '\000')
/* 1088 */           pos = pos + 1 & mask; 
/*      */       } 
/* 1090 */       newKey[pos] = key[i];
/* 1091 */       if (prev != -1) {
/* 1092 */         newLink[newPrev] = newLink[newPrev] ^ (newLink[newPrev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1093 */         newLink[pos] = newLink[pos] ^ (newLink[pos] ^ (newPrev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1094 */         newPrev = pos;
/*      */       } else {
/* 1096 */         newPrev = this.first = pos;
/*      */         
/* 1098 */         newLink[pos] = -1L;
/*      */       } 
/* 1100 */       int t = i;
/* 1101 */       i = (int)link[i];
/* 1102 */       prev = t;
/*      */     } 
/* 1104 */     this.link = newLink;
/* 1105 */     this.last = newPrev;
/* 1106 */     if (newPrev != -1)
/*      */     {
/* 1108 */       newLink[newPrev] = newLink[newPrev] | 0xFFFFFFFFL; } 
/* 1109 */     this.n = newN;
/* 1110 */     this.mask = mask;
/* 1111 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/* 1112 */     this.key = newKey;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public CharLinkedOpenHashSet clone() {
/*      */     CharLinkedOpenHashSet c;
/*      */     try {
/* 1129 */       c = (CharLinkedOpenHashSet)super.clone();
/* 1130 */     } catch (CloneNotSupportedException cantHappen) {
/* 1131 */       throw new InternalError();
/*      */     } 
/* 1133 */     c.key = (char[])this.key.clone();
/* 1134 */     c.containsNull = this.containsNull;
/* 1135 */     c.link = (long[])this.link.clone();
/* 1136 */     return c;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */ 
/*      */   
/*      */   public int hashCode() {
/* 1149 */     int h = 0;
/* 1150 */     for (int j = realSize(), i = 0; j-- != 0; ) {
/* 1151 */       while (this.key[i] == '\000')
/* 1152 */         i++; 
/* 1153 */       h += this.key[i];
/* 1154 */       i++;
/*      */     } 
/*      */     
/* 1157 */     return h;
/*      */   }
/*      */   private void writeObject(ObjectOutputStream s) throws IOException {
/* 1160 */     CharIterator i = iterator();
/* 1161 */     s.defaultWriteObject();
/* 1162 */     for (int j = this.size; j-- != 0;)
/* 1163 */       s.writeChar(i.nextChar()); 
/*      */   }
/*      */   
/*      */   private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
/* 1167 */     s.defaultReadObject();
/* 1168 */     this.n = HashCommon.arraySize(this.size, this.f);
/* 1169 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/* 1170 */     this.mask = this.n - 1;
/* 1171 */     char[] key = this.key = new char[this.n + 1];
/* 1172 */     long[] link = this.link = new long[this.n + 1];
/* 1173 */     int prev = -1;
/* 1174 */     this.first = this.last = -1;
/*      */     
/* 1176 */     for (int i = this.size; i-- != 0; ) {
/* 1177 */       int pos; char k = s.readChar();
/* 1178 */       if (k == '\000') {
/* 1179 */         pos = this.n;
/* 1180 */         this.containsNull = true;
/*      */       }
/* 1182 */       else if (key[pos = HashCommon.mix(k) & this.mask] != '\000') {
/* 1183 */         while (key[pos = pos + 1 & this.mask] != '\000');
/*      */       } 
/* 1185 */       key[pos] = k;
/* 1186 */       if (this.first != -1) {
/* 1187 */         link[prev] = link[prev] ^ (link[prev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1188 */         link[pos] = link[pos] ^ (link[pos] ^ (prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1189 */         prev = pos; continue;
/*      */       } 
/* 1191 */       prev = this.first = pos;
/*      */       
/* 1193 */       link[pos] = link[pos] | 0xFFFFFFFF00000000L;
/*      */     } 
/*      */     
/* 1196 */     this.last = prev;
/* 1197 */     if (prev != -1)
/*      */     {
/* 1199 */       link[prev] = link[prev] | 0xFFFFFFFFL;
/*      */     }
/*      */   }
/*      */   
/*      */   private void checkTable() {}
/*      */ }


/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\chars\CharLinkedOpenHashSet.class
 * Java compiler version: 8 (52.0)
 * JD-Core Version:       1.1.3
 */