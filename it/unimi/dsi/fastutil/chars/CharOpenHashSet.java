...
/* 598 */     char[] key = this.key;
/* 599 */     int mask = newN - 1;
/* 600 */     char[] newKey = new char[newN + 1];
/* 601 */     int i = this.n;
/* 602 */     for (int j = realSize(); j-- != 0; ) {
/* 603 */       while (key[--i] == '\000'); int pos;
/* 604 */       if (newKey[pos = HashCommon.mix(key[i]) & mask] != '\000')
/* 605 */         while (newKey[pos = pos + 1 & mask] != '\000'); 
...
/* 658 */     CharIterator i = iterator();
/* 659 */     s.defaultWriteObject();
/* 660 */     for (int j = this.size; j-- != 0;)
/* 661 */       s.writeChar(i.nextChar()); 
/*     */   }
/*     */   
...
/* 669 */     char[] key = this.key = new char[this.n + 1];
/*     */     
/* 671 */     for (int i = this.size; i-- != 0; ) {
/* 672 */       int pos; char k = s.readChar();
/* 673 */       if (k == '\000') {
/* 674 */         pos = this.n;
/* 675 */         this.containsNull = true;
/*     */       }
/* 677 */       else if (key[pos = HashCommon.mix(k) & this.mask] != '\000') {
/* 678 */         while (key[pos = pos + 1 & this.mask] != '\000');
/*     */       } 
/* 680 */       key[pos] = k;
/*     */     } 
/*     */   }
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\chars\CharOpenHashSet.class
