...
/*  109 */     char t = array[BigArrays.segment(first)][BigArrays.displacement(first)];
...
/*  125 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (char)(array[BigArrays.segment(index)][BigArrays.displacement(index)] + incr);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  139 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (char)(array[BigArrays.segment(index)][BigArrays.displacement(index)] * factor);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  150 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (char)(array[BigArrays.segment(index)][BigArrays.displacement(index)] + 1);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  161 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = (char)(array[BigArrays.segment(index)][BigArrays.displacement(index)] - 1);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  309 */     char[][] base = new char[baseLength][];
/*  310 */     int residual = (int)(length & 0x7FFFFFFL);
/*  311 */     if (residual != 0) {
/*  312 */       for (int i = 0; i < baseLength - 1; i++)
/*  313 */         base[i] = new char[134217728]; 
/*  314 */       base[baseLength - 1] = new char[residual];
/*      */     } else {
/*  316 */       for (int i = 0; i < baseLength; i++)
/*  317 */         base[i] = new char[134217728]; 
/*  318 */     }  return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  335 */       return new char[][] { array }; 
/*  336 */     char[][] bigArray = newBigArray(array.length);
...
/*  388 */     char[][] base = Arrays.<char[]>copyOf(array, baseLength);
/*  389 */     int residual = (int)(length & 0x7FFFFFFL);
/*  390 */     if (residual != 0) {
/*  391 */       for (int i = valid; i < baseLength - 1; i++)
/*  392 */         base[i] = new char[134217728]; 
/*  393 */       base[baseLength - 1] = new char[residual];
/*      */     } else {
/*  395 */       for (int i = valid; i < baseLength; i++)
/*  396 */         base[i] = new char[134217728]; 
...
/*  507 */     char[][] base = Arrays.<char[]>copyOf(array, baseLength);
/*  508 */     int residual = (int)(length & 0x7FFFFFFL);
/*  509 */     if (residual != 0)
/*  510 */       base[baseLength - 1] = CharArrays.trim(base[baseLength - 1], residual); 
/*  511 */     return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  567 */     char[][] base = (char[][])array.clone();
/*  568 */     for (int i = base.length; i-- != 0;)
/*  569 */       base[i] = (char[])array[i].clone(); 
/*  570 */     return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  733 */       return CharBigArrays.equals(a, b);
/*      */     } }
/*      */ 
/*      */ 
/*      */   
...
/*  839 */     int ab = Character.compare(get(x, a), get(x, b));
/*  840 */     int ac = Character.compare(get(x, a), get(x, c));
/*  841 */     int bc = Character.compare(get(x, b), get(x, c));
...
/*      */     
/*      */     int comparison;
/*  915 */     while (b <= c && (comparison = Character.compare(get(x, b), v)) <= 0) {
/*  916 */       if (comparison == 0)
/*  917 */         swap(x, a++, b); 
/*  918 */       b++;
/*      */     } 
/*  920 */     while (c >= b && (comparison = Character.compare(get(x, c), v)) >= 0) {
/*  921 */       if (comparison == 0)
/*  922 */         swap(x, c, d--); 
/*  923 */       c--;
/*      */     } 
...
/* 1181 */             (byte)(get(a, first + i) >>> shift & 0xFF ^ 0x0)); 
...
/* 1341 */             (byte)(get(k, first + i) >>> shift & 0xFF ^ 0x0)); 
...
/* 1364 */         char t = get(a, l1 + first);
/* 1365 */         char u = get(b, l1 + first);
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\chars\CharBigArrays.class
