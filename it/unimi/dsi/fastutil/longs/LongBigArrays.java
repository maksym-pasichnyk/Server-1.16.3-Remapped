...
/*      */ {
/*   63 */   public static final long[][] EMPTY_BIG_ARRAY = new long[0][];
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*   72 */   public static final long[][] DEFAULT_EMPTY_BIG_ARRAY = new long[0][];
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*  150 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = array[BigArrays.segment(index)][BigArrays.displacement(index)] + 1L;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  161 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] = array[BigArrays.segment(index)][BigArrays.displacement(index)] - 1L;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  309 */     long[][] base = new long[baseLength][];
/*  310 */     int residual = (int)(length & 0x7FFFFFFL);
/*  311 */     if (residual != 0) {
/*  312 */       for (int i = 0; i < baseLength - 1; i++)
/*  313 */         base[i] = new long[134217728]; 
/*  314 */       base[baseLength - 1] = new long[residual];
/*      */     } else {
/*  316 */       for (int i = 0; i < baseLength; i++)
/*  317 */         base[i] = new long[134217728]; 
/*  318 */     }  return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  335 */       return new long[][] { array }; 
/*  336 */     long[][] bigArray = newBigArray(array.length);
...
/*  388 */     long[][] base = Arrays.<long[]>copyOf(array, baseLength);
/*  389 */     int residual = (int)(length & 0x7FFFFFFL);
/*  390 */     if (residual != 0) {
/*  391 */       for (int i = valid; i < baseLength - 1; i++)
/*  392 */         base[i] = new long[134217728]; 
/*  393 */       base[baseLength - 1] = new long[residual];
/*      */     } else {
/*  395 */       for (int i = valid; i < baseLength; i++)
/*  396 */         base[i] = new long[134217728]; 
...
/*  507 */     long[][] base = Arrays.<long[]>copyOf(array, baseLength);
/*  508 */     int residual = (int)(length & 0x7FFFFFFL);
/*  509 */     if (residual != 0)
/*  510 */       base[baseLength - 1] = LongArrays.trim(base[baseLength - 1], residual); 
/*  511 */     return base;
/*      */   }
/*      */ 
/*      */ 
...
/*  567 */     long[][] base = (long[][])array.clone();
/*  568 */     for (int i = base.length; i-- != 0;)
/*  569 */       base[i] = (long[])array[i].clone(); 
/*  570 */     return base;
/*      */   }
/*      */ 
/*      */ 
...
/*      */   }
/*      */   private static final class BigArrayHashStrategy implements Hash.Strategy<long[][]>, Serializable { private static final long serialVersionUID = -7046029254386353129L;
/*      */     
...
/*  733 */       return LongBigArrays.equals(a, b);
/*      */     } }
/*      */ 
/*      */ 
/*      */   
...
/*      */       return;
/*      */     } 
/*      */     swap(x, b++, c--); } private static long med3(long[][] x, long a, long b, long c) {
/*  839 */     int ab = Long.compare(get(x, a), get(x, b));
/*  840 */     int ac = Long.compare(get(x, a), get(x, c));
/*  841 */     int bc = Long.compare(get(x, b), get(x, c));
...
/*      */     
/*      */     int comparison;
/*  915 */     while (b <= c && (comparison = Long.compare(get(x, b), v)) <= 0) {
/*  916 */       if (comparison == 0)
/*  917 */         swap(x, a++, b); 
/*  918 */       b++;
/*      */     } 
/*  920 */     while (c >= b && (comparison = Long.compare(get(x, c), v)) >= 0) {
/*  921 */       if (comparison == 0)
/*  922 */         swap(x, c, d--); 
/*  923 */       c--;
/*      */     } 
...
/* 1151 */     int maxLevel = 7;
/* 1152 */     int stackSize = 1786;
/* 1153 */     long[] offsetStack = new long[1786];
/* 1154 */     int offsetPos = 0;
/* 1155 */     long[] lengthStack = new long[1786];
/* 1156 */     int lengthPos = 0;
/* 1157 */     int[] levelStack = new int[1786];
/* 1158 */     int levelPos = 0;
...
/* 1169 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 1170 */       if (length < 40L) {
/* 1171 */         selectionSort(a, first, first + length);
/*      */         continue;
/*      */       } 
/* 1174 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */       
/*      */       long i;
/*      */       
...
/* 1181 */             (byte)(int)(get(a, first + i) >>> shift & 0xFFL ^ signMask)); 
...
/* 1310 */     int maxLevel = 15;
/* 1311 */     int stackSize = 3826;
/* 1312 */     long[] offsetStack = new long[3826];
/* 1313 */     int offsetPos = 0;
/* 1314 */     long[] lengthStack = new long[3826];
/* 1315 */     int lengthPos = 0;
/* 1316 */     int[] levelStack = new int[3826];
/* 1317 */     int levelPos = 0;
...
/* 1328 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 1329 */       if (length < 40L) {
/* 1330 */         selectionSort(a, b, first, first + length);
/*      */         continue;
/*      */       } 
/* 1333 */       long[][] k = (level < 8) ? a : b;
/* 1334 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */       
/*      */       long i;
/*      */       
...
/* 1341 */             (byte)(int)(get(k, first + i) >>> shift & 0xFFL ^ signMask)); 
...
/* 1364 */         long t = get(a, l1 + first);
/* 1365 */         long u = get(b, l1 + first);
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\longs\LongBigArrays.class
