...
/*   69 */     this.actualComparator = LongComparators.asLongComparator(this.storedComparator);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  142 */     this(LongIterators.asLongIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  158 */     this(c);
/*  159 */     LongArrays.ensureOffsetLength(a, offset, length);
...
/*  174 */     this(a, offset, length, (Comparator<? super Long>)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  229 */     return (this.actualComparator == null) ? Long.compare(k1, k2) : this.actualComparator.compare(k1, k2);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     }
/*      */     public int hashCode() {
/*  910 */       return HashCommon.long2int(this.key);
/*      */     }
...
/*  980 */     int index = 0;
/*      */     SetIterator() {
/*  982 */       this.next = LongAVLTreeSet.this.firstEntry;
/*      */     }
/*      */     SetIterator(long k) {
/*  985 */       if ((this.next = LongAVLTreeSet.this.locateKey(k)) != null)
/*  986 */         if (LongAVLTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1050 */       updatePrevious();
/* 1051 */       updateNext();
/* 1052 */       LongAVLTreeSet.this.remove(this.curr.key);
/* 1053 */       this.curr = null;
/*      */     }
/*      */   }
/*      */   
...
/* 1070 */     return new Subset(0L, true, to, false);
/*      */   }
/*      */   
/*      */   public LongSortedSet tailSet(long from) {
/* 1074 */     return new Subset(from, false, 0L, true);
/*      */   }
/*      */   
...
/* 1116 */       if (!bottom && !top && LongAVLTreeSet.this.compare(from, to) > 0) {
...
/* 1140 */       return ((this.bottom || LongAVLTreeSet.this.compare(k, this.from) >= 0) && (this.top || LongAVLTreeSet.this
/* 1141 */         .compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean contains(long k) {
/* 1146 */       return (in(k) && LongAVLTreeSet.this.contains(k));
/*      */     }
/*      */     
/*      */     public boolean add(long k) {
/* 1150 */       if (!in(k))
...
/* 1153 */       return LongAVLTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean remove(long k) {
/* 1158 */       if (!in(k))
/* 1159 */         return false; 
/* 1160 */       return LongAVLTreeSet.this.remove(k);
/*      */     }
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public LongComparator comparator() {
/* 1178 */       return LongAVLTreeSet.this.actualComparator;
/*      */     }
/*      */     
/*      */     public LongBidirectionalIterator iterator() {
...
/* 1192 */       return (LongAVLTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */     
/*      */     public LongSortedSet tailSet(long from) {
/* 1196 */       if (this.bottom)
/* 1197 */         return new Subset(from, false, this.to, this.top); 
/* 1198 */       return (LongAVLTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */     
/*      */     public LongSortedSet subSet(long from, long to) {
...
/* 1204 */       if (!this.top)
/* 1205 */         to = (LongAVLTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1206 */       if (!this.bottom)
/* 1207 */         from = (LongAVLTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1219 */       if (LongAVLTreeSet.this.tree == null) {
/* 1220 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1224 */       if (this.bottom) {
/* 1225 */         e = LongAVLTreeSet.this.firstEntry;
/*      */       } else {
/* 1227 */         e = LongAVLTreeSet.this.locateKey(this.from);
/*      */         
/* 1229 */         if (LongAVLTreeSet.this.compare(e.key, this.from) < 0) {
/* 1230 */           e = e.next();
/*      */         }
/*      */       } 
/*      */       
/* 1234 */       if (e == null || (!this.top && LongAVLTreeSet.this.compare(e.key, this.to) >= 0))
/* 1235 */         return null; 
/* 1236 */       return e;
/*      */     }
/*      */ 
...
/*      */       LongAVLTreeSet.Entry e;
/* 1245 */       if (LongAVLTreeSet.this.tree == null) {
/* 1246 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1250 */       if (this.top) {
/* 1251 */         e = LongAVLTreeSet.this.lastEntry;
/*      */       } else {
/* 1253 */         e = LongAVLTreeSet.this.locateKey(this.to);
/*      */         
/* 1255 */         if (LongAVLTreeSet.this.compare(e.key, this.to) >= 0) {
/* 1256 */           e = e.prev();
/*      */         }
/*      */       } 
/*      */       
/* 1260 */       if (e == null || (!this.bottom && LongAVLTreeSet.this.compare(e.key, this.from) < 0))
/* 1261 */         return null; 
/* 1262 */       return e;
/*      */     }
/*      */     
/*      */     public long firstLong() {
/* 1266 */       LongAVLTreeSet.Entry e = firstEntry();
/* 1267 */       if (e == null)
...
/*      */     
/*      */     public long lastLong() {
/* 1273 */       LongAVLTreeSet.Entry e = lastEntry();
/* 1274 */       if (e == null)
...
/*      */     {
/*      */       SubsetIterator() {
/* 1289 */         this.next = LongAVLTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       SubsetIterator(long k) {
/* 1292 */         this();
/* 1293 */         if (this.next != null)
/* 1294 */           if (!LongAVLTreeSet.Subset.this.bottom && LongAVLTreeSet.this.compare(k, this.next.key) < 0) {
/* 1295 */             this.prev = null;
/* 1296 */           } else if (!LongAVLTreeSet.Subset.this.top && LongAVLTreeSet.this.compare(k, (this.prev = LongAVLTreeSet.Subset.this.lastEntry()).key) >= 0) {
/* 1297 */             this.next = null;
/*      */           } else {
/* 1299 */             this.next = LongAVLTreeSet.this.locateKey(k);
/* 1300 */             if (LongAVLTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1311 */         if (!LongAVLTreeSet.Subset.this.bottom && this.prev != null && LongAVLTreeSet.this.compare(this.prev.key, LongAVLTreeSet.Subset.this.from) < 0)
/* 1312 */           this.prev = null; 
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1316 */         this.next = this.next.next();
/* 1317 */         if (!LongAVLTreeSet.Subset.this.top && this.next != null && LongAVLTreeSet.this.compare(this.next.key, LongAVLTreeSet.Subset.this.to) >= 0) {
/* 1318 */           this.next = null;
/*      */         }
/*      */       }
/*      */     }
...
/*      */     LongAVLTreeSet c;
/*      */     try {
/* 1337 */       c = (LongAVLTreeSet)super.clone();
...
/* 1411 */     if (n == 1) {
/* 1412 */       Entry entry = new Entry(s.readLong());
...
/*      */ 
/*      */       
/* 1422 */       Entry entry = new Entry(s.readLong());
/* 1423 */       entry.right(new Entry(s.readLong()));
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\longs\LongAVLTreeSet.class
