...
/* 598 */     long[] key = this.key;
/* 599 */     int mask = newN - 1;
/* 600 */     long[] newKey = new long[newN + 1];
/* 601 */     int i = this.n;
/* 602 */     for (int j = realSize(); j-- != 0; ) {
/* 603 */       while (key[--i] == 0L); int pos;
/* 604 */       if (newKey[pos = (int)HashCommon.mix(key[i]) & mask] != 0L)
/* 605 */         while (newKey[pos = pos + 1 & mask] != 0L); 
...
/* 649 */       while (this.key[i] == 0L)
/* 650 */         i++; 
/* 651 */       h += HashCommon.long2int(this.key[i]);
/* 652 */       i++;
/*     */     } 
/*     */     
/* 655 */     return h;
...
/* 658 */     LongIterator i = iterator();
/* 659 */     s.defaultWriteObject();
/* 660 */     for (int j = this.size; j-- != 0;)
/* 661 */       s.writeLong(i.nextLong()); 
/*     */   }
/*     */   
...
/* 669 */     long[] key = this.key = new long[this.n + 1];
/*     */     
/* 671 */     for (int i = this.size; i-- != 0; ) {
/* 672 */       int pos; long k = s.readLong();
/* 673 */       if (k == 0L) {
/* 674 */         pos = this.n;
/* 675 */         this.containsNull = true;
/*     */       }
/* 677 */       else if (key[pos = (int)HashCommon.mix(k) & this.mask] != 0L) {
/* 678 */         while (key[pos = pos + 1 & this.mask] != 0L);
/*     */       } 
/* 680 */       key[pos] = k;
/*     */     } 
/*     */   }
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\longs\LongOpenHashSet.class
