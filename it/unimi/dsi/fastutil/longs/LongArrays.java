...
/*      */ {
/*   97 */   public static final long[] EMPTY_ARRAY = new long[0];
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*  106 */   public static final long[] DEFAULT_EMPTY_ARRAY = new long[0];
/*      */   
...
/*  228 */       return array; 
/*  229 */     long[] t = (length == 0) ? EMPTY_ARRAY : new long[length];
...
/*  269 */     long[] a = (length == 0) ? EMPTY_ARRAY : new long[length];
...
/*  582 */       if (len < 8192) {
/*  583 */         LongArrays.quickSort(x, this.from, this.to, this.comp);
/*      */         
/*      */         return;
/*      */       } 
...
/*  589 */       int n = this.to - 1;
/*  590 */       int s = len / 8;
/*  591 */       l = LongArrays.med3(x, l, l + s, l + 2 * s, this.comp);
/*  592 */       m = LongArrays.med3(x, m - s, m, m + s, this.comp);
/*  593 */       n = LongArrays.med3(x, n - 2 * s, n - s, n, this.comp);
/*  594 */       m = LongArrays.med3(x, l, m, n, this.comp);
/*  595 */       long v = x[m];
/*      */       
...
/*  601 */           if (comparison == 0)
/*  602 */             LongArrays.swap(x, a++, b); 
/*  603 */           b++; continue;
/*      */         } 
/*  605 */         while (c >= b && (comparison = this.comp.compare(x[c], v)) >= 0) {
/*  606 */           if (comparison == 0)
/*  607 */             LongArrays.swap(x, c, d--); 
/*  608 */           c--;
/*      */         } 
/*  610 */         if (b > c)
/*      */           break; 
/*  612 */         LongArrays.swap(x, b++, c--);
/*      */       } 
/*      */ 
/*      */       
/*  616 */       s = Math.min(a - this.from, b - a);
/*  617 */       LongArrays.swap(x, this.from, b - s, s);
/*  618 */       s = Math.min(d - c, this.to - d - 1);
/*  619 */       LongArrays.swap(x, b, this.to - s, s);
/*      */       
/*  621 */       s = b - a;
/*  622 */       int t = d - c;
/*  623 */       if (s > 1 && t > 1) {
...
/*  686 */     int ab = Long.compare(x[a], x[b]);
/*  687 */     int ac = Long.compare(x[a], x[c]);
/*  688 */     int bc = Long.compare(x[b], x[c]);
...
/*      */     while (true) {
/*      */       int comparison;
/*  765 */       if (b <= c && (comparison = Long.compare(x[b], v)) <= 0) {
/*  766 */         if (comparison == 0)
/*  767 */           swap(x, a++, b); 
/*  768 */         b++; continue;
/*      */       } 
/*  770 */       while (c >= b && (comparison = Long.compare(x[c], v)) >= 0) {
/*  771 */         if (comparison == 0)
/*  772 */           swap(x, c, d--); 
/*  773 */         c--;
/*      */       } 
...
/*  826 */       if (len < 8192) {
/*  827 */         LongArrays.quickSort(x, this.from, this.to);
/*      */         
/*      */         return;
/*      */       } 
...
/*  833 */       int n = this.to - 1;
/*  834 */       int s = len / 8;
/*  835 */       l = LongArrays.med3(x, l, l + s, l + 2 * s);
/*  836 */       m = LongArrays.med3(x, m - s, m, m + s);
/*  837 */       n = LongArrays.med3(x, n - 2 * s, n - s, n);
/*  838 */       m = LongArrays.med3(x, l, m, n);
/*  839 */       long v = x[m];
/*      */       
/*  841 */       int a = this.from, b = a, c = this.to - 1, d = c;
/*      */       while (true) {
/*      */         int comparison;
/*  844 */         if (b <= c && (comparison = Long.compare(x[b], v)) <= 0) {
/*  845 */           if (comparison == 0)
/*  846 */             LongArrays.swap(x, a++, b); 
/*  847 */           b++; continue;
/*      */         } 
/*  849 */         while (c >= b && (comparison = Long.compare(x[c], v)) >= 0) {
/*  850 */           if (comparison == 0)
/*  851 */             LongArrays.swap(x, c, d--); 
/*  852 */           c--;
/*      */         } 
/*  854 */         if (b > c)
/*      */           break; 
/*  856 */         LongArrays.swap(x, b++, c--);
/*      */       } 
/*      */ 
/*      */       
/*  860 */       s = Math.min(a - this.from, b - a);
/*  861 */       LongArrays.swap(x, this.from, b - s, s);
/*  862 */       s = Math.min(d - c, this.to - d - 1);
/*  863 */       LongArrays.swap(x, b, this.to - s, s);
/*      */       
/*  865 */       s = b - a;
/*  866 */       int t = d - c;
/*  867 */       if (s > 1 && t > 1) {
...
/*  926 */     long aa = x[perm[a]];
/*  927 */     long bb = x[perm[b]];
/*  928 */     long cc = x[perm[c]];
/*  929 */     int ab = Long.compare(aa, bb);
/*  930 */     int ac = Long.compare(aa, cc);
/*  931 */     int bc = Long.compare(bb, cc);
...
/*      */     while (true) {
/*      */       int comparison;
/* 1001 */       if (b <= c && (comparison = Long.compare(x[perm[b]], v)) <= 0) {
/* 1002 */         if (comparison == 0)
/* 1003 */           IntArrays.swap(perm, a++, b); 
/* 1004 */         b++; continue;
/*      */       } 
/* 1006 */       while (c >= b && (comparison = Long.compare(x[perm[c]], v)) >= 0) {
...
/* 1071 */       if (len < 8192) {
/* 1072 */         LongArrays.quickSortIndirect(this.perm, x, this.from, this.to);
/*      */         
/*      */         return;
/*      */       } 
...
/* 1079 */       int s = len / 8;
/* 1080 */       l = LongArrays.med3Indirect(this.perm, x, l, l + s, l + 2 * s);
/* 1081 */       m = LongArrays.med3Indirect(this.perm, x, m - s, m, m + s);
/* 1082 */       n = LongArrays.med3Indirect(this.perm, x, n - 2 * s, n - s, n);
/* 1083 */       m = LongArrays.med3Indirect(this.perm, x, l, m, n);
/* 1084 */       long v = x[this.perm[m]];
/*      */       
/* 1086 */       int a = this.from, b = a, c = this.to - 1, d = c;
/*      */       while (true) {
/*      */         int comparison;
/* 1089 */         if (b <= c && (comparison = Long.compare(x[this.perm[b]], v)) <= 0) {
/* 1090 */           if (comparison == 0)
/* 1091 */             IntArrays.swap(this.perm, a++, b); 
/* 1092 */           b++; continue;
/*      */         } 
/* 1094 */         while (c >= b && (comparison = Long.compare(x[this.perm[c]], v)) >= 0) {
...
/* 1258 */     int t, ab = ((t = Long.compare(x[a], x[b])) == 0) ? Long.compare(y[a], y[b]) : t;
/* 1259 */     int ac = ((t = Long.compare(x[a], x[c])) == 0) ? Long.compare(y[a], y[c]) : t;
/* 1260 */     int bc = ((t = Long.compare(x[b], x[c])) == 0) ? Long.compare(y[b], y[c]) : t;
...
/* 1280 */         int u; if ((u = Long.compare(a[j], a[m])) < 0 || (u == 0 && b[j] < b[m]))
/* 1281 */           m = j; 
/* 1282 */       }  if (m != i) {
/* 1283 */         long t = a[i];
/* 1284 */         a[i] = a[m];
...
/* 1335 */     long v = x[m], w = y[m];
/*      */     
/* 1337 */     int a = from, b = a, c = to - 1, d = c;
/*      */     
/*      */     while (true) {
/* 1340 */       if (b <= c) {
/* 1341 */         int comparison; int t; if ((comparison = ((t = Long.compare(x[b], v)) == 0) ? Long.compare(y[b], w) : t) <= 0) {
/* 1342 */           if (comparison == 0)
/* 1343 */             swap(x, y, a++, b); 
/* 1344 */           b++; continue;
/*      */         } 
/* 1346 */       }  while (c >= b) {
/* 1347 */         int comparison; int t; if ((comparison = ((t = Long.compare(x[c], v)) == 0) ? Long.compare(y[c], w) : t) >= 0) {
...
/* 1410 */       if (len < 8192) {
/* 1411 */         LongArrays.quickSort(x, y, this.from, this.to);
/*      */         
/*      */         return;
/*      */       } 
...
/* 1418 */       int s = len / 8;
/* 1419 */       l = LongArrays.med3(x, y, l, l + s, l + 2 * s);
/* 1420 */       m = LongArrays.med3(x, y, m - s, m, m + s);
/* 1421 */       n = LongArrays.med3(x, y, n - 2 * s, n - s, n);
/* 1422 */       m = LongArrays.med3(x, y, l, m, n);
/* 1423 */       long v = x[m], w = y[m];
/*      */       
...
/* 1430 */           if ((comparison = ((i = Long.compare(x[b], v)) == 0) ? Long.compare(y[b], w) : i) <= 0) {
/* 1431 */             if (comparison == 0)
/* 1432 */               LongArrays.swap(x, y, a++, b); 
/* 1433 */             b++; continue;
/*      */           } 
/* 1435 */         }  while (c >= b) {
/*      */           int comparison; int i;
/* 1437 */           if ((comparison = ((i = Long.compare(x[c], v)) == 0) ? Long.compare(y[c], w) : i) >= 0) {
/* 1438 */             if (comparison == 0)
/* 1439 */               LongArrays.swap(x, y, c, d--); 
/* 1440 */             c--;
/*      */           } 
/* 1442 */         }  if (b > c)
/*      */           break; 
/* 1444 */         LongArrays.swap(x, y, b++, c--);
/*      */       } 
/*      */ 
/*      */       
/* 1448 */       s = Math.min(a - this.from, b - a);
/* 1449 */       LongArrays.swap(x, y, this.from, b - s, s);
/* 1450 */       s = Math.min(d - c, this.to - d - 1);
/* 1451 */       LongArrays.swap(x, y, b, this.to - s, s);
/* 1452 */       s = b - a;
/* 1453 */       int t = d - c;
/*      */       
/* 1455 */       if (s > 1 && t > 1) {
...
/* 1591 */     mergeSort(a, from, to, (long[])a.clone());
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/* 1673 */     mergeSort(a, from, to, comp, (long[])a.clone());
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     } 
/* 1877 */     int maxLevel = 7;
/* 1878 */     int stackSize = 1786;
/* 1879 */     int stackPos = 0;
/* 1880 */     int[] offsetStack = new int[1786];
/* 1881 */     int[] lengthStack = new int[1786];
/* 1882 */     int[] levelStack = new int[1786];
...
/* 1892 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 1893 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 1898 */       for (int i = first + length; i-- != first;) {
/* 1899 */         count[(int)(a[i] >>> shift & 0xFFL ^ signMask)] = count[(int)(a[i] >>> shift & 0xFFL ^ signMask)] + 1;
/*      */       }
/* 1901 */       int lastUsed = -1;
...
/* 1910 */         long t = a[k];
/* 1911 */         c = (int)(t >>> shift & 0xFFL ^ signMask);
/* 1912 */         if (k < end) {
/* 1913 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > k; ) {
/* 1914 */             long z = t;
/* 1915 */             t = a[d];
/* 1916 */             a[d] = z;
/* 1917 */             c = (int)(t >>> shift & 0xFFL ^ signMask);
/*      */           } 
/* 1919 */           a[k] = t;
/*      */         } 
/* 1921 */         if (level < 7 && count[c] > 1)
...
/*      */     } 
/* 2129 */     int maxLevel = 7;
/* 2130 */     int stackSize = 1786;
/* 2131 */     int stackPos = 0;
/* 2132 */     int[] offsetStack = new int[1786];
/* 2133 */     int[] lengthStack = new int[1786];
/* 2134 */     int[] levelStack = new int[1786];
...
/* 2145 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 2146 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2151 */       for (int i = first + length; i-- != first;) {
/* 2152 */         count[(int)(a[perm[i]] >>> shift & 0xFFL ^ signMask)] = count[(int)(a[perm[i]] >>> shift & 0xFFL ^ signMask)] + 1;
/*      */       }
...
/* 2162 */           pos[(int)(a[perm[j]] >>> shift & 0xFFL ^ signMask)] = pos[(int)(a[perm[j]] >>> shift & 0xFFL ^ signMask)] - 1; support[pos[(int)(a[perm[j]] >>> shift & 0xFFL ^ signMask)] - 1] = perm[j];
/* 2163 */         }  System.arraycopy(support, 0, perm, first, length);
/* 2164 */         for (j = 0, p = first; j <= lastUsed; j++) {
/* 2165 */           if (level < 7 && count[j] > 1) {
...
/* 2181 */         int t = perm[k];
/* 2182 */         c = (int)(a[t] >>> shift & 0xFFL ^ signMask);
/* 2183 */         if (k < end) {
/* 2184 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > k; ) {
/* 2185 */             int z = t;
/* 2186 */             t = perm[d];
/* 2187 */             perm[d] = z;
/* 2188 */             c = (int)(a[t] >>> shift & 0xFFL ^ signMask);
/*      */           } 
/* 2190 */           perm[k] = t;
/*      */         } 
/* 2192 */         if (level < 7 && count[c] > 1) {
...
/*      */     } 
/* 2415 */     int layers = 2;
/* 2416 */     int maxLevel = 15;
/* 2417 */     int stackSize = 3826;
/* 2418 */     int stackPos = 0;
/* 2419 */     int[] offsetStack = new int[3826];
/* 2420 */     int[] lengthStack = new int[3826];
/* 2421 */     int[] levelStack = new int[3826];
...
/* 2431 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 2432 */       long[] k = (level < 8) ? a : b;
/* 2433 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2438 */       for (int i = first + length; i-- != first;) {
/* 2439 */         count[(int)(k[i] >>> shift & 0xFFL ^ signMask)] = count[(int)(k[i] >>> shift & 0xFFL ^ signMask)] + 1;
/*      */       }
/* 2441 */       int lastUsed = -1;
...
/* 2450 */         long t = a[m];
/* 2451 */         long u = b[m];
/* 2452 */         c = (int)(k[m] >>> shift & 0xFFL ^ signMask);
/* 2453 */         if (m < end) {
/* 2454 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > m; ) {
/* 2455 */             c = (int)(k[d] >>> shift & 0xFFL ^ signMask);
/* 2456 */             long z = t;
/* 2457 */             t = a[d];
/* 2458 */             a[d] = z;
/* 2459 */             z = u;
...
/*      */     } 
/* 2707 */     int layers = 2;
/* 2708 */     int maxLevel = 15;
/* 2709 */     int stackSize = 3826;
/* 2710 */     int stackPos = 0;
/* 2711 */     int[] offsetStack = new int[3826];
/* 2712 */     int[] lengthStack = new int[3826];
/* 2713 */     int[] levelStack = new int[3826];
...
/* 2724 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 2725 */       long[] k = (level < 8) ? a : b;
/* 2726 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2731 */       for (int i = first + length; i-- != first;) {
/* 2732 */         count[(int)(k[perm[i]] >>> shift & 0xFFL ^ signMask)] = count[(int)(k[perm[i]] >>> shift & 0xFFL ^ signMask)] + 1;
/*      */       }
...
/* 2742 */           pos[(int)(k[perm[j]] >>> shift & 0xFFL ^ signMask)] = pos[(int)(k[perm[j]] >>> shift & 0xFFL ^ signMask)] - 1; support[pos[(int)(k[perm[j]] >>> shift & 0xFFL ^ signMask)] - 1] = perm[j];
/* 2743 */         }  System.arraycopy(support, 0, perm, first, length);
/* 2744 */         for (j = 0, p = first; j < 256; j++) {
/* 2745 */           if (level < 15 && count[j] > 1) {
...
/* 2761 */         int t = perm[m];
/* 2762 */         c = (int)(k[t] >>> shift & 0xFFL ^ signMask);
/* 2763 */         if (m < end) {
/* 2764 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > m; ) {
/* 2765 */             int z = t;
/* 2766 */             t = perm[d];
/* 2767 */             perm[d] = z;
/* 2768 */             c = (int)(k[t] >>> shift & 0xFFL ^ signMask);
/*      */           } 
/* 2770 */           perm[m] = t;
/*      */         } 
/* 2772 */         if (level < 15 && count[c] > 1) {
...
/*      */     } 
/* 2860 */     int stackSize = 255 * (layers * 8 - 1) + 1;
/* 2861 */     int stackPos = 0;
...
/* 2875 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 2876 */       long[] k = a[level / 8];
/* 2877 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2882 */       for (int i = first + length; i-- != first;) {
/* 2883 */         count[(int)(k[i] >>> shift & 0xFFL ^ signMask)] = count[(int)(k[i] >>> shift & 0xFFL ^ signMask)] + 1;
/*      */       }
/* 2885 */       int lastUsed = -1;
...
/* 2895 */           t[i1] = a[i1][m]; 
/* 2896 */         c = (int)(k[m] >>> shift & 0xFFL ^ signMask);
/* 2897 */         if (m < end) {
/* 2898 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > m; ) {
/* 2899 */             c = (int)(k[d] >>> shift & 0xFFL ^ signMask);
...
/*      */ 
/*      */   
/* 3019 */   public static final Hash.Strategy<long[]> HASH_STRATEGY = new ArrayHashStrategy();
/*      */ }


/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\longs\LongArrays.class
