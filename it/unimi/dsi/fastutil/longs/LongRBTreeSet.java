...
/*   70 */     this.actualComparator = LongComparators.asLongComparator(this.storedComparator);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  143 */     this(LongIterators.asLongIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  159 */     this(c);
/*  160 */     LongArrays.ensureOffsetLength(a, offset, length);
...
/*  175 */     this(a, offset, length, (Comparator<? super Long>)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  230 */     return (this.actualComparator == null) ? Long.compare(k1, k2) : this.actualComparator.compare(k1, k2);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     }
/*      */     public int hashCode() {
/*  868 */       return HashCommon.long2int(this.key);
/*      */     }
...
/*  939 */     int index = 0;
/*      */     SetIterator() {
/*  941 */       this.next = LongRBTreeSet.this.firstEntry;
/*      */     }
/*      */     SetIterator(long k) {
/*  944 */       if ((this.next = LongRBTreeSet.this.locateKey(k)) != null)
/*  945 */         if (LongRBTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1009 */       updatePrevious();
/* 1010 */       updateNext();
/* 1011 */       LongRBTreeSet.this.remove(this.curr.key);
/* 1012 */       this.curr = null;
/*      */     }
/*      */   }
/*      */   
...
/* 1029 */     return new Subset(0L, true, to, false);
/*      */   }
/*      */   
/*      */   public LongSortedSet tailSet(long from) {
/* 1033 */     return new Subset(from, false, 0L, true);
/*      */   }
/*      */   
...
/* 1075 */       if (!bottom && !top && LongRBTreeSet.this.compare(from, to) > 0) {
...
/* 1099 */       return ((this.bottom || LongRBTreeSet.this.compare(k, this.from) >= 0) && (this.top || LongRBTreeSet.this
/* 1100 */         .compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean contains(long k) {
/* 1105 */       return (in(k) && LongRBTreeSet.this.contains(k));
/*      */     }
/*      */     
/*      */     public boolean add(long k) {
/* 1109 */       if (!in(k))
...
/* 1112 */       return LongRBTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean remove(long k) {
/* 1117 */       if (!in(k))
/* 1118 */         return false; 
/* 1119 */       return LongRBTreeSet.this.remove(k);
/*      */     }
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public LongComparator comparator() {
/* 1137 */       return LongRBTreeSet.this.actualComparator;
/*      */     }
/*      */     
/*      */     public LongBidirectionalIterator iterator() {
...
/* 1151 */       return (LongRBTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */     
/*      */     public LongSortedSet tailSet(long from) {
/* 1155 */       if (this.bottom)
/* 1156 */         return new Subset(from, false, this.to, this.top); 
/* 1157 */       return (LongRBTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */     
/*      */     public LongSortedSet subSet(long from, long to) {
...
/* 1163 */       if (!this.top)
/* 1164 */         to = (LongRBTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1165 */       if (!this.bottom)
/* 1166 */         from = (LongRBTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1178 */       if (LongRBTreeSet.this.tree == null) {
/* 1179 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1183 */       if (this.bottom) {
/* 1184 */         e = LongRBTreeSet.this.firstEntry;
/*      */       } else {
/* 1186 */         e = LongRBTreeSet.this.locateKey(this.from);
/*      */         
/* 1188 */         if (LongRBTreeSet.this.compare(e.key, this.from) < 0) {
/* 1189 */           e = e.next();
/*      */         }
/*      */       } 
/*      */       
/* 1193 */       if (e == null || (!this.top && LongRBTreeSet.this.compare(e.key, this.to) >= 0))
/* 1194 */         return null; 
/* 1195 */       return e;
/*      */     }
/*      */ 
...
/*      */       LongRBTreeSet.Entry e;
/* 1204 */       if (LongRBTreeSet.this.tree == null) {
/* 1205 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1209 */       if (this.top) {
/* 1210 */         e = LongRBTreeSet.this.lastEntry;
/*      */       } else {
/* 1212 */         e = LongRBTreeSet.this.locateKey(this.to);
/*      */         
/* 1214 */         if (LongRBTreeSet.this.compare(e.key, this.to) >= 0) {
/* 1215 */           e = e.prev();
/*      */         }
/*      */       } 
/*      */       
/* 1219 */       if (e == null || (!this.bottom && LongRBTreeSet.this.compare(e.key, this.from) < 0))
/* 1220 */         return null; 
/* 1221 */       return e;
/*      */     }
/*      */     
/*      */     public long firstLong() {
/* 1225 */       LongRBTreeSet.Entry e = firstEntry();
/* 1226 */       if (e == null)
...
/*      */     
/*      */     public long lastLong() {
/* 1232 */       LongRBTreeSet.Entry e = lastEntry();
/* 1233 */       if (e == null)
...
/*      */     {
/*      */       SubsetIterator() {
/* 1248 */         this.next = LongRBTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       SubsetIterator(long k) {
/* 1251 */         this();
/* 1252 */         if (this.next != null)
/* 1253 */           if (!LongRBTreeSet.Subset.this.bottom && LongRBTreeSet.this.compare(k, this.next.key) < 0) {
/* 1254 */             this.prev = null;
/* 1255 */           } else if (!LongRBTreeSet.Subset.this.top && LongRBTreeSet.this.compare(k, (this.prev = LongRBTreeSet.Subset.this.lastEntry()).key) >= 0) {
/* 1256 */             this.next = null;
/*      */           } else {
/* 1258 */             this.next = LongRBTreeSet.this.locateKey(k);
/* 1259 */             if (LongRBTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1270 */         if (!LongRBTreeSet.Subset.this.bottom && this.prev != null && LongRBTreeSet.this.compare(this.prev.key, LongRBTreeSet.Subset.this.from) < 0)
/* 1271 */           this.prev = null; 
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1275 */         this.next = this.next.next();
/* 1276 */         if (!LongRBTreeSet.Subset.this.top && this.next != null && LongRBTreeSet.this.compare(this.next.key, LongRBTreeSet.Subset.this.to) >= 0) {
/* 1277 */           this.next = null;
/*      */         }
/*      */       }
/*      */     }
...
/*      */     LongRBTreeSet c;
/*      */     try {
/* 1296 */       c = (LongRBTreeSet)super.clone();
...
/* 1370 */     if (n == 1) {
/* 1371 */       Entry entry = new Entry(s.readLong());
...
/*      */ 
/*      */       
/* 1382 */       Entry entry = new Entry(s.readLong());
/* 1383 */       entry.black(true);
/* 1384 */       entry.right(new Entry(s.readLong()));
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\longs\LongRBTreeSet.class
