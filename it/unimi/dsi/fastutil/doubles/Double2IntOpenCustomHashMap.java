...
/* 1186 */     double[] key = this.key;
/* 1187 */     int[] value = this.value;
/* 1188 */     int mask = newN - 1;
/* 1189 */     double[] newKey = new double[newN + 1];
/* 1190 */     int[] newValue = new int[newN + 1];
/* 1191 */     int i = this.n;
/* 1192 */     for (int j = realSize(); j-- != 0; ) {
/* 1193 */       while (Double.doubleToLongBits(key[--i]) == 0L); int pos;
/* 1194 */       if (Double.doubleToLongBits(newKey[
/* 1195 */             pos = HashCommon.mix(this.strategy.hashCode(key[i])) & mask]) != 0L)
/* 1196 */         while (Double.doubleToLongBits(newKey[pos = pos + 1 & mask]) != 0L); 
...
/*      */     try {
/* 1222 */       c = (Double2IntOpenCustomHashMap)super.clone();
...
/* 1230 */     c.key = (double[])this.key.clone();
/* 1231 */     c.value = (int[])this.value.clone();
...
/* 1248 */       while (Double.doubleToLongBits(this.key[i]) == 0L)
/* 1249 */         i++; 
...
/* 1277 */     double[] key = this.key = new double[this.n + 1];
/* 1278 */     int[] value = this.value = new int[this.n + 1];
/*      */ 
/*      */     
/* 1281 */     for (int i = this.size; i-- != 0; ) {
/* 1282 */       int pos; double k = s.readDouble();
/* 1283 */       int v = s.readInt();
/* 1284 */       if (this.strategy.equals(k, 0.0D)) {
/* 1285 */         pos = this.n;
/* 1286 */         this.containsNullKey = true;
/*      */       } else {
/* 1288 */         pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask;
/* 1289 */         while (Double.doubleToLongBits(key[pos]) != 0L)
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\doubles\Double2IntOpenCustomHashMap.class
