...
/*      */   
/*      */   public boolean trim() {
/* 1797 */     int l = HashCommon.arraySize(this.size, this.f);
/* 1798 */     if (l >= this.n || this.size > HashCommon.maxFill(l, this.f))
/* 1799 */       return true; 
/*      */     try {
/* 1801 */       rehash(l);
...
/* 1829 */     int l = HashCommon.nextPowerOfTwo((int)Math.ceil((n / this.f)));
/* 1830 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1831 */       return true; 
/*      */     try {
/* 1833 */       rehash(l);
/* 1834 */     } catch (OutOfMemoryError cantDoIt) {
/* 1835 */       return false;
/*      */     } 
/* 1837 */     return true;
/*      */   }
...
/* 1853 */     double[] key = this.key;
/* 1854 */     float[] value = this.value;
/* 1855 */     int mask = newN - 1;
/* 1856 */     double[] newKey = new double[newN + 1];
/* 1857 */     float[] newValue = new float[newN + 1];
...
/* 1863 */       int pos; if (Double.doubleToLongBits(key[i]) == 0L) {
/* 1864 */         pos = newN;
/*      */       } else {
/* 1866 */         pos = (int)HashCommon.mix(Double.doubleToRawLongBits(key[i])) & mask;
/* 1867 */         while (Double.doubleToLongBits(newKey[pos]) != 0L)
/* 1868 */           pos = pos + 1 & mask; 
/*      */       } 
/* 1870 */       newKey[pos] = key[i];
/* 1871 */       newValue[pos] = value[i];
/* 1872 */       if (prev != -1) {
/* 1873 */         newLink[newPrev] = newLink[newPrev] ^ (newLink[newPrev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1874 */         newLink[pos] = newLink[pos] ^ (newLink[pos] ^ (newPrev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1875 */         newPrev = pos;
/*      */       } else {
/* 1877 */         newPrev = this.first = pos;
/*      */         
/* 1879 */         newLink[pos] = -1L;
/*      */       } 
/* 1881 */       int t = i;
...
/* 1883 */       prev = t;
/*      */     } 
/* 1885 */     this.link = newLink;
/* 1886 */     this.last = newPrev;
/* 1887 */     if (newPrev != -1)
/*      */     {
/* 1889 */       newLink[newPrev] = newLink[newPrev] | 0xFFFFFFFFL; } 
/* 1890 */     this.n = newN;
/* 1891 */     this.mask = mask;
/* 1892 */     this.maxFill = HashCommon.maxFill(this.n, this.f);
/* 1893 */     this.key = newKey;
/* 1894 */     this.value = newValue;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     try {
/* 1911 */       c = (Double2FloatLinkedOpenHashMap)super.clone();
/* 1912 */     } catch (CloneNotSupportedException cantHappen) {
/* 1913 */       throw new InternalError();
/*      */     } 
/* 1915 */     c.keys = null;
/* 1916 */     c.values = null;
/* 1917 */     c.entries = null;
/* 1918 */     c.containsNullKey = this.containsNullKey;
/* 1919 */     c.key = (double[])this.key.clone();
/* 1920 */     c.value = (float[])this.value.clone();
/* 1921 */     c.link = (long[])this.link.clone();
/* 1922 */     return c;
/*      */   }
/*      */ 
/*      */ 
...
/* 1937 */       while (Double.doubleToLongBits(this.key[i]) == 0L)
/* 1938 */         i++; 
/* 1939 */       t = HashCommon.double2int(this.key[i]);
/* 1940 */       t ^= HashCommon.float2int(this.value[i]);
/* 1941 */       h += t;
/* 1942 */       i++;
/*      */     } 
/*      */     
/* 1945 */     if (this.containsNullKey)
/* 1946 */       h += HashCommon.float2int(this.value[this.n]); 
/* 1947 */     return h;
/*      */   }
/*      */   private void writeObject(ObjectOutputStream s) throws IOException {
/* 1950 */     double[] key = this.key;
/* 1951 */     float[] value = this.value;
/* 1952 */     MapIterator i = new MapIterator();
/* 1953 */     s.defaultWriteObject();
/* 1954 */     for (int j = this.size; j-- != 0; ) {
/* 1955 */       int e = i.nextEntry();
/* 1956 */       s.writeDouble(key[e]);
/* 1957 */       s.writeFloat(value[e]);
/*      */     } 
/*      */   }
/*      */   
...
/* 1966 */     double[] key = this.key = new double[this.n + 1];
...
/* 1974 */       int pos; double k = s.readDouble();
/* 1975 */       float v = s.readFloat();
/* 1976 */       if (Double.doubleToLongBits(k) == 0L) {
/* 1977 */         pos = this.n;
/* 1978 */         this.containsNullKey = true;
/*      */       } else {
/* 1980 */         pos = (int)HashCommon.mix(Double.doubleToRawLongBits(k)) & this.mask;
/* 1981 */         while (Double.doubleToLongBits(key[pos]) != 0L)
/* 1982 */           pos = pos + 1 & this.mask; 
/*      */       } 
/* 1984 */       key[pos] = k;
/* 1985 */       value[pos] = v;
/* 1986 */       if (this.first != -1) {
/* 1987 */         link[prev] = link[prev] ^ (link[prev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1988 */         link[pos] = link[pos] ^ (link[pos] ^ (prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1989 */         prev = pos; continue;
/*      */       } 
/* 1991 */       prev = this.first = pos;
/*      */       
/* 1993 */       link[pos] = link[pos] | 0xFFFFFFFF00000000L;
/*      */     } 
/*      */     
/* 1996 */     this.last = prev;
/* 1997 */     if (prev != -1)
/*      */     {
/* 1999 */       link[prev] = link[prev] | 0xFFFFFFFFL;
/*      */     }
/*      */   }
/*      */   
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\doubles\Double2FloatLinkedOpenHashMap.class
