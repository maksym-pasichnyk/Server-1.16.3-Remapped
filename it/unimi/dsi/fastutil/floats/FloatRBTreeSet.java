...
/*   70 */     this.actualComparator = FloatComparators.asFloatComparator(this.storedComparator);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  143 */     this(FloatIterators.asFloatIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  159 */     this(c);
/*  160 */     FloatArrays.ensureOffsetLength(a, offset, length);
...
/*  175 */     this(a, offset, length, (Comparator<? super Float>)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  230 */     return (this.actualComparator == null) ? Float.compare(k1, k2) : this.actualComparator.compare(k1, k2);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  863 */         return false; 
/*  864 */       Entry e = (Entry)o;
/*  865 */       return (Float.floatToIntBits(this.key) == Float.floatToIntBits(e.key));
/*      */     }
/*      */     public int hashCode() {
/*  868 */       return HashCommon.float2int(this.key);
/*      */     }
...
/*  939 */     int index = 0;
/*      */     SetIterator() {
/*  941 */       this.next = FloatRBTreeSet.this.firstEntry;
/*      */     }
/*      */     SetIterator(float k) {
/*  944 */       if ((this.next = FloatRBTreeSet.this.locateKey(k)) != null)
/*  945 */         if (FloatRBTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1009 */       updatePrevious();
/* 1010 */       updateNext();
/* 1011 */       FloatRBTreeSet.this.remove(this.curr.key);
/* 1012 */       this.curr = null;
/*      */     }
/*      */   }
/*      */   
...
/* 1029 */     return new Subset(0.0F, true, to, false);
/*      */   }
/*      */   
/*      */   public FloatSortedSet tailSet(float from) {
/* 1033 */     return new Subset(from, false, 0.0F, true);
/*      */   }
/*      */   
...
/* 1075 */       if (!bottom && !top && FloatRBTreeSet.this.compare(from, to) > 0) {
...
/* 1099 */       return ((this.bottom || FloatRBTreeSet.this.compare(k, this.from) >= 0) && (this.top || FloatRBTreeSet.this
/* 1100 */         .compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean contains(float k) {
/* 1105 */       return (in(k) && FloatRBTreeSet.this.contains(k));
/*      */     }
/*      */     
/*      */     public boolean add(float k) {
/* 1109 */       if (!in(k))
...
/* 1112 */       return FloatRBTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean remove(float k) {
/* 1117 */       if (!in(k))
/* 1118 */         return false; 
/* 1119 */       return FloatRBTreeSet.this.remove(k);
/*      */     }
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public FloatComparator comparator() {
/* 1137 */       return FloatRBTreeSet.this.actualComparator;
/*      */     }
/*      */     
/*      */     public FloatBidirectionalIterator iterator() {
...
/* 1151 */       return (FloatRBTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */     
/*      */     public FloatSortedSet tailSet(float from) {
/* 1155 */       if (this.bottom)
/* 1156 */         return new Subset(from, false, this.to, this.top); 
/* 1157 */       return (FloatRBTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */     
/*      */     public FloatSortedSet subSet(float from, float to) {
...
/* 1163 */       if (!this.top)
/* 1164 */         to = (FloatRBTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1165 */       if (!this.bottom)
/* 1166 */         from = (FloatRBTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1178 */       if (FloatRBTreeSet.this.tree == null) {
/* 1179 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1183 */       if (this.bottom) {
/* 1184 */         e = FloatRBTreeSet.this.firstEntry;
/*      */       } else {
/* 1186 */         e = FloatRBTreeSet.this.locateKey(this.from);
/*      */         
/* 1188 */         if (FloatRBTreeSet.this.compare(e.key, this.from) < 0) {
/* 1189 */           e = e.next();
/*      */         }
/*      */       } 
/*      */       
/* 1193 */       if (e == null || (!this.top && FloatRBTreeSet.this.compare(e.key, this.to) >= 0))
/* 1194 */         return null; 
/* 1195 */       return e;
/*      */     }
/*      */ 
...
/*      */       FloatRBTreeSet.Entry e;
/* 1204 */       if (FloatRBTreeSet.this.tree == null) {
/* 1205 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1209 */       if (this.top) {
/* 1210 */         e = FloatRBTreeSet.this.lastEntry;
/*      */       } else {
/* 1212 */         e = FloatRBTreeSet.this.locateKey(this.to);
/*      */         
/* 1214 */         if (FloatRBTreeSet.this.compare(e.key, this.to) >= 0) {
/* 1215 */           e = e.prev();
/*      */         }
/*      */       } 
/*      */       
/* 1219 */       if (e == null || (!this.bottom && FloatRBTreeSet.this.compare(e.key, this.from) < 0))
/* 1220 */         return null; 
/* 1221 */       return e;
/*      */     }
/*      */     
/*      */     public float firstFloat() {
/* 1225 */       FloatRBTreeSet.Entry e = firstEntry();
/* 1226 */       if (e == null)
...
/*      */     
/*      */     public float lastFloat() {
/* 1232 */       FloatRBTreeSet.Entry e = lastEntry();
/* 1233 */       if (e == null)
...
/*      */     {
/*      */       SubsetIterator() {
/* 1248 */         this.next = FloatRBTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       SubsetIterator(float k) {
/* 1251 */         this();
/* 1252 */         if (this.next != null)
/* 1253 */           if (!FloatRBTreeSet.Subset.this.bottom && FloatRBTreeSet.this.compare(k, this.next.key) < 0) {
/* 1254 */             this.prev = null;
/* 1255 */           } else if (!FloatRBTreeSet.Subset.this.top && FloatRBTreeSet.this.compare(k, (this.prev = FloatRBTreeSet.Subset.this.lastEntry()).key) >= 0) {
/* 1256 */             this.next = null;
/*      */           } else {
/* 1258 */             this.next = FloatRBTreeSet.this.locateKey(k);
/* 1259 */             if (FloatRBTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1270 */         if (!FloatRBTreeSet.Subset.this.bottom && this.prev != null && FloatRBTreeSet.this.compare(this.prev.key, FloatRBTreeSet.Subset.this.from) < 0)
/* 1271 */           this.prev = null; 
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1275 */         this.next = this.next.next();
/* 1276 */         if (!FloatRBTreeSet.Subset.this.top && this.next != null && FloatRBTreeSet.this.compare(this.next.key, FloatRBTreeSet.Subset.this.to) >= 0) {
/* 1277 */           this.next = null;
/*      */         }
/*      */       }
/*      */     }
...
/*      */     FloatRBTreeSet c;
/*      */     try {
/* 1296 */       c = (FloatRBTreeSet)super.clone();
...
/* 1370 */     if (n == 1) {
/* 1371 */       Entry entry = new Entry(s.readFloat());
...
/*      */ 
/*      */       
/* 1382 */       Entry entry = new Entry(s.readFloat());
/* 1383 */       entry.black(true);
/* 1384 */       entry.right(new Entry(s.readFloat()));
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\floats\FloatRBTreeSet.class
