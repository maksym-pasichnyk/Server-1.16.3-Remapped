...
/* 1848 */     float[] key = this.key;
/* 1849 */     byte[] value = this.value;
/* 1850 */     int mask = newN - 1;
/* 1851 */     float[] newKey = new float[newN + 1];
/* 1852 */     byte[] newValue = new byte[newN + 1];
...
/* 1858 */       int pos; if (Float.floatToIntBits(key[i]) == 0) {
/* 1859 */         pos = newN;
/*      */       } else {
/* 1861 */         pos = HashCommon.mix(HashCommon.float2int(key[i])) & mask;
/* 1862 */         while (Float.floatToIntBits(newKey[pos]) != 0)
/* 1863 */           pos = pos + 1 & mask; 
/*      */       } 
...
/*      */     try {
/* 1906 */       c = (Float2ByteLinkedOpenHashMap)super.clone();
...
/* 1914 */     c.key = (float[])this.key.clone();
/* 1915 */     c.value = (byte[])this.value.clone();
...
/* 1932 */       while (Float.floatToIntBits(this.key[i]) == 0)
/* 1933 */         i++; 
/* 1934 */       t = HashCommon.float2int(this.key[i]);
/* 1935 */       t ^= this.value[i];
/* 1936 */       h += t;
/* 1937 */       i++;
/*      */     } 
...
/* 1961 */     float[] key = this.key = new float[this.n + 1];
/* 1962 */     byte[] value = this.value = new byte[this.n + 1];
...
/* 1969 */       int pos; float k = s.readFloat();
/* 1970 */       byte v = s.readByte();
/* 1971 */       if (Float.floatToIntBits(k) == 0) {
/* 1972 */         pos = this.n;
/* 1973 */         this.containsNullKey = true;
/*      */       } else {
/* 1975 */         pos = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
/* 1976 */         while (Float.floatToIntBits(key[pos]) != 0)
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\floats\Float2ByteLinkedOpenHashMap.class
