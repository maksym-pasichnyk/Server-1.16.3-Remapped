...
/* 1151 */     float[] key = this.key;
/* 1152 */     short[] value = this.value;
/* 1153 */     int mask = newN - 1;
/* 1154 */     float[] newKey = new float[newN + 1];
/* 1155 */     short[] newValue = new short[newN + 1];
/* 1156 */     int i = this.n;
/* 1157 */     for (int j = realSize(); j-- != 0; ) {
/* 1158 */       while (Float.floatToIntBits(key[--i]) == 0); int pos;
/* 1159 */       if (Float.floatToIntBits(newKey[
/* 1160 */             pos = HashCommon.mix(HashCommon.float2int(key[i])) & mask]) != 0)
/* 1161 */         while (Float.floatToIntBits(newKey[pos = pos + 1 & mask]) != 0); 
...
/*      */     try {
/* 1187 */       c = (Float2ShortOpenHashMap)super.clone();
...
/* 1212 */       while (Float.floatToIntBits(this.key[i]) == 0)
/* 1213 */         i++; 
/* 1214 */       t = HashCommon.float2int(this.key[i]);
/* 1215 */       t ^= this.value[i];
/* 1216 */       h += t;
/* 1217 */       i++;
/*      */     } 
...
/* 1241 */     float[] key = this.key = new float[this.n + 1];
/* 1242 */     short[] value = this.value = new short[this.n + 1];
/*      */ 
/*      */     
/* 1245 */     for (int i = this.size; i-- != 0; ) {
/* 1246 */       int pos; float k = s.readFloat();
/* 1247 */       short v = s.readShort();
/* 1248 */       if (Float.floatToIntBits(k) == 0) {
/* 1249 */         pos = this.n;
/* 1250 */         this.containsNullKey = true;
/*      */       } else {
/* 1252 */         pos = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
/* 1253 */         while (Float.floatToIntBits(key[pos]) != 0)
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\floats\Float2ShortOpenHashMap.class
