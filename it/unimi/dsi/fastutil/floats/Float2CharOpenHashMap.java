...
/*      */   
/*      */   public boolean trim() {
/* 1046 */     int l = HashCommon.arraySize(this.size, this.f);
/* 1047 */     if (l >= this.n || this.size > HashCommon.maxFill(l, this.f))
/* 1048 */       return true; 
/*      */     try {
/* 1050 */       rehash(l);
/* 1051 */     } catch (OutOfMemoryError cantDoIt) {
/* 1052 */       return false;
/*      */     } 
/* 1054 */     return true;
/*      */   }
...
/* 1079 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1080 */       return true; 
/*      */     try {
/* 1082 */       rehash(l);
...
/* 1102 */     float[] key = this.key;
/* 1103 */     char[] value = this.value;
/* 1104 */     int mask = newN - 1;
/* 1105 */     float[] newKey = new float[newN + 1];
/* 1106 */     char[] newValue = new char[newN + 1];
/* 1107 */     int i = this.n;
/* 1108 */     for (int j = realSize(); j-- != 0; ) {
/* 1109 */       while (Float.floatToIntBits(key[--i]) == 0); int pos;
/* 1110 */       if (Float.floatToIntBits(newKey[
/* 1111 */             pos = HashCommon.mix(HashCommon.float2int(key[i])) & mask]) != 0)
/* 1112 */         while (Float.floatToIntBits(newKey[pos = pos + 1 & mask]) != 0); 
...
/*      */     try {
/* 1138 */       c = (Float2CharOpenHashMap)super.clone();
...
/* 1146 */     c.key = (float[])this.key.clone();
...
/* 1163 */       while (Float.floatToIntBits(this.key[i]) == 0)
/* 1164 */         i++; 
/* 1165 */       t = HashCommon.float2int(this.key[i]);
/* 1166 */       t ^= this.value[i];
/* 1167 */       h += t;
/* 1168 */       i++;
/*      */     } 
...
/* 1192 */     float[] key = this.key = new float[this.n + 1];
/* 1193 */     char[] value = this.value = new char[this.n + 1];
/*      */ 
/*      */     
/* 1196 */     for (int i = this.size; i-- != 0; ) {
/* 1197 */       int pos; float k = s.readFloat();
/* 1198 */       char v = s.readChar();
/* 1199 */       if (Float.floatToIntBits(k) == 0) {
/* 1200 */         pos = this.n;
/* 1201 */         this.containsNullKey = true;
/*      */       } else {
/* 1203 */         pos = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
/* 1204 */         while (Float.floatToIntBits(key[pos]) != 0)
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\floats\Float2CharOpenHashMap.class
