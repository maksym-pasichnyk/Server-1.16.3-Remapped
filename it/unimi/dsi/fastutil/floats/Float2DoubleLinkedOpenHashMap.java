...
/* 1855 */     float[] key = this.key;
/* 1856 */     double[] value = this.value;
/* 1857 */     int mask = newN - 1;
/* 1858 */     float[] newKey = new float[newN + 1];
...
/* 1865 */       int pos; if (Float.floatToIntBits(key[i]) == 0) {
/* 1866 */         pos = newN;
/*      */       } else {
/* 1868 */         pos = HashCommon.mix(HashCommon.float2int(key[i])) & mask;
/* 1869 */         while (Float.floatToIntBits(newKey[pos]) != 0)
/* 1870 */           pos = pos + 1 & mask; 
/*      */       } 
...
/*      */     try {
/* 1913 */       c = (Float2DoubleLinkedOpenHashMap)super.clone();
...
/* 1921 */     c.key = (float[])this.key.clone();
...
/* 1939 */       while (Float.floatToIntBits(this.key[i]) == 0)
/* 1940 */         i++; 
/* 1941 */       t = HashCommon.float2int(this.key[i]);
...
/* 1968 */     float[] key = this.key = new float[this.n + 1];
...
/* 1976 */       int pos; float k = s.readFloat();
/* 1977 */       double v = s.readDouble();
/* 1978 */       if (Float.floatToIntBits(k) == 0) {
/* 1979 */         pos = this.n;
/* 1980 */         this.containsNullKey = true;
/*      */       } else {
/* 1982 */         pos = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
/* 1983 */         while (Float.floatToIntBits(key[pos]) != 0)
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\floats\Float2DoubleLinkedOpenHashMap.class
