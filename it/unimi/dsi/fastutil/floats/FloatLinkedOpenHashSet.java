...
/*      */   
/*      */   public boolean trim() {
/* 1026 */     int l = HashCommon.arraySize(this.size, this.f);
/* 1027 */     if (l >= this.n || this.size > HashCommon.maxFill(l, this.f))
/* 1028 */       return true; 
/*      */     try {
/* 1030 */       rehash(l);
/* 1031 */     } catch (OutOfMemoryError cantDoIt) {
/* 1032 */       return false;
/*      */     } 
/* 1034 */     return true;
/*      */   }
...
/* 1058 */     int l = HashCommon.nextPowerOfTwo((int)Math.ceil((n / this.f)));
/* 1059 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1060 */       return true; 
/*      */     try {
/* 1062 */       rehash(l);
/* 1063 */     } catch (OutOfMemoryError cantDoIt) {
/* 1064 */       return false;
/*      */     } 
/* 1066 */     return true;
/*      */   }
...
/* 1082 */     float[] key = this.key;
/* 1083 */     int mask = newN - 1;
/* 1084 */     float[] newKey = new float[newN + 1];
/* 1085 */     int i = this.first, prev = -1, newPrev = -1;
/* 1086 */     long[] link = this.link;
/* 1087 */     long[] newLink = new long[newN + 1];
/* 1088 */     this.first = -1;
/* 1089 */     for (int j = this.size; j-- != 0; ) {
/* 1090 */       int pos; if (Float.floatToIntBits(key[i]) == 0) {
/* 1091 */         pos = newN;
/*      */       } else {
/* 1093 */         pos = HashCommon.mix(HashCommon.float2int(key[i])) & mask;
/* 1094 */         while (Float.floatToIntBits(newKey[pos]) != 0)
/* 1095 */           pos = pos + 1 & mask; 
/*      */       } 
...
/*      */     try {
/* 1136 */       c = (FloatLinkedOpenHashSet)super.clone();
...
/*      */   public int hashCode() {
/* 1156 */     int h = 0;
/* 1157 */     for (int j = realSize(), i = 0; j-- != 0; ) {
/* 1158 */       while (Float.floatToIntBits(this.key[i]) == 0)
/* 1159 */         i++; 
/* 1160 */       h += HashCommon.float2int(this.key[i]);
/* 1161 */       i++;
/*      */     } 
/*      */     
/* 1164 */     return h;
...
/* 1167 */     FloatIterator i = iterator();
/* 1168 */     s.defaultWriteObject();
/* 1169 */     for (int j = this.size; j-- != 0;)
/* 1170 */       s.writeFloat(i.nextFloat()); 
/*      */   }
/*      */   
...
/* 1178 */     float[] key = this.key = new float[this.n + 1];
/* 1179 */     long[] link = this.link = new long[this.n + 1];
/* 1180 */     int prev = -1;
/* 1181 */     this.first = this.last = -1;
/*      */     
/* 1183 */     for (int i = this.size; i-- != 0; ) {
/* 1184 */       int pos; float k = s.readFloat();
/* 1185 */       if (Float.floatToIntBits(k) == 0) {
/* 1186 */         pos = this.n;
/* 1187 */         this.containsNull = true;
/*      */       }
/* 1189 */       else if (Float.floatToIntBits(key[
/* 1190 */             pos = HashCommon.mix(HashCommon.float2int(k)) & this.mask]) != 0) {
/*      */         
/* 1192 */         while (Float.floatToIntBits(key[pos = pos + 1 & this.mask]) != 0);
/*      */       } 
/* 1194 */       key[pos] = k;
/* 1195 */       if (this.first != -1) {
/* 1196 */         link[prev] = link[prev] ^ (link[prev] ^ pos & 0xFFFFFFFFL) & 0xFFFFFFFFL;
/* 1197 */         link[pos] = link[pos] ^ (link[pos] ^ (prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L;
/* 1198 */         prev = pos; continue;
/*      */       } 
/* 1200 */       prev = this.first = pos;
/*      */       
/* 1202 */       link[pos] = link[pos] | 0xFFFFFFFF00000000L;
/*      */     } 
/*      */     
/* 1205 */     this.last = prev;
/* 1206 */     if (prev != -1)
/*      */     {
/* 1208 */       link[prev] = link[prev] | 0xFFFFFFFFL;
/*      */     }
/*      */   }
/*      */   
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\floats\FloatLinkedOpenHashSet.class
