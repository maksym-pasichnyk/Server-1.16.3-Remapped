...
/*      */   
/*      */   public boolean trim() {
/* 1099 */     int l = HashCommon.arraySize(this.size, this.f);
...
/* 1131 */     int l = HashCommon.nextPowerOfTwo((int)Math.ceil((n / this.f)));
/* 1132 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1133 */       return true; 
/*      */     try {
/* 1135 */       rehash(l);
...
/* 1155 */     float[] key = this.key;
/* 1156 */     float[] value = this.value;
/* 1157 */     int mask = newN - 1;
/* 1158 */     float[] newKey = new float[newN + 1];
/* 1159 */     float[] newValue = new float[newN + 1];
/* 1160 */     int i = this.n;
/* 1161 */     for (int j = realSize(); j-- != 0; ) {
/* 1162 */       while (Float.floatToIntBits(key[--i]) == 0); int pos;
/* 1163 */       if (Float.floatToIntBits(newKey[
/* 1164 */             pos = HashCommon.mix(HashCommon.float2int(key[i])) & mask]) != 0)
/* 1165 */         while (Float.floatToIntBits(newKey[pos = pos + 1 & mask]) != 0); 
...
/*      */     try {
/* 1191 */       c = (Float2FloatOpenHashMap)super.clone();
...
/* 1199 */     c.key = (float[])this.key.clone();
...
/* 1216 */       while (Float.floatToIntBits(this.key[i]) == 0)
/* 1217 */         i++; 
/* 1218 */       t = HashCommon.float2int(this.key[i]);
...
/* 1245 */     float[] key = this.key = new float[this.n + 1];
/* 1246 */     float[] value = this.value = new float[this.n + 1];
/*      */ 
/*      */     
/* 1249 */     for (int i = this.size; i-- != 0; ) {
/* 1250 */       int pos; float k = s.readFloat();
/* 1251 */       float v = s.readFloat();
/* 1252 */       if (Float.floatToIntBits(k) == 0) {
/* 1253 */         pos = this.n;
/* 1254 */         this.containsNullKey = true;
/*      */       } else {
/* 1256 */         pos = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
/* 1257 */         while (Float.floatToIntBits(key[pos]) != 0)
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\floats\Float2FloatOpenHashMap.class
