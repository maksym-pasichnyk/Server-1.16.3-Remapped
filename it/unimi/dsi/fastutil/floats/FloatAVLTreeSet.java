...
/*   69 */     this.actualComparator = FloatComparators.asFloatComparator(this.storedComparator);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  142 */     this(FloatIterators.asFloatIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  158 */     this(c);
/*  159 */     FloatArrays.ensureOffsetLength(a, offset, length);
...
/*  174 */     this(a, offset, length, (Comparator<? super Float>)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  229 */     return (this.actualComparator == null) ? Float.compare(k1, k2) : this.actualComparator.compare(k1, k2);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  905 */         return false; 
/*  906 */       Entry e = (Entry)o;
/*  907 */       return (Float.floatToIntBits(this.key) == Float.floatToIntBits(e.key));
/*      */     }
/*      */     public int hashCode() {
/*  910 */       return HashCommon.float2int(this.key);
/*      */     }
...
/*  980 */     int index = 0;
/*      */     SetIterator() {
/*  982 */       this.next = FloatAVLTreeSet.this.firstEntry;
/*      */     }
/*      */     SetIterator(float k) {
/*  985 */       if ((this.next = FloatAVLTreeSet.this.locateKey(k)) != null)
/*  986 */         if (FloatAVLTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1050 */       updatePrevious();
/* 1051 */       updateNext();
/* 1052 */       FloatAVLTreeSet.this.remove(this.curr.key);
/* 1053 */       this.curr = null;
/*      */     }
/*      */   }
/*      */   
...
/* 1070 */     return new Subset(0.0F, true, to, false);
/*      */   }
/*      */   
/*      */   public FloatSortedSet tailSet(float from) {
/* 1074 */     return new Subset(from, false, 0.0F, true);
/*      */   }
/*      */   
...
/* 1116 */       if (!bottom && !top && FloatAVLTreeSet.this.compare(from, to) > 0) {
...
/* 1140 */       return ((this.bottom || FloatAVLTreeSet.this.compare(k, this.from) >= 0) && (this.top || FloatAVLTreeSet.this
/* 1141 */         .compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean contains(float k) {
/* 1146 */       return (in(k) && FloatAVLTreeSet.this.contains(k));
/*      */     }
/*      */     
/*      */     public boolean add(float k) {
/* 1150 */       if (!in(k))
...
/* 1153 */       return FloatAVLTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean remove(float k) {
/* 1158 */       if (!in(k))
/* 1159 */         return false; 
/* 1160 */       return FloatAVLTreeSet.this.remove(k);
/*      */     }
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public FloatComparator comparator() {
/* 1178 */       return FloatAVLTreeSet.this.actualComparator;
/*      */     }
/*      */     
/*      */     public FloatBidirectionalIterator iterator() {
...
/* 1192 */       return (FloatAVLTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */     
/*      */     public FloatSortedSet tailSet(float from) {
/* 1196 */       if (this.bottom)
/* 1197 */         return new Subset(from, false, this.to, this.top); 
/* 1198 */       return (FloatAVLTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */     
/*      */     public FloatSortedSet subSet(float from, float to) {
...
/* 1204 */       if (!this.top)
/* 1205 */         to = (FloatAVLTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1206 */       if (!this.bottom)
/* 1207 */         from = (FloatAVLTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1219 */       if (FloatAVLTreeSet.this.tree == null) {
/* 1220 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1224 */       if (this.bottom) {
/* 1225 */         e = FloatAVLTreeSet.this.firstEntry;
/*      */       } else {
/* 1227 */         e = FloatAVLTreeSet.this.locateKey(this.from);
/*      */         
/* 1229 */         if (FloatAVLTreeSet.this.compare(e.key, this.from) < 0) {
/* 1230 */           e = e.next();
/*      */         }
/*      */       } 
/*      */       
/* 1234 */       if (e == null || (!this.top && FloatAVLTreeSet.this.compare(e.key, this.to) >= 0))
/* 1235 */         return null; 
/* 1236 */       return e;
/*      */     }
/*      */ 
...
/*      */       FloatAVLTreeSet.Entry e;
/* 1245 */       if (FloatAVLTreeSet.this.tree == null) {
/* 1246 */         return null;
/*      */       }
/*      */ 
/*      */       
/* 1250 */       if (this.top) {
/* 1251 */         e = FloatAVLTreeSet.this.lastEntry;
/*      */       } else {
/* 1253 */         e = FloatAVLTreeSet.this.locateKey(this.to);
/*      */         
/* 1255 */         if (FloatAVLTreeSet.this.compare(e.key, this.to) >= 0) {
/* 1256 */           e = e.prev();
/*      */         }
/*      */       } 
/*      */       
/* 1260 */       if (e == null || (!this.bottom && FloatAVLTreeSet.this.compare(e.key, this.from) < 0))
/* 1261 */         return null; 
/* 1262 */       return e;
/*      */     }
/*      */     
/*      */     public float firstFloat() {
/* 1266 */       FloatAVLTreeSet.Entry e = firstEntry();
/* 1267 */       if (e == null)
...
/*      */     
/*      */     public float lastFloat() {
/* 1273 */       FloatAVLTreeSet.Entry e = lastEntry();
/* 1274 */       if (e == null)
...
/*      */     {
/*      */       SubsetIterator() {
/* 1289 */         this.next = FloatAVLTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       SubsetIterator(float k) {
/* 1292 */         this();
/* 1293 */         if (this.next != null)
/* 1294 */           if (!FloatAVLTreeSet.Subset.this.bottom && FloatAVLTreeSet.this.compare(k, this.next.key) < 0) {
/* 1295 */             this.prev = null;
/* 1296 */           } else if (!FloatAVLTreeSet.Subset.this.top && FloatAVLTreeSet.this.compare(k, (this.prev = FloatAVLTreeSet.Subset.this.lastEntry()).key) >= 0) {
/* 1297 */             this.next = null;
/*      */           } else {
/* 1299 */             this.next = FloatAVLTreeSet.this.locateKey(k);
/* 1300 */             if (FloatAVLTreeSet.this.compare(this.next.key, k) <= 0) {
...
/* 1311 */         if (!FloatAVLTreeSet.Subset.this.bottom && this.prev != null && FloatAVLTreeSet.this.compare(this.prev.key, FloatAVLTreeSet.Subset.this.from) < 0)
/* 1312 */           this.prev = null; 
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1316 */         this.next = this.next.next();
/* 1317 */         if (!FloatAVLTreeSet.Subset.this.top && this.next != null && FloatAVLTreeSet.this.compare(this.next.key, FloatAVLTreeSet.Subset.this.to) >= 0) {
/* 1318 */           this.next = null;
/*      */         }
/*      */       }
/*      */     }
...
/*      */     FloatAVLTreeSet c;
/*      */     try {
/* 1337 */       c = (FloatAVLTreeSet)super.clone();
...
/* 1411 */     if (n == 1) {
/* 1412 */       Entry entry = new Entry(s.readFloat());
...
/*      */ 
/*      */       
/* 1422 */       Entry entry = new Entry(s.readFloat());
/* 1423 */       entry.right(new Entry(s.readFloat()));
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\floats\FloatAVLTreeSet.class
