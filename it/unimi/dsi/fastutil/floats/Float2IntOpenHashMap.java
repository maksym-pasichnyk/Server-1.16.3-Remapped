...
/*      */   
/*      */   public boolean trim() {
/* 1096 */     int l = HashCommon.arraySize(this.size, this.f);
...
/* 1129 */     if (l >= n || this.size > HashCommon.maxFill(l, this.f))
/* 1130 */       return true; 
/*      */     try {
/* 1132 */       rehash(l);
...
/* 1152 */     float[] key = this.key;
/* 1153 */     int[] value = this.value;
/* 1154 */     int mask = newN - 1;
/* 1155 */     float[] newKey = new float[newN + 1];
/* 1156 */     int[] newValue = new int[newN + 1];
/* 1157 */     int i = this.n;
/* 1158 */     for (int j = realSize(); j-- != 0; ) {
/* 1159 */       while (Float.floatToIntBits(key[--i]) == 0); int pos;
/* 1160 */       if (Float.floatToIntBits(newKey[
/* 1161 */             pos = HashCommon.mix(HashCommon.float2int(key[i])) & mask]) != 0)
/* 1162 */         while (Float.floatToIntBits(newKey[pos = pos + 1 & mask]) != 0); 
...
/*      */     try {
/* 1188 */       c = (Float2IntOpenHashMap)super.clone();
...
/* 1196 */     c.key = (float[])this.key.clone();
/* 1197 */     c.value = (int[])this.value.clone();
/* 1198 */     return c;
/*      */   }
/*      */ 
/*      */ 
...
/* 1213 */       while (Float.floatToIntBits(this.key[i]) == 0)
/* 1214 */         i++; 
/* 1215 */       t = HashCommon.float2int(this.key[i]);
/* 1216 */       t ^= this.value[i];
/* 1217 */       h += t;
/* 1218 */       i++;
/*      */     } 
...
/* 1222 */       h += this.value[this.n]; 
/* 1223 */     return h;
/*      */   }
/*      */   private void writeObject(ObjectOutputStream s) throws IOException {
/* 1226 */     float[] key = this.key;
/* 1227 */     int[] value = this.value;
...
/* 1242 */     float[] key = this.key = new float[this.n + 1];
/* 1243 */     int[] value = this.value = new int[this.n + 1];
/*      */ 
/*      */     
/* 1246 */     for (int i = this.size; i-- != 0; ) {
/* 1247 */       int pos; float k = s.readFloat();
/* 1248 */       int v = s.readInt();
/* 1249 */       if (Float.floatToIntBits(k) == 0) {
/* 1250 */         pos = this.n;
/* 1251 */         this.containsNullKey = true;
/*      */       } else {
/* 1253 */         pos = HashCommon.mix(HashCommon.float2int(k)) & this.mask;
/* 1254 */         while (Float.floatToIntBits(key[pos]) != 0)
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\i\\unimi\dsi\fastutil\floats\Float2IntOpenHashMap.class
