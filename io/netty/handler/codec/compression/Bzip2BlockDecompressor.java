...
/* 197 */         repeatIncrement <<= 1; continue;
/* 198 */       }  if (nextSymbol == 1) {
...
/*     */         }
/* 206 */         byte b = huffmanSymbolMap[mtfValue];
/* 207 */         bwtByteCounts[b & 0xFF] = bwtByteCounts[b & 0xFF] + repeatCount;
/* 208 */         while (--repeatCount >= 0) {
/* 209 */           bwtBlock[bwtBlockLength++] = b;
/*     */         }
/*     */         
/* 212 */         repeatCount = 0;
...
/* 227 */       bwtByteCounts[nextByte & 0xFF] = bwtByteCounts[nextByte & 0xFF] + 1;
...
/* 243 */     int[] characterBase = new int[256];
/*     */     
...
/*     */ 
/*     */     
/* 250 */     System.arraycopy(this.bwtByteCounts, 0, characterBase, 1, 255); int i;
/* 251 */     for (i = 2; i <= 255; i++) {
/* 252 */       characterBase[i] = characterBase[i] + characterBase[i - 1];
/*     */     }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */     
/* 259 */     for (i = 0; i < this.bwtBlockLength; i++) {
/* 260 */       int value = bwtBlock[i] & 0xFF;
/* 261 */       characterBase[value] = characterBase[value] + 1; bwtMergedPointers[characterBase[value]] = (i << 8) + value;
/*     */     } 
/*     */     
...
/* 285 */         this.crc.updateCRC(nextByte); continue;
/*     */       } 
/* 287 */       if (++this.rleAccumulator == 4) {
/*     */         
/* 289 */         int rleRepeat = decodeNextBWTByte() + 1;
/* 290 */         this.rleRepeat = rleRepeat;
/* 291 */         this.rleAccumulator = 0;
/* 292 */         this.crc.updateCRC(nextByte, rleRepeat); continue;
/*     */       } 
