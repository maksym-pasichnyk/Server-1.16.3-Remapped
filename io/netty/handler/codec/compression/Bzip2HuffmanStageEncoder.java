...
/*     */     
/*     */     int groupStart;
/* 217 */     for (groupStart = 0; groupStart < mtfLength; ) {
/*     */       
/* 219 */       int groupEnd = Math.min(groupStart + 50, mtfLength) - 1;
/*     */ 
/*     */       
/* 222 */       short[] cost = new short[totalTables];
/* 223 */       for (int j = groupStart; j <= groupEnd; j++) {
/* 224 */         int value = mtfBlock[j];
/* 225 */         for (int m = 0; m < totalTables; m++) {
/* 226 */           cost[m] = (short)(cost[m] + huffmanCodeLengths[m][value]);
/*     */         }
/*     */       } 
/*     */ 
/*     */       
/* 231 */       byte bestTable = 0;
/* 232 */       int bestCost = cost[0]; byte b1;
/* 233 */       for (b1 = 1; b1 < totalTables; b1 = (byte)(b1 + 1)) {
/* 234 */         int tableCost = cost[b1];
...
/* 243 */       for (int k = groupStart; k <= groupEnd; k++) {
/* 244 */         bestGroupFrequencies[mtfBlock[k]] = bestGroupFrequencies[mtfBlock[k]] + 1;
/*     */       }
/*     */ 
/*     */       
...
/*     */       
/* 285 */       int code = 0;
/* 286 */       for (int k = minimumLength; k <= maximumLength; k++) {
/* 287 */         for (int m = 0; m < mtfAlphabetSize; m++) {
/* 288 */           if ((huffmanCodeLengths[i][m] & 0xFF) == k) {
/* 289 */             huffmanMergedCodeSymbols[i][m] = k << 24 | code;
/* 290 */             code++;
/*     */           } 
/*     */         } 
/* 293 */         code <<= 1;
