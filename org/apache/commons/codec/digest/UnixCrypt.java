...
/* 182 */     return crypt(original, (String)null);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 202 */       salt = "" + SALT_CHARS[randomGenerator.nextInt(numSaltChars)] + SALT_CHARS[randomGenerator.nextInt(numSaltChars)];
/*     */     }
/* 204 */     else if (!salt.matches("^[./0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz]{2,}$")) {
...
/* 215 */     byte[] key = new byte[8]; int i;
/* 216 */     for (i = 0; i < key.length; i++) {
/* 217 */       key[i] = 0;
/*     */     }
/*     */     
/* 220 */     for (i = 0; i < key.length && i < original.length; i++) {
...
/* 245 */           u = 128;
/*     */         } 
/* 247 */         buffer.setCharAt(j, (char)COV2CHAR[c]);
/*     */       } 
/*     */     } 
...
/* 299 */     permOp(right, left, 1, 1431655765, results);
/* 300 */     right = results[0];
/* 301 */     left = results[1];
/* 302 */     permOp(left, right, 8, 16711935, results);
/* 303 */     left = results[0];
/* 304 */     right = results[1];
/* 305 */     permOp(right, left, 2, 858993459, results);
/* 306 */     right = results[0];
/* 307 */     left = results[1];
/* 308 */     permOp(left, right, 16, 65535, results);
/* 309 */     left = results[0];
/* 310 */     right = results[1];
/* 311 */     permOp(right, left, 4, 252645135, results);
/* 312 */     right = results[0];
/* 313 */     left = results[1];
...
/* 331 */     t = t >>> 4 | t << 28;
/* 332 */     el ^= SPTRANS[1][t & 0x3F] | SPTRANS[3][t >>> 8 & 0x3F] | SPTRANS[5][t >>> 16 & 0x3F] | SPTRANS[7][t >>> 24 & 0x3F] | SPTRANS[0][u & 0x3F] | SPTRANS[2][u >>> 8 & 0x3F] | SPTRANS[4][u >>> 16 & 0x3F] | SPTRANS[6][u >>> 24 & 0x3F];
/*     */ 
/*     */     
/* 335 */     return el;
/*     */   }
...
/* 343 */     permOp(d, c, 4, 252645135, results);
/* 344 */     d = results[0];
/* 345 */     c = results[1];
/* 346 */     c = hPermOp(c, -2, -859045888);
/* 347 */     d = hPermOp(d, -2, -859045888);
/* 348 */     permOp(d, c, 1, 1431655765, results);
/* 349 */     d = results[0];
/* 350 */     c = results[1];
/* 351 */     permOp(c, d, 8, 16711935, results);
/* 352 */     c = results[0];
/* 353 */     d = results[1];
/* 354 */     permOp(d, c, 1, 1431655765, results);
/* 355 */     d = results[0];
/* 356 */     c = results[1];
...
/* 368 */       c &= 0xFFFFFFF;
/* 369 */       d &= 0xFFFFFFF;
/* 370 */       int s = SKB[0][c & 0x3F] | SKB[1][c >>> 6 & 0x3 | c >>> 7 & 0x3C] | SKB[2][c >>> 13 & 0xF | c >>> 14 & 0x30] | SKB[3][c >>> 20 & 0x1 | c >>> 21 & 0x6 | c >>> 22 & 0x38];
/*     */ 
/*     */       
/* 373 */       int t = SKB[4][d & 0x3F] | SKB[5][d >>> 7 & 0x3 | d >>> 8 & 0x3C] | SKB[6][d >>> 15 & 0x3F] | SKB[7][d >>> 21 & 0xF | d >>> 22 & 0x30];
/*     */       
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\deobfuscated.jar!\org\apache\commons\codec\digest\UnixCrypt.class
